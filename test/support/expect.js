...
expect.extend({
	...
	toHaveOwnPropertySymbols(received, expectedKeys) {
		const keys = Object.getOwnPropertySymbols(received);
		const pass = keys.length === expectedKeys.length
			&& !keys.find((key, index) => key !== expectedKeys[index]);

		return {
			message: () => `expected ${printReceived(received)}${pass ? ' not' : ''} to have symbols ${printExpected(expectedKeys)} but has ${printReceived(keys)}`,
			pass
		};
	},
