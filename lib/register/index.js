...
const {
	useInternalModuleCache, useGlobalModuleCache, usingInternalModuleCache, exposeModule, Module
...
const {readFileSync, statSync} = require('fs'),
	{join: pathJoin, sep: pathSep} = require('path'),
 ...
 * @param {string} [options.skipCacheForDir] - If provided, skips cache for file in directory
 * @returns {undefined}
 */
function register(options) {
	...
	let {skipCacheForDir} = options;
	if (skipCacheForDir === null) {
		skipCacheForDir = undefined;
	} else if (skipCacheForDir !== undefined) {
		assert(
			isFullString(skipCacheForDir), 'options.skipCacheForDir must be a non-empty string if provided'
		);
		if (skipCacheForDir.slice(-1) !== pathSep) skipCacheForDir += pathSep;
	}
	...
	const piratesRevert = addHook((code, filename) => {
		...
		if (cache && (!skipCacheForDir || !filename.startsWith(skipCacheForDir))) {
		...
		if (cached) {
			...
			// Instrument code. Use internal module cache so any files `require`-ed by Livepack
			...
			if (shouldUseCache) cache.save(cacheKey, lastMod, code, map);
		}
