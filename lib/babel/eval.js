...
const {getBindingBlock} = require('./bindings.js'),
	...
	{
		getEvalIndirectVarName, getEvalDirectVarName, createTempVarNode, addToInternalVars
...
function processEvalCall(callPath, state) {
	...
	//   [...], // Scope vars
	//   true, // `isStrict` flag
	//   null // `argNames` (or null if `argNames` not defined)
	// )`
	const evalNode = callNode.callee;
	evalNode.name = getEvalDirectVarName();
...
function processEval(evalPath, state) {
	...
	const {node} = evalPath;
	node.name = getEvalIndirectVarName();
