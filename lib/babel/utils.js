...
// Imports
const {
		SCOPE_ID_VAR_NODE, BLOCK_ID, BLOCK_NAME, FUNCTION_PROPS, IS_INTERNAL, IS_EVAL_CODE, PROGRAM_PATH
	} = require('./symbols.js'),
...
function createTrackerComment(
	...
	if (superVarName && !superIsTemp) props.superVarName = superVarName;
	if (state[IS_EVAL_CODE]) props.isEval = true;
...
function initScope(blockPath, blockId, state) {
	// If block is a class, use scope var from parent block
	if (blockPath.isClass()) {
		const parentBlockPath = blockPath.findParent(path => path.isBlockStatement()) || state[PROGRAM_PATH];
		...
		blockPath[BLOCK_NAME] = blockPath.node.id.name;
