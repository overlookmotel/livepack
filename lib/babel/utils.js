...
// Imports
const {
		SCOPE_ID_VAR_NODE, BLOCK_ID, BLOCK_NAME, FUNCTION_PROPS, PARENT_FUNCTION_PATH,
		IS_INTERNAL, IS_EVAL_CODE
	} = require('./symbols.js'),
...
function recordVarUse(name, functionPath, bindingBlockPath, state) {
	...
	do {
		...
		// Use `PARENT_FUNCTION_PATH in path` rather than `path.isFunction()`
		// in order to skip over functions not entered yet.
		// This is possible if function is used in a method key
		// e.g. `const x = 'a', obj = { [ (() => x)() ]() {} }`.
		// In this case the method should not have vars added to its scope.
		functionPath = functionPath.findParent(
			path => PARENT_FUNCTION_PATH in path || path === bindingBlockPath
		);
 ...
 * @param {boolean} leadingComment - If `true`, insert comment as leading comment
 * @param {Object} state - Babel state object for file
 * @returns {undefined}
 */
function insertTrackerComment(
	bodyPath, id, scopes, isMethod, isProtoMethod, superVarName, superIsTemp, leadingComment, state
) {
	...
	const blockNode = bodyPath.node,
		commentsKey = leadingComment ? 'leadingComments' : 'innerComments';
	(blockNode[commentsKey] || (blockNode[commentsKey] = [])).unshift({
		type: 'CommentBlock',
