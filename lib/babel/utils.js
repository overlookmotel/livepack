 ...
 * @param {Object} commentHolderNode - AST node object for node to attach tracker comment to
 ...
 * @param {boolean} innerComment - If `true`, insert comment as inner comment
 * @param {Object} state - Babel state object for file
 * @returns {undefined}
 */
function insertTrackerComment(
	commentHolderNode, id, scopes, isMethod, isProtoMethod, superVarName, superIsTemp, innerComment, state
) {
	...
	const commentsKey = innerComment ? 'innerComments' : 'leadingComments';
	(commentHolderNode[commentsKey] || (commentHolderNode[commentsKey] = [])).unshift({
		type: 'CommentBlock',
