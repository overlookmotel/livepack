...
// Imports
const {SCOPE_ID_VAR_NAME, TEMP_VAR_NAME} = require('./constants.js'),
	{
		INTERNAL_VAR_NODES, SCOPE_ID_VAR_NODE,
		BLOCK_ID, BLOCK_NAME, FUNCTION_PROPS, IS_INTERNAL
	} = require('./symbols.js'),
	{TRACKER_COMMENT_PREFIX, TEMP_COMMENT_PREFIX} = require('../shared.js');
...
function initScope(blockPath, blockId, state) {
	...
	const scopeIdVarNode = internalIdentifier(`${SCOPE_ID_VAR_NAME}${blockId}`);
	state[INTERNAL_VAR_NODES].push(scopeIdVarNode);
...
function createTempVarNode(state) {
	const varNode = internalIdentifier(`${TEMP_VAR_NAME}${createBlockId(state)}`);
	state[INTERNAL_VAR_NODES].push(varNode);
	return varNode;
}
