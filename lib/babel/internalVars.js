...
const {internalIdentifier, createBlockId} = require('./utils.js'),
	{
		INTERNAL_VAR_NODES, INTERNAL_VAR_PREFIX_NUM,
		IMPORT_VAR_NODE, IMPORT_ASYNC_VAR_NODE, GLOBAL_VAR_NODE
	} = require('./symbols.js'),
	{
		INTERNAL_VAR_NAMES_PREFIX,
		TRACKER_VAR_NAME_BODY, SCOPE_ID_VAR_NAME_BODY, TEMP_VAR_NAME_BODY,
		IMPORT_VAR_NAME_BODY, IMPORT_ASYNC_VAR_NAME_BODY, MODULE_VAR_NAME_BODY, EXPORTS_VAR_NAME_BODY,
		GLOBAL_VAR_NAME_BODY, MAKE_DEFAULT_VAR_NAME_BODY,
...
const TRACKER_VAR_NAME = `${INTERNAL_VAR_NAMES_PREFIX}_${TRACKER_VAR_NAME_BODY}`,
	...
	IMPORT_VAR_NAME = `${INTERNAL_VAR_NAMES_PREFIX}_${IMPORT_VAR_NAME_BODY}`,
	IMPORT_ASYNC_VAR_NAME = `${INTERNAL_VAR_NAMES_PREFIX}_${IMPORT_ASYNC_VAR_NAME_BODY}`,
	MODULE_VAR_NAME = `${INTERNAL_VAR_NAMES_PREFIX}_${MODULE_VAR_NAME_BODY}_`,
	EXPORTS_VAR_NAME = `${INTERNAL_VAR_NAMES_PREFIX}_${EXPORTS_VAR_NAME_BODY}`,
	GLOBAL_VAR_NAME = `${INTERNAL_VAR_NAMES_PREFIX}_${GLOBAL_VAR_NAME_BODY}`,
	MAKE_DEFAULT_VAR_NAME = `${INTERNAL_VAR_NAMES_PREFIX}_${MAKE_DEFAULT_VAR_NAME_BODY}`,
...
module.exports = {
	...
	createTempVarNode,
	getOrCreateImportVarNode,
	getOrCreateImportAsyncVarNode,
	createModuleVarNode,
	createExportsVarNode,
	getOrCreateGlobalVarNode,
	createMakeDefaultVarNode,
	createEvalVarNode,
	createPrevalVarNode,
...
function getOrCreateImportVarNode(state) {
	return getOrCreateVar(IMPORT_VAR_NODE, IMPORT_VAR_NAME, state);
}

function getOrCreateImportAsyncVarNode(state) {
	return getOrCreateVar(IMPORT_ASYNC_VAR_NODE, IMPORT_ASYNC_VAR_NAME, state);
}

function createModuleVarNode(moduleId, state) {
	return createInternalVarNode(`${MODULE_VAR_NAME}${moduleId}`, state);
}

function createExportsVarNode(state) {
	return createInternalVarNode(EXPORTS_VAR_NAME, state);
}

function getOrCreateGlobalVarNode(state) {
	return getOrCreateVar(GLOBAL_VAR_NODE, GLOBAL_VAR_NAME, state);
}

function createMakeDefaultVarNode(state) {
	return createInternalVarNode(MAKE_DEFAULT_VAR_NAME, state);
}
...
/**
 * Get or create an internal var node.
 * If var node already exists, return it.
 * If doesn't exist, create it and return.
 * @param {Symbol} symbol - Symbol to store var node on `state` object with
 * @param {string} name - Variable name
 * @param {Object} state - Babel state object
 * @returns {Object} - AST node for var
 */
function getOrCreateVar(symbol, name, state) {
	let node = state[symbol];
	if (!node) {
		if (isFunction(name)) name = name();
		node = createInternalVarNode(name, state);
		state[symbol] = node;
	}
	return node;
}
