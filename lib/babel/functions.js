...
function functionDeclarationEnterVisitor(fnPath, state) {
	state.currentFullFunction = enterFunctionOrMethod(
		fnPath, fnPath.node.id.name, false, false, false, state
	);
}
...
function functionExpressionEnterVisitor(fnPath, state) {
	...
	state.currentFullFunction = enterFunctionOrMethod(fnPath, blockName, false, false, false, state);
...
function arrowFunctionEnterVisitor(fnPath, state) {
	...
	enterFunctionOrMethod(fnPath, undefined, false, false, hasNoBodyBlock, state);
}
...
function methodEnterVisitor(methodPath, state) {
	...
	// Create function
	enterFunctionOrMethod(methodPath, blockName, true, false, false, state);
}
...
function enterClassConstructor(constructorPath, state) {
	...
	const fn = enterFunctionOrMethod(constructorPath, 'constructor', false, true, false, state);
 ...
 */
function enterFunctionOrMethod(fnPath, blockName, isMethod, isClassConstructor, hasNoBodyBlock, state) {
	...
	// Enter function
	return enterFunctionOrClass(fnPath, paramsBlock.id, isStrict, isMethod, isClassConstructor, state);
}
