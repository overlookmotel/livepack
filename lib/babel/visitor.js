...
const {
		...
		arrowFunctionEnterVisitor, classMethodEnterVisitor, methodEnterVisitor,
		exitMethodKey, exitFunctionParams,
	...
	{
		blockStatementEnterVisitor, blockStatementExitVisitor, createBlockProps, insertBlockVars
	} = require('./blocks.js'),
	{variableDeclarationExitVisitor} = require('./params.js'),
	...
	{insertComment, containsUseStrictDirective} = require('./utils.js'),
	{
		...
		createEvalIndirectVarNode, createEvalDirectVarNode, createGetEvalVarNode,
		renameInternalVars
...
module.exports = function(programPath, babelState, evalState) {
	...
	programPath.traverse({
		...
		VariableDeclaration: {
			exit: path => variableDeclarationExitVisitor(path, state)
		},
...
function programExitVisitor(programPath, state, isEval) {
	...
	insertBlockVars(programPath.node, state);
...
function nodeExitVisitor(path, state) {
	...
	// Handle special cases with functions
	if (path.key === 'key') {
		const {parentPath} = path;
		if (parentPath.isMethod() && parentPath.node.kind !== 'constructor') {
			// Exiting method key - enter the method
			exitMethodKey(parentPath, state);
		}
	} else if (path.listKey === 'params') {
		const {parentPath} = path;
		if (parentPath.isFunction() && path.key === parentPath.node.params.length - 1) {
			// Exiting function params
			exitFunctionParams(parentPath, state);
		}
	}
}
