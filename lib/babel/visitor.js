...
const {
	...
	} = require('./utils.js'),
	{
		...
		createTrackerVarNode, createGetScopeIdVarNode, createEsmVarNode, createTempVarNode,
	...
	{
		TOP_BLOCK_ID, COMMON_JS_LOCAL_VAR_NAMES, TRANSFORMED_COMMENT, EVAL_COMMENT, LOADER_RESOLVE_PREFIX
...
module.exports = function(programPath, babelState, evalState) {
	...
	programPath.traverse({
		...
		Import: path => dynamicImportVisitor(path, state)
	});
...
function programEnterVisitor(path, babelState, evalState, filename) {
	// Init state
	const sourceType = babelState ? babelState.file.opts.sourceType : undefined,
	...
	const state = {
		...
		isCommonJs: sourceType === 'script',
		isStrict: isEsm,
		topLevelIsStrict: false,
		esmImports: undefined,
		esmExportNames: undefined,
		esmAliases: undefined,
		trackerVarNode: undefined,
		getScopeIdVarNode: undefined,
		esmVarNode: undefined,
		importUrlVarNodes: undefined,
		programPath: path,
	...
	// Find imported vars
	if (isEsm) recordImportsAndExports(path, state);
...
/**
 * Find static imports/exports and record their details.
 * Record a map of local var name to imported module and exported name.
 * Also create map of aliases from exported var name to local name for `export {x, x as y}`.
 * For namespace imports, call `livepack_esm.recordStatic()` with module object and resolved URL.
 * @param {Object} programPath - Babel path object for program
 * @param {Object} state - State object
 * @returns {undefined}
 */
function recordImportsAndExports(programPath, state) {
	state.importUrlVarNodes = Object.create(null); // Keyed by source specifier

	const importVars = Object.create(null), // Keyed by local var name
		exportNames = new Set(),
		aliases = [];

	const statementNodes = programPath.node.body,
		{topLevelVarNames} = state,
		exportNodes = [];
	let statementIndex;
	function insertStatement(node) {
		programPath.get(`body.${statementIndex}`).insertAfter(node);
		statementIndex++;
	}

	function insertRecordStaticStatement(moduleNamespaceVarNode, urlVarNode) {
		insertStatement(
			t.expressionStatement(t.callExpression(
				t.memberExpression(getOrCreateEsmVarNode(state), t.identifier('recordStatic')),
				[moduleNamespaceVarNode, urlVarNode]
			))
		);
	}

	for (statementIndex = 0; statementIndex < statementNodes.length; statementIndex++) {
		const statementNode = statementNodes[statementIndex];
		if (t.isImportDeclaration(statementNode)) {
			// Import statement
			const specifier = statementNode.source.value;

			// Record non-namespace specifiers + if namespace specifier, record module object
			for (const specifierNode of statementNode.specifiers) {
				if (t.isImportNamespaceSpecifier(specifierNode)) {
					...
					importVars[localNode.name] = {specifier, name: '*'};

					// `livepack_esm.recordStatic(mod, livepack_temp_1);`
					// where `mod` is the module namespace object and `livepack_temp_1` is variable
					// containing resolved URL
					insertRecordStaticStatement(localNode, getOrCreateUrlVarNode(specifier, state));
				} else {
					const localName = specifierNode.local.name;
					importVars[localName] = {
						specifier,
						name: t.isImportDefaultSpecifier(specifierNode) ? 'default' : specifierNode.imported.name
					};

					// Record as top-level var declaration.
					// This is a hack to work around Babel not seeing vars created by
					// `@babel/plugin-transform-react-jsx` in automatic mode when binding is searched for later.
					...
					topLevelVarNames.add(localName);
				}
			}
		} else if (t.isExportDefaultDeclaration(statementNode)) {
			// `export default ...`
			const declarationNode = statementNode.declaration;
			if (t.isFunctionDeclaration(declarationNode) || t.isClassDeclaration(declarationNode)) {
				const idNode = declarationNode.id;
				if (idNode) {
					const localName = idNode.name;
					exportNames.add(localName);
					aliases.push({name: 'default', targetName: localName, urlVarNode: undefined});
				}
			}
		} else if (t.isExportNamedDeclaration(statementNode)) {
			const declarationNode = statementNode.declaration,
				sourceNode = statementNode.source;
			if (sourceNode) {
				// `export ... from '...'`
				const urlVarNode = getOrCreateUrlVarNode(sourceNode.value, state);
				for (const specifierNode of statementNode.specifiers) {
					if (t.isExportSpecifier(specifierNode)) {
						// `export { x } from '...'` / `export { x as y } from '...'`
						aliases.push({
							name: specifierNode.exported.name, targetName: specifierNode.local.name, urlVarNode
						});
					} else {
						// `export * as ... from '...'`
						assertWithLocation(
							t.isExportNamespaceSpecifier(specifierNode), specifierNode, state,
							`Unexpected export named specifier type ${specifierNode.type}`
						);

						// Import namespace object and call `livepack_esm.recordStatic()` with it
						const namespaceVarNode = createTempVarNode(state);
						insertStatement(
							t.importDeclaration([t.importNamespaceSpecifier(namespaceVarNode)], sourceNode)
						);
						insertRecordStaticStatement(namespaceVarNode, urlVarNode);

						aliases.push({name: specifierNode.exported.name, targetName: '*', urlVarNode});
					}
				}
				...
				if (t.isFunctionDeclaration(declarationNode) || t.isClassDeclaration(declarationNode)) {
					...
					const {name} = declarationNode.id;
					exportNames.add(name);
					aliases.push({name, targetName: name, urlVarNode: undefined});
				} else {
					// `export const` / `export let` / `export var`
					assertWithLocation(
						t.isVariableDeclaration(declarationNode), declarationNode, state,
						`Unexpected export named declaration type ${declarationNode.type}`
					);

					for (const varDeclarationNode of declarationNode.declarations) {
						getVarNamesFromVarDeclaration(varDeclarationNode.id, exportNames, aliases, state);
					}
				}
			} else {
				// `export { ... as ... }` - store for 2nd pass.
				// Not processed now as may be re-exporting an imported var which is imported after this.
				exportNodes.push(statementNode);
			}
		} else if (t.isExportAllDeclaration(statementNode)) {
			// `export * from '...'`
			const urlVarNode = getOrCreateUrlVarNode(statementNode.source.value, state);
			aliases.push({name: '*', targetName: '*', urlVarNode});
		}
	}

	// Record `export { ... as ... }` statements
	for (const statementNode of exportNodes) {
		for (const specifierNode of statementNode.specifiers) {
			const localName = specifierNode.local.name,
				importVar = importVars[localName];
			if (importVar) {
				aliases.push({
					name: specifierNode.exported.name,
					targetName: importVar.name,
					urlVarNode: getOrCreateUrlVarNode(importVar.specifier, state)
				});
			} else {
				exportNames.add(localName);
				aliases.push({name: specifierNode.exported.name, targetName: localName, urlVarNode: undefined});
			}
		}
	}

	state.esmImports = importVars;
	state.esmExportNames = exportNames;
	state.esmAliases = aliases.length > 0 ? aliases : undefined;
}

function getVarNamesFromVarDeclaration(node, exportNames, aliases, state) {
	const {type} = node;
	if (type === 'Identifier') {
		const {name} = node;
		exportNames.add(name);
		...
		for (const propNode of node.properties) {
			...
			if (propType === 'ObjectProperty') {
				getVarNamesFromVarDeclaration(propNode.value, exportNames, aliases, state);
			} else {
				assertWithLocation(
					propType === 'RestElement', propNode, state,
					`Unexpected var declaration object property type '${propType}'`
				);
				getVarNamesFromVarDeclaration(propNode.argument, exportNames, aliases, state);
			}
		}
	} else if (type === 'ArrayPattern') {
		for (const elementNode of node.elements) {
			getVarNamesFromVarDeclaration(elementNode, exportNames, aliases, state);
		}
	} else {
		assertWithLocation(type === 'RestElement', node, state, `Unexpected var declaration type '${type}'`);
		getVarNamesFromVarDeclaration(node.argument, exportNames, aliases, state);
	}
}
...
function insertImportStatements(path, state) {
	const {isCommonJs} = state,
		statementNodes = [];
	function createImportNode(filePath) {
		if (isCommonJs) return t.callExpression(internalIdentifier('require'), [t.stringLiteral(filePath)]);

		const varNode = createTempVarNode(state);
		statementNodes.push(
			t.importDeclaration([t.importDefaultSpecifier(varNode)], t.stringLiteral(filePath))
		);
		return varNode;
	}

	// Insert imports of resolved import URLs
	// `import livepack_temp_1 from 'livepackresolve://./imported.js';`
	const urlVarNodes = state.importUrlVarNodes;
	if (urlVarNodes) {
		const urlVarNodePairs = Object.entries(urlVarNodes);
		if (urlVarNodePairs.length > 0) {
			statementNodes.push(
				...urlVarNodePairs.map(([specifier, urlVarNode]) => (
					t.importDeclaration(
						[t.importDefaultSpecifier(urlVarNode)],
						t.stringLiteral(`${LOADER_RESOLVE_PREFIX}${specifier}`)
					)
				))
			);
		}
	...
	// CJS: `const [livepack_tracker, livepack_getScopeId] = require('/path/to/app/node_modules/livepack/lib/init/index.js')('/path/to/app/src/index.js', module, require);`
	// ESM: `import livepack_temp_1 from '/path/to/app/node_modules/livepack/lib/init/index.js'; const [livepack_tracker, livepack_getScopeId, livepack_esm] = livepack_temp_1('/path/to/app/src/index.js', import.meta, [['x', 'y']]);`
	/* eslint-enable max-len */
	const argNodes = [t.stringLiteral(state.filename)];
	if (isCommonJs) {
		argNodes.push(t.identifier('module'), t.identifier('require'));
	} else {
		argNodes.push(t.metaProperty(t.identifier('import'), t.identifier('meta')));

		const aliases = state.esmAliases;
		if (aliases) {
			argNodes.push(
				t.arrayExpression(
					aliases.map(({name, targetName, urlVarNode}) => t.arrayExpression([
						t.stringLiteral(name),
						t.stringLiteral(targetName),
						...(urlVarNode ? [urlVarNode] : [])
					]))
				)
			);
		}
	...
	const {esmVarNode} = state;
	statementNodes.push(
		t.variableDeclaration(
			'const', [
				t.variableDeclarator(
					t.arrayPattern([
						state.trackerVarNode,
						state.getScopeIdVarNode,
						...(esmVarNode ? [esmVarNode] : [])
					]),
					t.callExpression(createImportNode(INIT_PATH), argNodes)
				)
			]
		)
	);
	...
	/* eslint-disable max-len */
	// CJS: `const [livepack_eval, livepack_preval, livepack_getEval] = require('/path/to/app/node_modules/livepack/lib/init/eval.js')('/path/to/app/src/index.js', 123, 0);`
	// ESM: `import livepack_temp_2 from '/path/to/app/node_modules/livepack/lib/init/eval.js'; const [livepack_eval, livepack_preval, livepack_getEval] = livepack_temp_2('/path/to/app/src/index.js', 123, 0);`
	/* eslint-enable max-len */
	if (state.evalIsUsed) {
		statementNodes.push(
			t.variableDeclaration(
	...
	addComments(path.node, false, false, {type: 'CommentBlock', value: TRANSFORMED_COMMENT});
	...
	if (classNode.superClass) {
		...
		addComments(
			...
			createTrackerComment(
				createBlockId(state), [], true, false, false, undefined, undefined, undefined, state
			)
		);
	}
}
...
function functionExitVisitor(fnPath, state) {
	...
	// Create scopes for tracker call
	const scopeNodes = scopes.map(({scopeIdVarNode, varNames}) => (
		t.arrayExpression([
			scopeIdVarNode,
	...
	// Identify imported/exported ESM vars.
	// If imported vars, add array of resolved URLs to scopes array.
	// If exported vars, add this module's URL to end of scopes array.
	// `importVars` is mapping of local name to target name. `exportNames` is array of local names.
	// Both will be added to tracker comment.
	let importVars, exportNames;
	if (scopes.length > 0) {
		const topScope = scopes[0];
		if (topScope.blockId === TOP_BLOCK_ID) {
			const fileImportVars = state.esmImports;
			if (fileImportVars) {
				const fileExportNames = state.esmExportNames,
					importUrlNodes = [];
				importVars = [];
				exportNames = [];
				for (const varName of topScope.varNames) {
					const importVar = fileImportVars[varName];
					if (importVar) {
						// Imported var
						importVars.push([varName, importVar.name]);
						importUrlNodes.push(getOrCreateUrlVarNode(importVar.specifier, state));
					} else if (fileExportNames.has(varName)) {
						// Exported var
				...
				if (importVars.length > 0) {
					// Some imported vars. Add array of source URLs to tracker code.
					scopeNodes.push(t.arrayExpression(importUrlNodes));
				} else {
					importVars = undefined;
				}

				if (exportNames.length > 0) {
					// Some exported vars. Add `livepack_esm.url` to tracker code.
					// NB Using `livepack_esm.url` rather than `import.meta.url` as `import.meta.url` is mutable -
					// user code could have altered it.
					scopeNodes.push(t.memberExpression(getOrCreateEsmVarNode(state), t.identifier('url')));
				} else {
	...
	const trackerFnNode = t.arrowFunctionExpression([], t.arrayExpression(scopeNodes));
	...
	addComments(
		...
		createTrackerComment(
			props.id, scopes, props.isStrict, isMethod, props.superIsProto,
			importVars, exportNames, props.argNames, state
		)
	);
...
function identifierVisitor(identifierPath, state) {
	...
	if (!binding) {
		...
		if (bindingBlockPath.isExportNamedDeclaration() || bindingBlockPath.isExportDefaultDeclaration()) {
			bindingBlockPath = state.programPath;
		} else {
			assertWithLocation(
				bindingBlockPath.isBlockStatement() || bindingBlockPath.isProgram()
				|| bindingBlockPath.isClass(),
...
function getBindingLocation(bindingPath, identifierPath, name, state) {
	// Imports are bound to top level scope
	if (
		bindingPath.isImportDefaultSpecifier() || bindingPath.isImportSpecifier()
		|| bindingPath.isImportNamespaceSpecifier()
	) {
		return {path: state.programPath, isConst: true};
	}
...
function superVisitor(superPath, state) {
	...
	// If `super` is used in a function nested within method, and usage is something other than `super()`,
	// the transpiled `super` will need access to `this` from within method.
	if (
		fnPath !== methodPath
...
/**
 * Visitor to instrument dynamic import statements to record module namespace object.
 * @param {Object} importPath - Babel path object for `super`
 * @param {Object} state - Babel state object for file
 * @returns {undefined}
 */
function dynamicImportVisitor(importPath, state) {
	const callPath = importPath.parentPath;
	if (!callPath.isCallExpression() || importPath.key !== 'callee') return;

	// `import('foo')` ->
	// `((specifier) => livepack_esm.recordDynamic(import(specifier), import(`livepackresolve://${specifier}`))('foo' + '')`
	// Replace node directly rather than using `importPath.replaceWith()`
	// so Babel doesn't continue traversing into the replacement code.
	const esmVarNode = getOrCreateEsmVarNode(state),
		specifierNode = internalIdentifier('specifier'),
		callNode = callPath.node;
	callNode.callee = t.arrowFunctionExpression(
		[specifierNode],
		t.callExpression(
			t.memberExpression(esmVarNode, t.identifier('recordDynamic')),
			[
				t.callExpression(t.import(), [specifierNode]),
				t.callExpression(t.import(), [
					t.templateLiteral(
						[t.templateElement({raw: LOADER_RESOLVE_PREFIX}), t.templateElement({raw: ''}, true)],
						[specifierNode]
					)
				])
			]
	...
	// Convert specifier to string to avoid `.toString()` being called twice
	// if specifier is an Object with a `.toString()` method
	callNode.arguments[0] = t.binaryExpression('+', callNode.arguments[0], t.stringLiteral(''));
}
...
function processEval(evalPath, state) {
	...
	if (!parentPath.isCallExpression() || evalPath.key !== 'callee') {
		...
		addComments(node, false, true, {type: 'CommentBlock', value: EVAL_COMMENT});
...
/**
 * Get or create `livepack_esm` var node for this file.
 * @param {Object} state - Babel state object for file
 * @returns {Object} - AST node
 */
function getOrCreateEsmVarNode(state) {
	// eslint-disable-next-line no-return-assign
	return state.esmVarNode || (state.esmVarNode = createEsmVarNode(state));
}

/**
 * Get or create temp var node for resolved URL for import specifier.
 * @param {string} specifier - Import specifier e.g. './imported.js', 'react'
 * @param {Object} state - Babel state object for file
 * @returns {Object} - AST node for var
 */
function getOrCreateUrlVarNode(specifier, state) {
	const urlVarNodes = state.importUrlVarNodes;
	// eslint-disable-next-line no-return-assign
	return urlVarNodes[specifier] || (urlVarNodes[specifier] = createTempVarNode(state));
}
