...
const {
	...
	} = require('./internalVars.js'),
	{
		...
		PARENT_FUNCTION_PATH, PARENT_FULL_FUNCTION_PATH, SUPER_VAR_NODE
	} = require('./symbols.js'),
	{
...
function functionExitVisitor(fnPath, state) {
	...
	if (superVarNode) {
		...
		if (isInternalNode(superVarNode)) superIsTemp = true;
	}
...
function getBindingLocation(bindingPath, identifierPath, name, state) {
	...
	if (bindingPath.isFunctionDeclaration()) {
		if (!bindingPath[PARENT_IS_STRICT]) {
		...
		return {path: bindingPath.parentPath, isConst: false};
	}
...
function superVisitor(superPath, state) {
	...
	const fnPath = state.parentFullFunctionPath;
	...
	const isClass = fnPath.isClassMethod(),
		...
		blockPath = encloserPath.findParent(path => path.isBlockStatement()) || state.programPath;
	...
	if (!isClass || varNode !== encloserNode.id) {
		recordVarUse(varNode.name, fnPath, blockPath, false, state);
	} else if (fnPath.node.kind !== 'constructor') {
		// Class method where class name accessible - bind to class itself, not block above
		recordVarUse(varNode.name, fnPath, encloserPath, true, state);
	}
...
function processEval(evalPath, state) {
	...
	firstArgPath.replaceWith(t.callExpression(
		...
		[
			...
			...(state.isStrict ? [t.booleanLiteral(true)] : [])
		]
	));
