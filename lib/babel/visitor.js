...
function classExitVisitor(classPath, state) {
	...
	if (classNode.superClass) {
		...
		const commentNode = createTrackerComment(
			createBlockId(state), [], false, false, undefined, false, state
		);
		const bodyNode = classNode.body;
		(bodyNode.innerComments || (bodyNode.innerComments = [])).unshift(commentNode);
	}
}
...
function functionExitVisitor(fnPath, state) {
	...
	// Create tracker comment and determine where it should be inserted.
	// Tracker comment will be inserted:
	...
	const commentNode = createTrackerComment(
		props.id, scopes, isMethod, isProtoMethod, superVarName, superIsTemp, state
	);
	...
	if (!commentHolderNode) commentHolderNode = trackerFnNode;
	(commentHolderNode[commentsKey] || (commentHolderNode[commentsKey] = [])).unshift(commentNode);
