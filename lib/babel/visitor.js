...
const {
	...
	{
		SCOPE_ID_VAR_NODE, ESM_VAR_NODE, TEMP_VAR_NODES,
		IS_ESM, ESM_IMPORTS, ESM_EXPORT_NAMES, ESM_ALIASES, IMPORT_URL_VAR_NODES,
		FUNCTION_PROPS, HAS_CONSTRUCTOR, PARENT_IS_STRICT,
	...
	{
		TOP_BLOCK_ID, COMMON_JS_LOCAL_VAR_NAMES, TRANSFORMED_COMMENT, EVAL_COMMENT
...
function programEnterVisitor(path, babelState, evalState, filename) {
	...
	const state = {
		...
		[IS_ESM]: isEsm,
		[ESM_VAR_NODE]: undefined,
		[IMPORT_URL_VAR_NODES]: undefined,
		[ESM_IMPORTS]: undefined,
		[ESM_EXPORT_NAMES]: undefined,
...
function recordImportsAndExports(programPath, state) {
	state[IMPORT_URL_VAR_NODES] = Object.create(null); // Keyed by source specifier
	...
	state[ESM_IMPORTS] = importVars;
	state[ESM_EXPORT_NAMES] = exportNames;
	state[ESM_ALIASES] = aliases.length > 0 ? aliases : undefined;
}
...
function insertImportStatements(path, state) {
	...
	// Insert var definitions for resolved import URLs
	// `const livepack_temp_1 = await import.meta.resolve('./imported.js');`
	const urlVarNodes = state[IMPORT_URL_VAR_NODES];
	if (urlVarNodes) {
		...
		if (urlVarNodePairs.length > 0) {
			statementNodes.push(
				t.variableDeclaration(
					'const',
					urlVarNodePairs.map(([specifier, urlVarNode]) => t.variableDeclarator(
						urlVarNode,
						t.awaitExpression(t.callExpression(
							t.memberExpression(
								t.metaProperty(t.identifier('import'), t.identifier('meta')), t.identifier('resolve')
							),
	...
	if (isCommonJs) {
		...
		const aliases = state[ESM_ALIASES];
		if (aliases) {
			argNodes.push(
				t.arrayExpression(
	...
	const esmVarNode = state[ESM_VAR_NODE];
	statementNodes.push(
		t.variableDeclaration(
			'const', [
...
function functionExitVisitor(fnPath, state) {
	...
	if (scopes.length > 0) {
		...
		if (topScope.blockId === TOP_BLOCK_ID) {
			const fileImportVars = state[ESM_IMPORTS];
			if (fileImportVars) {
				const fileExportNames = state[ESM_EXPORT_NAMES],
					importUrlNodes = [];
				importVars = [];
				exportNames = [];
...
function dynamicImportVisitor(importPath, state) {
	...
	// `import('foo')` ->
	// `((specifier) => livepack_esm.recordDynamic(import(specifier), specifier))('foo')`
...
function getOrCreateEsmVarNode(state) {
	...
	return state[ESM_VAR_NODE] || (state[ESM_VAR_NODE] = createEsmVarNode(state));
}
