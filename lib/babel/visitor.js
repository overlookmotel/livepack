...
const {
	...
	{forEnterVisitor, forExitVisitor, whileEnterVisitor, whileExitVisitor} = require('./forWhile.js'),
...
module.exports = function(programPath, babelState, evalState) {
	...
	programPath.traverse({
		...
		For: {
			enter: path => forEnterVisitor(path, state),
			exit: path => forExitVisitor(path, state)
		},
		While: {
			enter: path => whileEnterVisitor(path, state),
			exit: path => whileExitVisitor(path, state)
		},
