...
function functionExitVisitor(fnPath, state) {
	...
	addComments(
		...
		createTrackerComment(props.id, scopes, props.isStrict, isMethod, isProtoMethod, state)
	);
...
function identifierVisitor(identifierPath, state) {
	...
	// Record variable use
	recordVarUse(name, functionPath, bindingBlockPath, isConst, undefined, state);
}
...
function superVisitor(superPath, state) {
	...
	// Record var use
	recordVarUse('super', fnPath, encloserPath, true, superVar.node, state);
}
...
function processThisOrArguments(name, state) {
		...
		if (functionPath) {
			...
			if (parentVar) {
				const scopes = recordVarUse(
					name, functionPath, createParentVarBindingBlockPath(parentVar), isConst, undefined, state
				);
	...
	// Record variable use
	const scopes = recordVarUse(name, functionPath, bindingBlockPath, isConst, undefined, state);
