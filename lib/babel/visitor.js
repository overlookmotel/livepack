...
const pathJoin = require('path').join,
	{isModule} = require('@babel/helper-module-transforms'),
...
const {
		...
		internalIdentifier, flagAsInternal, tagTempVarWithType, createParentVarBindingBlockPath,
		assertWithLocation
	} = require('./utils.js'),
	{
		initInternalVars, checkInternalVarNameClash,
		createTrackerVarNode, getOrCreateImportAsyncVarNode, createTempVarNode, getOrCreateGlobalVarNode,
	...
	} = require('./internalVars.js'),
	{
		...
		IS_ESM, IMPORT_VAR_NODE, IMPORT_ASYNC_VAR_NODE, EXPORTS_VAR_NODE, GLOBAL_VAR_NODE, MODULE_VARS,
		BLOCK_ID, EVAL_IS_USED, PARENT_VARS, IS_EVAL_CODE,
		FUNCTION_PROPS, HAS_CONSTRUCTOR,
		PROGRAM_PATH, PARENT_FUNCTION_PATH, PARENT_FULL_FUNCTION_PATH,
		SUPER_VAR_NODE, IS_INTERNAL
	} = require('./symbols.js'),
	{identifierIsVariable, replaceWith, isInternalPath} = require('../shared/functions.js'),
	{transformEsModule, wrapInBlockIfReservedVars, getEsmHeaders} = require('./esm.js'),
...
function programEnterVisitor(path, state) {
	...
	// Init ES modules state
	const isEsm = isModule(path);
	state[IS_ESM] = isEsm;
	state[IMPORT_VAR_NODE] = null;
	state[IMPORT_ASYNC_VAR_NODE] = null;
	state[EXPORTS_VAR_NODE] = null;
	state[GLOBAL_VAR_NODE] = null;
	state[MODULE_VARS] = new Map();
	...
	if (isEsm) transformEsModule(path, state);
}
...
function programExitVisitor(path, state, isEval) {
	...
	// Wrap program in block if is ESM and CommonJS vars used at top level
	wrapInBlockIfReservedVars(path, state);

	// Insert `init`, `eval` import statements and ESM headers at top of file
...
function insertImportStatements(path, state) {
	...
	const argNodes = [t.identifier('__filename'), t.identifier('module'), t.identifier('require')];

	if (state[IS_ESM]) {
		argNodes.push(
			t.newExpression(t.identifier('Map'), [
				t.arrayExpression(
					[...state[MODULE_VARS].entries()].map(([varName, {key, url, moduleVarNode}]) => (
						t.arrayExpression([
							t.stringLiteral(varName),
							t.objectExpression([
								t.objectProperty(t.identifier('moduleVarName'), t.stringLiteral(moduleVarNode.name)),
								t.objectProperty(t.identifier('key'), key ? t.stringLiteral(key) : t.nullLiteral()),
								...(url ? [t.objectProperty(t.identifier('url'), t.stringLiteral(url))] : [])
							])
						])
					))
				)
	...
	// Insert eval import statement
	// eslint-disable-next-line max-len
	// `const [livepack_eval, livepack_preval, livepack_getEval] = require('/path/to/app/node_modules/livepack/lib/init/eval.js')(__filename, 100, 0);`
	...
	// Insert ESM headers
	statementNodes.push(...getEsmHeaders(state));
...
function functionExitVisitor(fnPath, state) {
	...
	// Skip module vars (`import` or `export`)
	if (scopes.length > 0 && scopes[0].blockId === 0) scopes.shift();
...
function identifierVisitor(identifierPath, state) {
	...
	if (!functionPath) {
		...
		if (name === 'eval' && !identifierPath.scope.getBinding('eval')) {
			processEval(identifierPath, state);
			return;
		}

		// Handle module vars outside functions
		const binding = identifierPath.scope.getBinding(name);
		if (binding) {
			const bindingBlockPath = locateBindingBlockPath(binding.path, name, state);
			if (!bindingBlockPath.isProgram()) return;
		}
		handleModuleVar(identifierPath, name, state);

		// TODO Replace `exports` in ESM file with `global.exports` (ditto `module`, `require`)
	...
	if (!binding) {
		// Global var (or could be import var where original import statement has now been removed)
		...
		if (parentVar) {
			...
			if (state[IS_ESM]) {
				// Replace `exports` in ESM file with `global.exports` (ditto `module`, `require`)
				const globalVarNode = getOrCreateGlobalVarNode(state);
				identifierPath.replaceWith(t.memberExpression(globalVarNode, identifierPath.node));
				return;
			}

			// In CommonJS files, treat `exports` etc as external vars, not globals
	...
	// Handle module vars.
	// NB Use `t.isProgram()` rather than `bindingBlockPath.isProgram()` for case where
	// `bindingBlockPath` is synthetic binding block created for parent var in eval.
	if (!bindingBlockPath || t.isProgram(bindingBlockPath.node)) {
		const moduleVarName = handleModuleVar(identifierPath, name, state);
		if (moduleVarName) {
			bindingBlockPath = {
				[BLOCK_ID]: 0,
				[SCOPE_ID_VAR_NODE]: t.numericLiteral(0)
			};
			recordVarUse(name, functionPath, bindingBlockPath, state);
			return;
		}
		if (!bindingBlockPath) return;
	}
...
function handleModuleVar(identifierPath, name, state) {
	const moduleVar = state[MODULE_VARS].get(name);
	if (!moduleVar) return null;

	const {key, moduleVarNode, url} = moduleVar;
	if (!key) {
		identifierPath.replaceWith(moduleVarNode);
	} else {
		const {parentPath} = identifierPath;
		if (!url && parentPath.isAssignmentExpression() && identifierPath.key === 'left') {
			// Assignment - transform `x = 1` to `livepack_exports.x = x = 1`
			// so that if `x` is a const, it will result in error.
			// Not needed for imports as module object proxy will throw if write to its properties.
			// TODO Also need to recognise and replace `([x] = [1])`, `({x} = {x: 1})`, `[...x] = []`.
			flagAsInternal(identifierPath.node); // Prevent being visited again
			parentPath.replaceWith(t.assignmentExpression(
				'=',
				t.memberExpression(moduleVarNode, t.identifier(key)),
				parentPath.node
			));
		} else {
			// Not assignment or export - replace `x` with `livepack_module_0.x`
			identifierPath.replaceWith(t.memberExpression(moduleVarNode, t.identifier(key)));
		}
	}
	return moduleVarNode.name;
}

function locateBindingBlockPath(bindingPath, name, state) {
	// TODO In non-strict mode, function declarations (but not class declarations)
	...
	if (bindingPath.isFunctionDeclaration() || bindingPath.isClassDeclaration()) {
		...
		// Workaround bug in Babel where `export function x() {}`
		// TODO Find better way to fix this - maybe transform import/export vars after everything else?
		if (bindingParentPath.node === bindingPath.node) bindingParentPath = bindingParentPath.parentPath;
	...
	if (
		...
		// Import statement - scoped to program
		return bindingPath.parentPath.parentPath;
	}
	...
	if (bindingParentPath.node.kind === 'var') {
		...
		if (bindingFnPath) return bindingFnPath.get('body');
		return state[PROGRAM_PATH];
	}
	...
	if (varParentPath.type === 'ExportNamedDeclaration') return varParentPath.parentPath;
		...
		if (functionPath) {
			...
			if (parentVar) {
				const scopes = recordVarUse(
					name, functionPath, createParentVarBindingBlockPath(parentVar), state
				);
...
/**
 * Visitor to transform dynamic `import()` to `livepack_importAsync()`
 * @param {Object} importPath - Babel path object for `import`
 * @param {Object} state - Babel state object for file
 * @returns {undefined}
 */
function importVisitor(importPath, state) {
	importPath.replaceWith(getOrCreateImportAsyncVarNode(state));
}
