...
const {
	...
	} = require('./internalVars.js'),
	{
		...
		PROGRAM_PATH, PARENT_FUNCTION_PATH, PARENT_FULL_FUNCTION_PATH, METHOD_ENTRY_PATH,
...
module.exports = function(programPath, state) {
	...
	programPath.traverse({
		enter: path => methodVisitor(path, state),
		Class: {
...
function classExitVisitor(classPath, state) {
	...
	if (classPath.node.superClass) {
		...
		insertTrackerComment(
			classPath.get('body'), createBlockId(state), [], false, false, undefined, false, false, state
		);
	}
}
...
function functionEnterVisitor(fnPath, state) {
	...
	if (fnPath.isMethod() && fnPath.node.computed) {
		// Method with computed key.
		// Key should be treated as within parent function so delay entry to this one.
		// Setting `state[METHOD_ENTRY_PATH]` will cause `methodVisitor()` to be called at the entry point.
		// Locate path to enter on - first argument, or function body if no args.
		if (state[METHOD_ENTRY_PATH]) fnPath[METHOD_ENTRY_PATH] = state[METHOD_ENTRY_PATH];
		state[METHOD_ENTRY_PATH] = fnPath.node.params.length > 0
			? fnPath.get('params.0')
			: fnPath.get('body');
	} else {
		functionEnterUpdateState(fnPath, state);
	}
}

function functionEnterUpdateState(fnPath, state) {
...
/**
 * Visitor to enter method late (object method or class method).
 * `state[METHOD_ENTRY_PATH]` will only be set if in a method which has a computed key.
 * It will be set to be the path for first node after the key.
 * This is necessary to ensure any vars used in the key are not considered to be used in the method.
 * @param {Object} fnPath - Babel path object
 * @param {Object} state - Babel state object for file
 * @returns {undefined}
 */
function methodVisitor(path, state) {
	if (path !== state[METHOD_ENTRY_PATH]) return;

	const fnPath = path.parentPath;
	state[METHOD_ENTRY_PATH] = fnPath[METHOD_ENTRY_PATH] || null;
...
function functionExitVisitor(fnPath, state) {
	// Insert tracking info comment:
	// - Functions: At start of function body
	...
	let commentHolderPath,
		isMethod = false,
		isProtoMethod = false,
		leadingComment = false;
	if (fnPath.node.kind === 'constructor') { // Class constructor
		const classBodyPath = fnPath.parentPath,
			classPath = classBodyPath.parentPath;
		if (classPath.node.superClass) {
			commentHolderPath = classPath.get('superClass');
			leadingComment = true;
		} else {
			commentHolderPath = classBodyPath;
		}
		...
		if (fnNode.computed) {
			commentHolderPath = fnPath.get('key');
			leadingComment = true;
		} else {
			commentHolderPath = bodyPath;
		}
	} else {
		commentHolderPath = bodyPath;
	}
	...
	insertTrackerComment(
		commentHolderPath, props.id, scopes, isMethod, isProtoMethod,
		superVarName, superIsTemp, leadingComment, state
	);
...
function locateBindingBlockPath(bindingPath, name, state) {
	...
	if (bindingPath.isFunctionDeclaration() || bindingPath.isClassDeclaration()) {
		// Function declaration - scoped to enclosing block.
	...
	if (bindingPath.isFunctionExpression() || bindingPath.isClassExpression()) {
