...
function superVisitor(superPath, state) {
	...
	let encloserPath, superVarNode, superIsProto;
	if (methodPath) {
		...
		const encloserNode = encloserPath.node,
		...
		superVarNode = superVar.node;
		superIsProto = isClass && !methodPath.node.static;
	} else {
		const parentVar = state.parentVars.super;
		superVarNode = t.identifier(parentVar.superVarName);
		superIsProto = parentVar.superIsProto;
		encloserPath = createParentVarBindingBlockPath(parentVar);
	}
...
function processEval(evalPath, state) {
	...
	const scopes = new Map(),
		fnProps = {scopes, superVarNode: undefined, superIsProto: false, argNames: undefined};
	...
	// Capture `super`
	const {parentFullFunctionPath} = state;
	if (parentFullFunctionPath ? parentFullFunctionPath.isMethod() : state.parentVars.super) {
		superVisitor(idPath, state);
	}
	...
	firstArgPath.replaceWith(t.callExpression(
		...
		[
			...
			fnProps.superVarNode
				? t.stringLiteral(fnProps.superVarNode.name)
				: t.unaryExpression('void', t.numericLiteral(0)),
			t.booleanLiteral(fnProps.superIsProto),
			...(
				fnProps.argNames
