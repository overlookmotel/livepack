...
// Modules
const {ensureStatementsHoisted} = require('@babel/helper-module-transforms'),
	assert = require('simple-invariant'),
	t = require('@babel/types');

// Imports
const {
		recordVarUse, insertTrackerComment, enterBlock, createBlockId, initScope,
		createTempVarNode, tagTempVarWithType, internalIdentifier, assertWithLocation
	} = require('./utils.js'),
	{
		INIT_PATH, INTERNAL_VAR_NAMES_PREFIX, TRACKER_VAR_NAME,
		TOP_BLOCK_ID, COMMON_JS_VARS
	} = require('./constants.js'),
	{
		INTERNAL_VAR_PREFIX_NUM, INTERNAL_VAR_NODES,
		TRACKER_VAR_NODE, SCOPE_ID_VAR_NODE, TEMP_VAR_NODES,
		BLOCK_ID,
		FUNCTION_PROPS, HAS_CONSTRUCTOR,
		PROGRAM_PATH, PARENT_FUNCTION_PATH, PARENT_FULL_FUNCTION_PATH,
		SUPER_VAR_NODE, TOP_LEVEL_VAR_NAMES, IS_INTERNAL
	} = require('./symbols.js'),
	{identifierIsVariable, replaceWith, isInternalPath, TRANSFORMED_COMMENT} = require('../shared.js');
...
module.exports = function(programPath, state) {
	...
	programExitVisitor(programPath, state);
};
...
function programEnterVisitor(path, state) {
	...
	const trackerNode = internalIdentifier(TRACKER_VAR_NAME);
	state[TRACKER_VAR_NODE] = trackerNode;
	state[INTERNAL_VAR_PREFIX_NUM] = 0;
	state[INTERNAL_VAR_NODES] = [trackerNode];
 ...
 * @returns {undefined}
 */
function programExitVisitor(path, state) {
	...
	// `const livepack_tracker = require('/path/to/app/node_modules/livepack/lib/init/index.js')(__filename, module, require);`
	...
	// Determine unique names for internal vars and set all vars to use this prefix
