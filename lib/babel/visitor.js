...
const {initFunctionAstSerialization, unpatchNodeToJson} = require('./ast.js'),
	{
		recordVarUse, getFunctionType, insertTrackerComment, createBlockId, getOrCreateScope,
...
function insertFnInfoFunctions(programPath, state, isEval) {
	...
	for (const fn of state.fns) {
		...
		let json = JSON.stringify({
			id,
			scopes: fn.scopes.map(({blockId, varNames, constNames, constInStrictNames, blockName}) => ({
				blockId,
				varNames: [...varNames],
				constNames: constNames.size > 0 ? [...constNames] : undefined,
				constInStrictNames: constInStrictNames.size > 0 ? [...constInStrictNames] : undefined,
				blockName
			})),
...
function classExitVisitor(classPath, state) {
	...
	// If class has constructor, inherit scopes and contains eval flag from constructor
	const {constructorFn} = fn;
	if (constructorFn) {
		fn.scopes = constructorFn.scopes;
		fn.containsEval = constructorFn.containsEval;
		return;
	}
	...
	const classNode = classPath.node,
		...
		blockId = fn.id,
		nameBlock = classPath[NAME_BLOCK];
	const scopes = hasSuperClass
		? [{
			blockId,
			scopeIdVarNode: getOrCreateScope(nameBlock, state),
			varNames: new Set(['super']),
			constNames: new Set(['super']),
			constInStrictNames: new Set(),
			blockName: nameBlock.name
		}]
		: [];
	fn.scopes = scopes;
	...
	insertTrackerComment(blockId, FN_TYPE_CLASS, undefined, classNode, 'inner', state);
}
...
function methodExitVisitor(fnPath, state) {
	...
	insertTrackerComment(fnId, functionType, callArguments, commentHolderNode, commentType, state);
}
...
function createTrackerNode(blockId, scopes, superVarNode, state) {
	...
	return t.callExpression(state.trackerVarNode, [
		createFnInfoVarNode(blockId, state),
...
function processEvalCall(callPath, state) {
	...
	let currentScope = callPath.scope;
	do {
		for (const varName of Object.keys(currentScope.bindings)) {
			includeVar(varName);
		}
