...
function classExitVisitor(classPath, state) {
	...
	if (classNode.superClass) {
		...
		addComments(
			...
			createTrackerComment(createBlockId(state), [], true, false, false, state)
		);
	}
}
...
function functionExitVisitor(fnPath, state) {
	...
	addComments(
		...
		createTrackerComment(
			props.id, scopes, props.isStrict, isMethod, props.superIsProto, state
		)
	);
...
function identifierVisitor(identifierPath, state) {
	...
	// Record variable use
	recordVarUse(name, functionPath, bindingBlockPath, isConst, undefined, false, state);
}
...
function superVisitor(superPath, state) {
	...
	// Record var use
	const superIsProto = isClass && !methodPath.node.static;
	recordVarUse('super', fnPath, encloserPath, true, superVar.node, superIsProto, state);
}
...
function processThisOrArguments(name, state) {
		...
		if (functionPath) {
			...
			if (parentVar) {
				const scopes = recordVarUse(
					name, functionPath, createParentVarBindingBlockPath(parentVar),
					isConst, undefined, false, state
				);
	...
	// Record variable use
	const scopes = recordVarUse(name, functionPath, bindingBlockPath, isConst, undefined, false, state);
