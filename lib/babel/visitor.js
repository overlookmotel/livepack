...
function classExitVisitor(classPath, state) {
	...
	if (classNode.superClass) {
		...
		insertTrackerComment(
			classNode.body, createBlockId(state), [], false, false, undefined, false, true, state
		);
	}
}
...
function functionExitVisitor(fnPath, state) {
	...
	if (fnNode.kind === 'constructor') { // Class constructor
		...
		if (!commentHolderNode) {
			...
			if (paramNodes.length > 0) {
				...
				if (fnPath.isArrowFunctionExpression() && paramNodes.length === 1) {
					fnNode.typeParameters = t.noop();
				}

				commentHolderNode = paramNodes[0];
			} else {
				commentHolderNode = fnNode.body;
			}
		}
	}
	...
	insertTrackerComment(
		commentHolderNode, props.id, scopes, isMethod, isProtoMethod,
		superVarName, superIsTemp, innerComment, state
	);
