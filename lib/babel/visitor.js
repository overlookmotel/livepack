...
function functionExitVisitor(fnPath, state) {
	...
	// - Functions with params: Before first param
	// - Functions with no params: At start of function body
	...
	if (fnNode.kind === 'constructor') { // Class constructor
		...
		if (fnPath.isMethod()) {
			...
			if (fnNode.computed) commentHolderPath = fnPath.get('key');
		}

		if (!commentHolderPath) {
			if (fnNode.params.length > 0) {
				// Force parentheses around parameters in arrow functions
				// so tracker comment is within function definition, not before it
				if (fnPath.isArrowFunctionExpression() && fnNode.params.length === 1) {
					fnNode.typeParameters = t.noop();
				}

				commentHolderPath = fnPath.get('params.0');
			} else {
