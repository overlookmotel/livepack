...
function blockStatementEnterVisitor(blockPath, state) {
	// If is body of function or `for` / `while` statement, or `catch` clause, use block already created
...
function blockStatementExitVisitor(blockPath, state) {
	// If is function's body block, move params into block
	let blockNode = blockPath.node;
	const parentNode = blockPath.parentPath.node;
	if (t.isFunction(parentNode)) {
		moveFunctionParams(state.currentFunction, parentNode, state);
		blockNode = parentNode.body;
	}

	insertBlockVars(blockNode, state);
	exitBlock(state);
}
...
function createBlockProps(name, canHoldVars, state) {
	...
	block.varsBlock = canHoldVars ? block : parentBlock.varsBlock;
	return block;
}
