	...
	varMappings.forEach(([varName, externalName, isConst]) => {
		...
		if (varName === 'this') {
			...
			withObj[updateArgumentsVarName] = (argumentsLocal) => {
				...
				// Delete props of arguments object inside `eval()` not present on external arguments object
				getOwnPropertyNamesAndSymbols(argumentsLocal).forEach((key) => {
					if (!getOwnPropertyDescriptor(argumentsValue, key)) delete argumentsLocal[key];
				});

				// Add getters/setters for all props on arguments object inside `eval()`
				// TODO Does not handle `delete arguments[0]` inside `eval()`
				// TODO Does not handle addition of new properties `arguments.x = 1`
				// inside or outside of `eval()`
