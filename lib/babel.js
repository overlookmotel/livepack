...
module.exports = function livepackBabelPlugin(api, options) {
	...
	return {
		visitor: {
			Program: {
				exit: (path, state) => masterVisitor(path, state, trackerPath, initPath)
			}
		}
	};
};
 ...
 * @returns {undefined}
 */
function masterVisitor(programPath, state, trackerPath, initPath) {
	...
	programExitVisitor(programPath, state, trackerPath, initPath);
}
...
function programExitVisitor(path, state, trackerPath, initPath) {
	...
	// `require('/path/to/app/node_modules/livepack/lib/init.js')(__filename, module, require);`
