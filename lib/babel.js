 ...
 *
 * TODO Need to capture values in higher scopes too, in case do not have reference to the enclosing
 * function to call to get its values.
 *
...
function functionVisitor(fnPath, state) {
	 ...
	 *
	 * `const scope_123 = tracker( {a, b}, 123, scope_122 );`
	 *
				...
				t.variableDeclarator(
					...
					t.callExpression(
						...
						[
							...
							t.numericLiteral(id),
							...(parentFnPath ? [t.identifier(parentFnPath.node[SCOPE_VAR_NAME])] : [])
						]
					)
				)
			]
