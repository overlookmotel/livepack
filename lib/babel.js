...
function programVisitor(path, state, trackerPath) {
	...
	// TODO Could avoid these extra traversals by tracking all identifiers used during
	// main traversal and then determining names at end, and injecting them in at that point.
...
function functionEnterVisitor(fnPath, state) {
	...
	fnNode[FUNCTION_PROPS] = {id: blockId, scopes: new Map()};

	// Update parent functions
	fnNode[PARENT_FUNCTION_PATH] = state[PARENT_FUNCTION_PATH];
	fnNode[PARENT_FULL_FUNCTION_PATH] = state[PARENT_FULL_FUNCTION_PATH];
...
function functionExitVisitor(fnPath, state) {
	...
	const fnNode = fnPath.node;
	const props = fnNode[FUNCTION_PROPS];
	...
	state[PARENT_FUNCTION_PATH] = fnNode[PARENT_FUNCTION_PATH];
	state[PARENT_FULL_FUNCTION_PATH] = fnNode[PARENT_FULL_FUNCTION_PATH];
}
...
function identifierVisitor(identifierPath, state) {
	...
	// TODO Could these checks be removed if code is injected in exit rather than entry visitors?
...
function addScopeNode(blockPath, blockId, state) {
	...
	if (blockPath.isProgram()) {
		...
		// TODO Add tracker import statement in program exit not entry to avoid having to behave
		// differently for program
