...
function recordVarUse(name, functionPath, bindingBlockPath, state) {
	...
	let {[BLOCK_ID]: blockId, [SCOPE_ID_VAR_NAME]: bindingScopeIdVarName} = bindingBlockPath.node;
	if (!bindingScopeIdVarName) {
		({blockId, scopeIdVarName: bindingScopeIdVarName} = addScopeNode(bindingBlockPath, state));
	}

	// Insert reference to var into function's `tracker()` call
	// Array of scopeVars is ordered by block ID
	...
	let index = scopesArrayNodes.findIndex((elementNode) => {
		const thisBlockId = elementNode.elements[2].value;
		if (thisBlockId === blockId) return true;
	...
	if (!scopeVarsObjectPath.node.properties.find(propNode => propNode.key.name === name)) {
		// `IS_INTERNAL` symbol is to stop this being visited as an identifier
		const valueNode = t.identifier(name);
		valueNode[IS_INTERNAL] = true;
 ...
 * @param {Object} state - Babel state object for file
 * @returns {Object}
 * @returns {number} .blockId - Block ID e.g. 100
