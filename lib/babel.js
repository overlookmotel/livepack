...
function identifierVisitor(identifierPath, state) {
	...
	if (bindingPath === functionPath || bindingPath.findParent(p => p === functionPath)) return;
	...
	if (bindingPath.isFunctionDeclaration()) {
		...
		const bindingParentPath = bindingPath.findParent(p => p.isVariableDeclaration() || p.isFunction());
		if (bindingParentPath.isFunction()) {
			// Function parameter - scoped to function body
			assertWithLocation(
				bindingPath.isIdentifier() || bindingPath.isObjectPattern() || bindingPath.isAssignmentPattern(),
				bindingPath, state,
			...
			const bindingFnPath = bindingParentPath.findParent(p => p.isFunction() || p.isProgram());
...
function processArguments(state) {
	...
	if (scope.argNames !== undefined) return;
	...
	// Set to `null` if not all params are simple vars, to avoid this check being run again.
