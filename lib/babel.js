...
const {
	...
	TRACKER_COMMENT_PREFIX, TEMP_COMMENT_PREFIX, COMMON_JS_VAR_NAMES
} = require('./shared.js');
...
function classExitVisitor(classPath, state) {
	...
	if (classPath.node.superClass) {
		...
		insertTrackerComment(
			classPath.get('body'), createBlockId(state), [], false, false, false, undefined, false, state
		);
	}
}
...
function functionExitVisitor(fnPath, state) {
	...
	insertTrackerComment(
		commentBodyPath, props.id, scopes, isMethod, isProtoMethod, isStaticMethod,
...
function identifierVisitor(identifierPath, state) {
	...
	if (!binding) {
		if (name === 'arguments') {
			...
			// TODO If a var named `arguments` higher up tree, usage inside a function might be wrongly
			// identified as a reference to that var.
			// e.g. `let arguments = 123; function f() { return () => arguments; }`
			// - `arguments` in arrow func refers to `f()`'s arguments but will be wrongly identified as
			// reference to var containing `123`.
...
function insertTrackerComment(
	bodyPath, id, scopes, isMethod, isProtoMethod, isStaticMethod, superVarName, superIsTemp, state
) {
	// eslint-disable-next-line max-len
	...
	const propsStr = JSON.stringify({
		...
		...(isStaticMethod ? {isStaticMethod: true} : null),
