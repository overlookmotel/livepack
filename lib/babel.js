...
function functionVisitor(fnPath, state) {
	...
	// TODO Where var is assigned to only, the original value doesn't need to be provided
	// to scope. Need create a var for it, but it needs not have a value.
	const vars = new Set();
	const shouldRecordArguments = fnPath.isArrowFunctionExpression()
		&& !parentScopePath.isArrowFunctionExpression();
	let argNames;
	fnPath.traverse({
		Identifier(identifierPath) {
			// Skip function's own name
			if (identifierPath.parentPath === fnPath && identifierPath.key === 'id') return;

			// Skip identifiers not used as vars e.g. `{a: 1}`
			if (!identifierIsVariable(identifierPath)) return;

			// Skip if not bound within enclosing function (or file if this is top level function)
			const {name} = identifierPath.node;
			if (name === 'arguments') {
				// `arguments` used - Skip if not an arrow function or parent function is not a "real" function
				// (i.e. source of the arguments)
				if (!shouldRecordArguments) return;

				// Get argument names
				if (!argNames) {
					argNames = parentScopePath.node.params.map(
						// `argNode.left.name` is to handle case where argument has default - `function(a = 1) {}`
						(paramNode) => {
	 ...
	 * TODO Check works with class methods + object methods
	 */
							...
							t.objectExpression(
								[...vars].map(
									name => t.objectProperty(
										t.identifier(name), t.identifier(name), false, name !== 'this'
									)
								)
							)
						)
			...
			t.returnStatement(
				t.callExpression(
					...
					[
						t.numericLiteral(id),
						t.identifier(scopeVarName),
						...(argNames ? [t.arrayExpression(argNames.map(name => t.stringLiteral(name)))] : [])
					]
				)
			)
		)
	...
	if (bodyPath.isBlockStatement()) {
		// Function has `{ ... }` body
		bodyPath.unshiftContainer('body', trackerNodes);
	} else {
...
function identifierIsVariable(path) {
	...
	if (parentPath.isMemberExpression({computed: false}) && path.key === 'property') return false;
	if (parentPath.isObjectProperty({computed: false}) && path.key === 'key') return false;
	if (parentPath.isClassMethod({computed: false}) && path.key === 'key') return false;
	return true;
}
