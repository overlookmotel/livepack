...
// Imports
const livepack = require('./index.js'),
	pkgName = require('../package.json').name;

// Constants
const LIVEPACK_VAR_NAME = Symbol(`${pkgName}.LIVEPACK_VAR_NAME`);
...
// Insert `const livepack = require('livepack');` statement at top of file
		...
		t.variableDeclaration(
			'const', [
				t.variableDeclarator(
					...
					t.callExpression(t.identifier('require'), [t.stringLiteral(pkgName)])
				)
			]
		)
	);

	state[LIVEPACK_VAR_NAME] = varName;
}
...
function getLivepackVarName(path) {
	...
	path.traverse({
		Identifier(identifierPath) {
			const match = identifierPath.node.name.match(new RegExp(`^${pkgName}([1-9]\\d*)?$`));
...
function functionVisitor(path, state) {
	...
	path.get('body.body.0').insertBefore(
		t.ifStatement(
			t.memberExpression(
				t.identifier(state[LIVEPACK_VAR_NAME]),
				t.identifier('_on')
			),
	...
	livepack._fns[key] = {async: node.async, generator: node.generator};
}
