...
// Imports
const {functionAsts} = require('./tracker.js'),
	{identifierIsVariable} = require('./shared/functions.js'),
	{IS_INTERNAL} = require('./shared/symbols.js');
...
function programVisitor(path, state, trackerPath) {
	...
	const importNode = t.variableDeclaration(
		'const', [
...
function functionVisitor(fnPath, state) {
	...
	const {blockId, scopeIdVarName} = addScopeNode(bodyPath, state);

	// Insert reveal code
	const revealNode = t.ifStatement(
		t.binaryExpression(
			'===',
	...
	revealNode[IS_INTERNAL] = true;
	bodyPath.get('body.0').insertAfter(revealNode);

	// Store AST in `functions` array
	functionAsts[blockId] = fnPath.node;
}
...
function recordVarUse(name, functionPath, bindingBlockPath, state) {
	...
	if (!bindingScopeIdVarName) {
		bindingScopeIdVarName = addScopeNode(bindingBlockPath, state).scopeIdVarName;
	}
	...
	let index = scopesArrayNodes.findIndex(
		elementNode => elementNode.elements[0].name === bindingScopeIdVarName
	);

	if (index === -1) {
		scopesArrayPath.pushContainer('elements', t.arrayExpression([
 ...
 * @param {Object} state - Babel state object for file
 * @returns {Object}
 * @returns {number} .blockId - Block ID
 * @returns {string} .scopeIdVarName - `scopeId` var name e.g. 'scopeId_100'
 */
function addScopeNode(blockPath, state) {
	...
	blockPath.node[SCOPE_ID_VAR_NAME] = scopeIdVarName;
