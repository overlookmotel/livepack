...
const TRACKER_PATH = 'livepack/tracker',
	...
	BLOCK_IDS = Symbol('livepack.BLOCK_IDS'),
...
module.exports = function livepackBabelPlugin(api, options) {
	...
	return {
		visitor: {
			Program: {
				enter(path, state) {
					programEnterVisitor(path, state, trackerPath);
				},
 ...
 * @returns {undefined}
 */
function programEnterVisitor(path, state, trackerPath) {
...
/**
 * Visitor to pop block ID of list of block IDs when exiting block.
 * @param {Object} blockPath - Babel path object for statement block
 * @param {Object} state - Babel state object for file
 * @returns {undefined}
 */
...
function functionEnterVisitor(fnPath, state) {
	...
	const parentBlockIds = [...state[BLOCK_IDS]];
	...
	fnPath.node[FUNCTION_PROPS] = {id: blockId, parentBlockIds, scopes: new Map()};
}
...
function functionExitVisitor(fnPath, state) {
	...
	// `/*livepack_track:{"id":3,"blockIds":[1,2],"scopes":[{"blockId":1,"varNames":["a"]}, {"blockId":2,"varNames":["b", "arguments"],argNames:["b"]}],"isMethod":true}*/`
...
function identifierVisitor(identifierPath, state) {
	...
	if (!bindingPath.isFunctionDeclaration()) {
		...
		assertWithLocation(
			bindingPath.isVariableDeclaration(), bindingPath, state,
