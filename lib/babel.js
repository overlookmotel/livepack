...
function functionExitVisitor(fnPath, state) {
	...
	bodyPath.get('body.0').insertAfter(
		t.ifStatement(
			t.binaryExpression(
				'===',
				t.identifier(bodyPath[SCOPE_ID_VAR_NAME]),
				t.nullLiteral()
			),
			t.returnStatement(
				t.callExpression(
...
function recordVarUse(name, functionPath, bindingBlockPath, state) {
	...
	let scopeIdVarName = bindingBlockPath[SCOPE_ID_VAR_NAME];
...
function addScopeNode(blockPath, blockId, state) {
	// Get var name for scopeId + record on path
	const scopeIdVarName = `${state[SCOPE_ID_BASE_VAR_NAME]}${blockId}`;
	blockPath[SCOPE_ID_VAR_NAME] = scopeIdVarName;
