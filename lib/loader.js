...
// Determine loader API version
const isLoaderApiV2 = (() => {
	const {major, minor} = parseNodeVersion(process.version);
	return major > 16 || (major === 16 && minor >= 12);
})();
...
const plugin = require('./babel/index.js'),
	...
	{TRANSFORMED_COMMENT, LOADER_RESOLVE_PREFIX} = require('./shared/constants.js'),
...
const COMMONJS_EXTS = ['.js', '.cjs'],
	...
	LOADER_RESOLVE_PREFIX_LEN = LOADER_RESOLVE_PREFIX.length,
 ...
 * @returns {Object} - Loader hooks object
 */
function createLoader(options) {
	...
	// Return ESM loader.
	// Loader also provides resolution of specifiers to URLs, using URLs with prefix `livepackresolve://`.
	// This avoid having to use `import.meta.resolve()` (which is not supported on Node v12)
	// and also avoids need for top-level await, which `import.meta.resolve()` necessitates as it's async.
	...
	const isResolveUrl = url => url.startsWith(LOADER_RESOLVE_PREFIX);
	const urlIsJsx = options.jsx ? url => /\.jsx$/.test(fileURLToPath(url)) : undefined;
	const resolve = async (specifier, context, defaultResolve) => {
		// Resolve `livepackresolve://<specifer>` to `livepackresolve://<resolved URL>`.
		// e.g. `livepackresolve://lodash`
		// -> `livepackresolve://file:///path/to/app/node_modules/lodash/lodash.js`.
		// `load()` hook will output a module which exports the resolved URL.
		if (isResolveUrl(specifier)) {
			const {url: resolvedUrl} = await defaultResolve(
				specifier.slice(LOADER_RESOLVE_PREFIX_LEN), context, defaultResolve
			);
			return {format: 'module', url: `${LOADER_RESOLVE_PREFIX}${resolvedUrl}`};
		}

		// Treat JSX files as ESM
		if (urlIsJsx && urlIsJsx(new URL(specifier, 'file:///'))) {
			const parentURL = context.parentURL || pathToFileURL(`${process.cwd}/`).href;
			return {format: 'module', url: new URL(specifier, parentURL).href};
		}

		return defaultResolve(specifier, context, defaultResolve);
	};
	const createResolveModuleSource = url => (
		`export default ${JSON.stringify(url.slice(LOADER_RESOLVE_PREFIX_LEN))};`
	);
	const transformEsmWithUrl = (source, url) => transformEsm(source, fileURLToPath(url));

	if (isLoaderApiV2) {
		// Loader API for Node >= 16.12.0
		return {
			resolve,
			async load(url, context, defaultLoad) {
				// Output `livepackresolve://` URL as module with resolved path as default export
				if (isResolveUrl(url)) return {format: 'module', source: createResolveModuleSource(url)};

				// Transform ESM code (CommonJS is left for CommonJS loader)
				const ret = await defaultLoad(url, context, defaultLoad);
				if (ret.format === 'module') ret.source = transformEsmWithUrl(ret.source, url);
				return ret;
			}
		};
	}
	...
	return {
		...
		getFormat(url, context, defaultGetFormat) {
			// Treat `livepackresolve://` and JSX as ESM
			if (isResolveUrl(url) || (urlIsJsx && urlIsJsx(url))) return {format: 'module'};
			return defaultGetFormat(url, context, defaultGetFormat);
		},
		getSource(url, context, defaultGetSource) {
			// Output `livepackresolve://` URL as module with resolved path as default export
			if (isResolveUrl(url)) return {source: createResolveModuleSource(url)};
			return defaultGetSource(url, context, defaultGetSource);
		},
		transformSource(source, context, defaultTransformSource) {
			// Transform ESM code (CommonJS is left for CommonJS loader).
			// `livepackresolve://` URLs also skip transform.
			if (context.format === 'module') {
				const {url} = context;
				if (!isResolveUrl(url)) return {source: transformEsmWithUrl(source, url)};
			}
