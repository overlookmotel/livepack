...
function createLoader(options) {
	...
	// Return ESM loader
	const {jsx} = options;
	let resolveJsxToUrl, urlIsJsx;
	if (jsx) {
		urlIsJsx = url => /\.jsx$/.test(fileURLToPath(url));
		resolveJsxToUrl = (specifier, context) => {
			if (!urlIsJsx(new URL(specifier, 'file:///'))) return undefined;
		...
		return {
			resolve: jsx
				? (specifier, context, defaultResolve) => {
					const url = resolveJsxToUrl(specifier, context);
					return url ? {format: 'module', url} : defaultResolve(specifier, context, defaultResolve);
				}
				: undefined,
			async load(url, context, defaultLoad) {
				const ret = await defaultLoad(url, context, defaultLoad);
				if (ret.format === 'module') ret.source = transformEsm(ret.source, fileURLToPath(url));
				return ret;
			}
		};
	}
	...
	return {
		resolve: jsx
			? (specifier, context, defaultResolve) => {
				const url = resolveJsxToUrl(specifier, context);
				return url ? {url} : defaultResolve(specifier, context, defaultResolve);
			}
			: undefined,
		getFormat: jsx
			? (url, context, defaultGetFormat) => (
				urlIsJsx(url) ? {format: 'module'} : defaultGetFormat(url, context, defaultGetFormat)
			)
			: undefined,
		transformSource: async (source, context, defaultTransformSource) => (
			context.format === 'module'
				? {source: transformEsm(source, fileURLToPath(context.url))}
