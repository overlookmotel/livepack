...
function insertBlockVarsIntoBlockStatement(block, blockNode, state) {
	const {scopeIdVarNode, tempVarNodes} = block;
	if (!scopeIdVarNode && !tempVarNodes) return;

	const statementNodes = [];

	// `const livepack_scopeId_1 = livepack_getScopeId()`
	if (scopeIdVarNode) {
		statementNodes.push(t.variableDeclaration(
...
function insertTrackerCodeIntoFunctionParams(
	...
	const {scopeIdVarNode, tempVarNodes} = paramsBlock;
	if (scopeIdVarNode) addAssignment(scopeIdVarNode, t.callExpression(state.getScopeIdVarNode, []));
