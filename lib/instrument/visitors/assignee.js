...
const Expression = require('./expression.js'),
	...
	{visitEvalIdentifier} = require('./eval.js'),
...
function createAssigneeVisitor(Identifier, isDeclaration) {
	const Assignee = isDeclaration
		? function DeclarationAssignee(node, state, parent, key) {
			switch (node.type) {
				case 'Identifier': return Identifier(node, state, parent, key);
		...
		: function AnyAssignee(node, state, parent, key) {
			switch (node.type) {
				case 'Identifier': return Identifier(node, state, parent, key);
				case 'AssignmentPattern': return AssignmentPattern(node, state);
				case 'ArrayPattern': return ArrayPattern(node, state);
 ...
 * @returns {undefined}
 */
function IdentifierConst(node, state, parent, key) {
	visitConstOrLetIdentifier(node, true, parent, key, state);
}
 ...
 * @returns {undefined}
 */
function IdentifierLet(node, state, parent, key) {
	visitConstOrLetIdentifier(node, false, parent, key, state);
}
 ...
 * @returns {undefined}
 */
function visitConstOrLetIdentifier(node, isConst, parent, key, state) {
 ...
 * @returns {undefined}
 */
function IdentifierVar(node, state, parent, key) {
	// Create binding.
	...
	if (!block) {
		// No hoist block. Must be top level of indirect `eval` code. Creates global var, not local var.
...
function assertNoCommonJsVarsClash(block, varName, state) {
	assert(
		!state.isCommonJs || block !== state.programBlock
		|| varName === 'arguments' || !state.fileBlock.bindings[varName],
