...
function visitObjectMembers(objectNode, memberNodes, parent, key, state) {
	...
	visitContainer(
		memberNodes,
		(memberNode, index) => visitObjectMember(memberNode, memberNodes, index, methodIndexes, state),
		state
	);
 ...
 * @returns {undefined}
 */
function visitObjectMember(memberNode, memberNodes, index, methodIndexes, state) {
	const {type} = memberNode;
	if (type === 'ObjectProperty') {
		ObjectProperty(memberNode, state, memberNodes, index);
	} else if (type === 'SpreadElement') {
		SpreadElement(memberNode, state, memberNodes, index);
	} else {
