...
function visitEval(node, parent, key, state) {
	...
	state.secondPass(
		...
		fn, state.isStrict, state.currentSuperIsProto, [...state.trail], state
	);
}
...
function instrumentEval(
	node, isEvalCall, parent, key, block, superBlock, thisBlock, fn, isStrict, superIsProto, trail, state
) {
	...
	if (isEvalCall) {
		instrumentEvalCall(parent, block, superBlock, thisBlock, fn, isStrict, superIsProto, trail, state);
	} else {
...
function instrumentEvalCall(
	callNode, block, superBlock, thisBlock, fn, isStrict, superIsProto, trail, state
) {
	...
	do {
		...
		for (const [varName, binding] of Object.entries(block.bindings)) {
			...
			if (
				...
				&& varName !== 'new.target'
				&& ((varName !== 'require' && varName !== 'arguments') || block !== state.fileBlock)
			) {
		...
		// If some vars from block used, add to scopes definitions. Always include file block.
		if (varDefsNodes.length !== 0 || block === state.fileBlock) {
