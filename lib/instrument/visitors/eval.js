...
const {
	...
	{
		createTempVarNode, addToInternalVars, EVAL_INDIRECT_VAR_NAME, EVAL_DIRECT_VAR_NAME
...
function visitEval(node, parent, key, state) {
	...
	const isEvalCall = key === 'callee' && parent.type === 'CallExpression',
...
function instrumentEval(node, isEvalCall, parent, key, block, fn, isStrict, canUseSuper, state) {
	...
	if (isEvalCall) {
		...
		instrumentEvalIdentifier(node, state);
	}
}

/**
 * Instrument `eval()` call.
 * Convert `eval()` to `livepack_evalDirect()`.
 * Capture all vars which can be accessed by `eval()` and pass details to `livepack_evalDirect()`.
 * `livepack_evalDirect()` will reconstruct the scopes and instrument the `eval()`-ed code.
...
function instrumentEvalCall(callNode, block, fn, isStrict, canUseSuper, state) {
	...
	//   true // `isStrict` flag
	// )`
	const evalNode = callNode.callee;
	evalNode.name = EVAL_DIRECT_VAR_NAME;
	addToInternalVars(evalNode, state);
 ...
 * Convert `eval` to `livepack_evalIndirect`.
 * @param {Object} node - `eval` identifier AST node
 * @param {Object} state - State object
 * @returns {undefined}
 */
function instrumentEvalIdentifier(node, state) {
	node.name = EVAL_INDIRECT_VAR_NAME;
...
function activateSuperIfIsUsable(fn, state) {
	...
	// TODO Also need to pass `superIsProto` and `superVarNode` to `livepack_evalDirect()`
