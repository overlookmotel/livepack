...
function visitEval(node, parent, key, state) {
	...
	// If `eval()` call with no arguments, it doesn't do anything, so leave it as is
	if (isEvalCall && parent.arguments.length === 0) return;
	...
	if (isEvalCall) {
		...
		const canUseSuper = activateSuperIfIsUsable(fn, state);
