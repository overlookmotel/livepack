...
const Statement = require('./statement.js'),
	...
	{
		createBlock, createAndEnterBlock, createBindingWithoutNameCheck, insertBlockVars
	} = require('../blocks.js'),
...
function ForInitializer(node, state, parent) {
	if (node.type === 'VariableDeclaration') {
		VariableDeclaration(node, state, parent, 'init');
	} else {
...
function ForXInitializer(node, state, parent) {
	if (node.type === 'VariableDeclaration') {
		VariableDeclaration(node, state, parent, 'left');
	} else {
...
function instrumentLoopBody(bodyNode, statementNode, bodyBlock, hasBodyBlock, state) {
	...
	insertBlockVars(bodyBlock, bodyNode, state);
}
