...
function instrumentForXStatement(statementNode, initBlock, state) {
	...
	// TODO Handle `for (var x of ...)` / `for (var x in ...)`
	if (statementNode.left.type === 'VariableDeclaration' && statementNode.left.kind === 'var') return;
	...
	if (leftNode.type === 'VariableDeclaration') {
		...
		// `for ({x} of ...)`
		// -> `for (let [livepack_scopeId, livepack_temp1, livepack_temp2 = {x} = livepack_temp1] of ...)`
		...
		statementNode.left = t.variableDeclaration('let', [t.variableDeclarator(
			t.arrayPattern([
				...
				tempVarNode,
				t.assignmentPattern(
					createTempVarNode(state),
					t.assignmentExpression('=', leftNode, tempVarNode)
				)
			])
		)]);
	}
