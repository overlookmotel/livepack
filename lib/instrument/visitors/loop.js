...
function ForXStatement(node, state) {
	...
	state.secondPass(instrumentForXStatement, node, initBlock, state);
}
 ...
 *
 * `for (const x of y) ;` -> `for (const [livepack_scopeId, x] of livepack_getScopeId.wrapIterator(y)) ;`
 * `for (let x of y) ;` -> `for (let [livepack_scopeId, x] of livepack_getScopeId.wrapIterator(y)) ;`
 * `for (var {x, y} of z) ;` -> `{
 *   var x, y;
 *   for (
 *     let [livepack_scopeId, livepack_temp1, livepack_temp2 = {x, y} = livepack_temp1]
 *     of livepack_getScopeId.wrapIterator(z)
 *   ) ;
 * }`
 * `for (x of y) ;` -> `for (
 *   let [livepack_scopeId, livepack_temp1, livepack_temp2 = x = livepack_temp1]
 *   of livepack_getScopeId.wrapIterator(y)
 * ) ;`
 *
 ...
 * @returns {undefined}
 */
function instrumentForXStatement(statementNode, initBlock, state) {
	// Exit if init block doesn't need scope ID var injected into it
	if (!initBlock.scopeIdVarNode) return;

	if (statementNode.type !== 'ForOfStatement') return; // TODO Handle `for ( ... in ... )`

	const leftNode = statementNode.left;
	if (leftNode.type !== 'VariableDeclaration') return; // TODO Handle `for (x of ...)`
	if (leftNode.kind === 'var') return; // TODO Handle `for (var x of ...)`

	// TODO Convert temp var nodes to member expressions if `const`
	// e.g. `livepack_temp1._0` instead of just `livepack_temp1`

	const declaratorNode = leftNode.declarations[0],
		tempVarNodes = initBlock.tempVarNodes || [];
	...
	statementNode.right = t.callExpression(
		t.memberExpression(state.getScopeIdVarNode, t.identifier('wrapIterable')),
