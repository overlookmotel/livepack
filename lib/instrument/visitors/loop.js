...
function ForXStatement(node, state, parent, key) {
	...
	// Visit initializer.
	// If `var` declaration in init clause, get var names declared.
	// If `const` declaration, make temp vars be objects.
	let numVarsBeforeInit;
	if (node.left.type === 'VariableDeclaration') {
		if (node.left.kind === 'var') {
			numVarsBeforeInit = Object.keys(state.currentHoistBlock.bindings).length;
		} else if (node.left.kind === 'const') {
			initBlock.tempVarsAsObjects = true;
		}
	}
...
function instrumentForXStatement(statementNode, initBlock, varDeclarationNames, parent, key, state) {
	...
	if (leftNode.type !== 'VariableDeclaration') {
		...
		// If `const` declaration, initialize temp vars with objects.
		// Temp vars need to be written to after definition, so initialized as `livepack_temp1 = {}`
		// and used as `livepack_temp1.value`.
		if (leftNode.kind === 'const' && tempVarNodes.length > 0) {
			tempVarNodes = tempVarNodes.map(
				tempVarNode => t.assignmentPattern(tempVarNode.object, t.objectExpression([]))
			);
		}
