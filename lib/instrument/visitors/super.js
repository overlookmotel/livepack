...
function Super(node, state, parent, key) {
	...
	const isSuperCall = key === 'callee' && parent.type === 'CallExpression',
		isNotInArrowFunction = fn.node.type !== 'ArrowFunctionExpression';
	...
	if (isSuperCall) {
		...
		const {trail} = state,
			...
			isTopLevelStatement = isNotInArrowFunction && trail.length === 8 && trail[3] === 'body';
		...
		thisVarType = isNotInArrowFunction ? INTERNAL : EXTERNAL;
	}
