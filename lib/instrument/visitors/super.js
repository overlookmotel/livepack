...
function Super(node, state, parent, key) {
	...
	let thisBlockIfNeedsExternalThisVar;
	if (isSuperCall) {
		// `super()` call.
		...
		if (isTopLevelStatement && trail[6] === 'expression') {
			...
			thisBlockIfNeedsExternalThisVar = state.currentThisBlock;
		} else {
	...
	state.secondPass(
		recordSuperWithAmendment, node, superBlock, thisBlockIfNeedsExternalThisVar, isSuperCall,
		fn, [...state.trail], state
	);
}

/**
 * Record use of `super`.
 *   - Record function's reliance on external var for `super` target
 *   - Record function's reliance on external var `this` (if `super` is in arrow function)
 *   - Record amendment for `super` - used in serializer to transpile `super`
 * Done in 2nd pass because:
 *   - First 2 activate blocks, which must happen in 2nd pass because this creates internal
 *     `livepack_scopeId` vars, and internal vars can only be created in 2nd pass once prefix num known
 *   - The amendment that's recorded needs to be in correct order relative to other amendments
 *
 * @param {Object} node - `super` AST node (not needed but passed for debug reasons)
 * @param {Object} superBlock - `super` block object
 * @param {Object} [thisBlock] - `this` block object if `super` necessitates use of external `this`
 ...
 * @returns {undefined}
 */
function recordSuperWithAmendment(node, superBlock, thisBlock, isSuperCall, fn, trail, state) {
	// Create external var for `super`
	recordSuper(superBlock, fn, state);

	// Create external var for `this` if required
	if (thisBlock) createExternalVarForThis(fn, thisBlock, state);
 ...
 * @param {Object} fn - Function object
 * @param {Object} thisBlock - `this` block object
 * @param {Object} state - State object
 * @returns {undefined}
 */
function createExternalVarForThis(fn, thisBlock, state) {
