...
module.exports = {
	...
	isNamingAssignmentOperator,
	freezeAssigneeNameIfIdentifier
};
...
function ArrowFunctionExpression(node, state, parent, key) {
	freezeAssigneeNameIfIdentifier(parent, state);
...
/**
 * Determine if an assignment operator names a function/class on right-hand side of assignment.
...
/**
 * If unnamed function or class is gaining its name implicitly from an assignment,
 * flag that binding with `isFunction` so the var doesn't get renamed,
 * to prevent it altering the function's name.
 * TODO This is wrong approach. Should not affect binding as that would affect all functions which
 * access this var, even if the function where the var name is frozen isn't included
 * in code being serialized.
 * TODO Why is `isFunction` included in external vars function info at all? It doesn't seem to be used.
 * Need another mechanism to freeze var names.
 * @param {Object} node - AST node which is parent of a function/class expression
 * @param {Object} state - State object
 * @returns {undefined}
 */
function freezeAssigneeNameIfIdentifier(node, state) {
	const idNode = getNamingAssignee(node);
	if (!idNode || idNode.type !== 'Identifier') return;

	const varName = idNode.name;
	let block = state.currentBlock;
	do {
		const binding = block.bindings[varName];
		if (binding) {
			binding.isFunction = true;
			break;
		}
...
function getNamingAssignee(node) {
	const {type} = node;
	if (type === 'VariableDeclarator') return node.id;
	if (type === 'AssignmentExpression') {
		return isNamingAssignmentOperator(node.operator) ? node.left : undefined;
	}
	if (type === 'AssignmentPattern') return node.left;
	return undefined;
}
