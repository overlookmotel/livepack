...
// Imports
const {IdentifierReadOnly, ThisExpression, NewTargetExpression} = require('./identifier.js'),
	...
	{ImportExpression} = require('./module.js'),
	{visitEvalCall} = require('./eval.js'),
	{
...
function Expression(node, state, parent, key) {
	...
	switch (node.type) {
		...
		case 'CallExpression': return CallExpression(node, state, parent, key);
		case 'OptionalCallExpression':
		case 'NewExpression': return BasicCallExpression(node, state);
		...
		case 'Import': return ImportExpression(node, state); // Keep as 2 params
		case 'MetaProperty': return MetaProperty(node, state);
...
function CallExpression(node, state, parent, key) {
	const isEvalCall = node.callee.type === 'Identifier' && node.callee.name === 'eval';
	BasicCallExpression(node, state);
	if (isEvalCall) visitEvalCall(node, parent, key, state);
}

function BasicCallExpression(node, state) {
...
function MetaProperty(node, state) {
	...
	if (name === 'import') {
		...
		NewTargetExpression(node, state);
	} else {
		throw new Error(`Unexpected meta property '${name}.${node.property.name}'`);
	}
}
