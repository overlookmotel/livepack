 ...
 * @param {string} [options.filename] - File path - must be provided if source map not provided
...
function parse(code, options) {
	...
	assert(
		!sourceMaps || filename || inputSourceMap,
		'options.filename or options.inputSourceMap must be provided when source maps enabled'
	);

	return parseImpl(code, filename, isEsm, isCommonJs, isJsx, isStrict, sourceMaps, inputSourceMap);
}
