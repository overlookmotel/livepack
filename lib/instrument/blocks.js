...
const {
	...
	{createLocalEvalVarAccessorNode} = require('./visitors/eval.js'),
 ...
 * @returns {undefined}
 */
function activateBinding(block, binding, varName, state) {
	if (!binding.varNode) {
		binding.varNode = varName === 'eval'
			? createLocalEvalVarAccessorNode(block, state)
