...
// Imports
const {
		createScopeIdVarNode, createTempVarNode, createLocalEvalVarNode, checkInternalVarNameClash
 ...
 * @returns {undefined}
 */
function activateBinding(binding, varName, state) {
	if (!binding.varNode) {
		binding.varNode = varName === 'eval' ? createLocalEvalVarNode(state) : t.identifier(varName);
	}
}
