 ...
 * i.e. everything in `lib/` except for `lib/runtime/*`.
 * `runtime` is excluded as it contains code which is used in output.
...
function isInternalPath(path) {
	return path.startsWith(LIVEPACK_DIR_PATH) // eslint-disable-line no-use-before-define
		&& !path.startsWith(LIVEPACK_RUNTIME_PATH); // eslint-disable-line no-use-before-define
}

const LIVEPACK_DIR_PATH = pathJoin(__dirname, '../'),
	LIVEPACK_RUNTIME_PATH = pathJoin(__dirname, '../runtime/');
