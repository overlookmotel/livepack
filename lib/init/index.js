...
const modifyBuiltins = {
	...
	module(Module) {
		// Patch `createRequire` + `createRequireFromPath` to wrap `require` functions
		...
		const createRequireFromPathOriginal = Module.createRequireFromPath;
		if (createRequireFromPathOriginal) {
			Module.createRequireFromPath = function deprecated(...[filename]) {
				const require = createRequireFromPathOriginal(filename);
				return createWrappedRequire(require, filename);
			};
		}
	}
