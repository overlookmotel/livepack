...
// Imports
const perf = require('../perf.js')('Globals'),
...
/**
 * Catalog globals and built-in modules.
 ...
 * It prioritizing access by key `x.y` over prototypes (`Object.getPrototypeOf(x)`)
 ...
 * @returns {undefined}
 */
module.exports = function populateGlobals(globals) {
	// Register `undefined` as a global
	globals.set(undefined, {type: SPECIAL, parent: null, key: 'undefined'});

	perf('Before catalog globals');

	catalogGlobals(globals);
	perf('Catalog globals');

	catalogBuiltInModules(globals);
	perf('Catalog built-in modules');

	catalogSpecials(globals);
	perf('Catalog specials');
};
...
/**
 * Catalog built-in modules e.g. `require('fs')`.
 * @param {Map} globals - Map of globals
 * @returns {undefined}
 */
function catalogBuiltInModules(globals) {
	const queue = new Map();
	for (const key of Module.builtinModules) {
		...
		addToQueue(val, MODULE, null, key, false, queue);
	}
...
function catalogSpecials(globals) {
	...
	for (const [val, name] of [
		...
		addToQueue(val, SPECIAL, null, name, false, queue);
	}
...
function catalogValue(val, props, nextLevelQueue, nextPriorityQueue, globals) {
	...
	if (val instanceof Module) return;
	...
	if (descriptor.configurable) {
		const shimmedGet = {
			get() {
				...
				if (this === parent && !globals.has(val)) {
					processQueue(new Map([[val, {type: VALUE, parent, key, noPreload: true}]]), globals);
				}
				return val;
			}
		}.get;
