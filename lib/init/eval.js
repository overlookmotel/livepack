...
const nextBlockIds = Object.create(null);

module.exports = function(filename, nextBlockId, prefixNum) {
	nextBlockIds[filename] = nextBlockId;
...
function evalIndirect(code, filename, evalIndirectLocal) {
	...
	const {code: fnCode, usesEval, internalPrefixNum, errored} = compile(
		code, filename, undefined, false, 0
	);
...
function evalDirect(args, filename, prefixNum, evalDirectLocal) {
	...
	const {code: codeInstrumented, errored: shouldThrow} = compile(
		code, filename, state, isStrict, prefixNum
	);
...
function compile(code, filename, state, isStrict, externalPrefixNum) {
	const {ast, internalPrefixNum, usesEval} = codeToAst(
		code, filename, state, isStrict, externalPrefixNum
	);
	...
	if (internalPrefixNum !== externalPrefixNum || !state) {
		...
		if (state) node = wrapInFunctionCall(node, internalPrefixNum, externalPrefixNum, usesEval);
...
function codeToAst(code, filename, state, isStrict, externalPrefixNum) {
	// Parse code.
	...
	state = {
		filename,
		nextBlockId: nextBlockIds[filename],
		isStrict,
	...
	nextBlockIds[filename] = state.nextBlockId;

	return {
		ast,
		internalPrefixNum: state.internalVarsPrefixNum,
		usesEval: state.fileContainsEval
	};
}
