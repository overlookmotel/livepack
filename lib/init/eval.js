...
function getEvalFunctions(filename, prefixNum) {
	...
	const prevalLocal = (code, scopes, isStrict, argNames) => preval(
		code, scopes, isStrict, argNames, filename, prefixNum
	);
...
function preval(code, scopes, isStrict, argNames, filename, prefixNum) {
	...
	for (const [blockId, scopeId, blockName, varNames, constNames, constInStrictNames] of scopes) {
		const constNamesSet = new Set(constNames),
			constInStrictNamesSet = new Set(constInStrictNames);
		for (const varName of varNames) {
			scopeVars[varName] = {
				...
				isConstInStrict: constInStrictNamesSet.has(varName),
