...
const visitor = require('../babel/visitor.js'),
	{
		BLOCK_ID, PARENT_VARS, INTERNAL_VAR_PREFIX_NUM, EVAL_IS_USED, IS_EVAL_CODE, IS_COMMON_JS, IS_STRICT
...
function getEvalFunctions(filename, prefixNum) {
	...
	const prevalLocal = (code, scopes, isStrict) => preval(code, scopes, !!isStrict, filename, prefixNum);
...
function preval(code, scopes, isStrict, filename, prefixNum) {
...
function codeToAst(code, filename, scopeVars, isStrict, externalPrefixNum) {
	...
	// If Babel cannot parse, swallow error. Expression will be passed to `eval()` which should throw -
	// this maintains native errors.
	const state = {
		file: {opts: {filename}},
