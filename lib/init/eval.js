...
function evalDirect(
	...
	const addInternalVarsToAst = (ast, instrumentationState) => {
		prefixNumIsChanged = directEvalModifyAst(
			ast, instrumentationState.internalVarsPrefixNum, outsidePrefixNum, externalEvalHasProxy,
...
function directEvalModifyAst(
	ast, insidePrefixNum, outsidePrefixNum, externalEvalHasProxy,
	externalEvalVarNodes, externalEvalVarNodeIsProxyBoolNode
) {
	if (insidePrefixNum === outsidePrefixNum) return false;
	...
	if (externalEvalVarNodes && externalEvalVarNodes.length > 1) {
		...
		const internalLocalEvalVarNode = localEvalVarNode(insidePrefixNum),
			externalLocalEvalVarNode = localEvalVarNode(outsidePrefixNum);
