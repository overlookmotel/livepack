...
function getEvalFunctions(filename, prefixNum) {
	...
	const prevalLocal = (code, scopes, isStrict, superVarName, superIsProto, argNames) => preval(
		code, scopes, isStrict, superVarName, superIsProto, argNames, filename, prefixNum
	);
...
function preval(code, scopes, isStrict, superVarName, superIsProto, argNames, filename, prefixNum) {
	...
	for (const [blockId, scopeId, blockName, varNames, constNames] of scopes) {
		...
		for (const varName of varNames) {
			scopeVars[varName] = {
				...
				superVarName: varName === 'super' ? superVarName : undefined,
				superIsProto: varName === 'super' ? superIsProto : false,
...
function codeToAst(code, filename, scopeVars, isStrict, externalPrefixNum) {
	...
	let ast;
	try {
		ast = parse(code, {strictMode: isStrict, allowSuperOutsideMethod: true});
	} catch (err) {
		return {};
	}
