...
const {argv} = yargs
	...
	.option('source-maps', {
		...
		type: 'string',
		coerce(val) {
			if (val === 'inline' || val === false) return val;
			if (val === '') return true;
...
(async () => {
	// Load source
	const srcPathOriginal = argv.input,
		srcPath = pathResolve(srcPathOriginal),
		outPath = pathResolve(argv.output);

	let input;
	try {
		input = require(srcPath); // eslint-disable-line global-require, import/no-dynamic-require
	} catch (err) {
		if (
			err && err.code === 'MODULE_NOT_FOUND'
				&& err.message.split('\n')[0] === `Cannot find module '${srcPath}'`
				&& err.requireStack
				&& err.requireStack.length === 1
				&& err.requireStack[0] === __filename
		) {
			throw new Error(`Cannot load input file '${srcPathOriginal}'`);
		}
		throw err;
	}
