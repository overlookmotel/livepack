...
const perf = require('./perf.js')('Register');
perf('Start');
...
function register(options) {
	...
	const piratesRevert = addHook((code, filename) => {
		...
		if (!usingInternalModuleCache()) {
			assertBug(code.startsWith(TRANSFORMED_COMMENT_STR), `Babel transform failed on ${filename}`);
