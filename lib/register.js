...
// Imports
const {isInternalPath, parseSourceMapFromCode} = require('./shared/functions.js'),
	{createBabelConfig, EXT_TYPES} = require('./babel/config.js'),
...
function register(options) {
	...
	let {configFile, babelrc, jsx, cache: useCache} = options; // eslint-disable-line prefer-const
	assert(
	...
	const patchLoaderRevert = patchLoader();

	// Run Babel register
	const extTypes = jsx ? {...EXT_TYPES, '.jsx': 'module'} : EXT_TYPES,
		exts = Object.keys(extTypes);
	const config = createBabelConfig({configFile, babelrc, jsx, extTypes});
