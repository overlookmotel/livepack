...
module.exports = {
	...
	processBlock(block, parentCreateScopes, inheritedVars, frozenNames, isRoot) {
		...
		// Any other reference to a function defined in this block or blocks above is made circular.
		// Also count which params are most used in calls to scope function, so params which are most often
		// undefined (or circular, and so undefined on initial call to scope function), go last.
			...
			if (!isScopeInternalOnly) {
				for (const [scope, fnRecord] of fnScopes) {
					...
					if (!fnParamNames) {
						...
						if (fnRecord.assignments) {
							(scopeRecord.assignments || (scopeRecord.assignments = [])).push(...fnRecord.assignments);
