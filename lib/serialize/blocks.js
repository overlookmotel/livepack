...
module.exports = {
	...
	processBlock(block, getParentCreateScopeRecord, inheritedVars, frozenNames, isRoot) {
		...
		const params = [...block.params].map(([name, {isFrozenName}]) => {
			...
			// `super`, `new.target` and `with` are not actually frozen
			if (isFrozenName && !['super', 'new.target', 'with'].includes(name)) {
		...
		for (const param of params) {
			...
			if (paramName === 'new.target') {
				...
				assert(!param.injectionVarNode, 'Cannot handle circular `with ()` value');
				...
				// Frozen var name (potentially used in `eval()`, or behind a `with (...)` block)
				...
				if (paramName === 'this' || (paramName === 'arguments' && paramsByName.this?.isFrozenName)) {
					...
					assert(
						...
						'Cannot handle circular `this` or `arguments` where `this` cannot be renamed due to use of `eval()` or `with ()`'
					);
				} else {
					newName = paramName;
		...
		if (frozenThisVarName) {
			// Uses frozen `this`.
			// Wrap return value in an IIFE to inject actual `this` (and `arguments`, if it's used too).
