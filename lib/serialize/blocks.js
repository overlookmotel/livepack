...
const {
	...
	{setAddFrom, replaceRecordNode, getNodeWithinWrapperParent, deleteFirst} = require('./utils.js');
...
module.exports = {
	...
	processBlock(block, getParentCreateScopeRecord, inheritedVars, frozenNames, isRoot) {
		...
		let paramNames = [...block.paramNames];
		const localVars = Object.create(null), // Keyed by param name
		...
		// Identify params which can be consolidated as refer to same import/export binding.
		// Vars cannot be consolidated where block contains `eval()` as `eval()` might access either var.
		const duplicateParams = Object.create(null), // Map of param name being replaced to replacement name
			{scopes: blockScopes, bindings, containsEval} = block,
			undefinedRecord = this.serializeValue(undefined);
		if (bindings && !containsEval) {
			const replacementNames = new Map(); // Keyed by binding object

			// Identify exported vars which can be consolidated with imported vars where self-import.
			// e.g. `import {x as y} from './self.js'; export const x = 1, fn = () => [x, y];`
			// `x` and `y` can be consolidated into a single var.
			// Cannot consolidate if multiple module scopes for this module and the exported var
			// is used in more than one scope.
			const {exportVarNames} = block;
			if (exportVarNames) {
				for (const paramName of exportVarNames) {
					let binding;
					for (const {exportBindings} of blockScopes.values()) {
						if (!exportBindings) continue;
						const exportBinding = exportBindings[paramName];
						if (exportBinding) {
							if (binding) {
								// Multiple bindings for this var in different module scopes - cannot be consolidated
								binding = undefined;
								break;
							}
					...
					if (binding) replacementNames.set(binding, paramName);
				}
			}

			// Consolidate imported vars which refer to same binding
			for (const [paramName, binding] of Object.entries(bindings)) {
				const replacementName = replacementNames.get(binding);
				if (!replacementName) {
					// Not encountered this binding before - record it in case encountered again
					replacementNames.set(binding, paramName);
				} else {
					// This binding encountered before - replace param
					duplicateParams[paramName] = replacementName;

					// Delete binding for replaced param
					delete bindings[paramName];

					// Transfer values to replacement param in all scopes
					for (const scope of blockScopes.values()) {
						const {values} = scope,
							paramValProps = values[paramName];
						if (!paramValProps) continue;

						const replacementValProps = values[replacementName];
						if (!replacementValProps) {
							values[replacementName] = paramValProps;
						} else {
							if (paramValProps.isCircular) {
								// I think it's possible for `values[paramName].isCircular` to be true
								// and `values[replacementName].isCircular` to be false
								// if there's a self-import involved. Something like:
								// ```js
								...
								replacementValProps.isCircular = true;
							}

							// Delete dependency of scope on value
							const valRecord = paramValProps.record,
								valDependents = valRecord.dependents;
							if (valDependents && valRecord !== undefinedRecord) { // Skip primitives and `undefined`
								const scopeRecord = scope.record;
								deleteFirst(scopeRecord.dependencies, dependency => dependency.record === valRecord);
								deleteFirst(
									valDependents,
			...
			// Delete replaced params from `paramNames`
			if (Object.keys(duplicateParams).length > 0) {
				paramNames = paramNames.filter(paramName => !duplicateParams[paramName]);
			}
		}
		...
		// Consolidate duplicate params
		for (const [paramName, replacementName] of Object.entries(duplicateParams)) {
			const replacementVarNodes = localVars[replacementName];
			for (const varNode of localVars[paramName]) {
				varNode.name = replacementName;
				replacementVarNodes.push(varNode);
			}
		}
