...
// Imports
const {
	STRING_TYPE, BOOLEAN_TYPE, NUMBER_TYPE, BIGINT_TYPE, NULL_TYPE, UNDEFINED_TYPE, NEGATIVE_TYPE,
	registerSerializer
} = require('./types.js');
...
function tracePrimitive(val, type, record) {
	switch (type) { // eslint-disable-line default-case
		...
		case 'number': return traceNumber.call(this, val, record);
		case 'bigint': return traceBigInt.call(this, val, record);
	}
...
function traceNumber(val, record) {
	if (val < 0) {
		...
		record.extra = {positive: this.traceValue(-val, null, null)};
...
function traceMinusZero(record) {
	record.name = 'minus0';
	record.extra = {positive: this.traceValue(0, null, null)};
	return NEGATIVE_TYPE;
}
...
function serializeNumber(record) {
	return t.numericLiteral(record.extra.val);
}
...
function serializeBigInt(record) {
	return t.bigIntLiteral(bigIntToString.call(record.extra.val));
}
 ...
 * @param {Object} record - Record
 * @param {Object} record.extra - Extra props object
 * @param {Object} record.extra.positive - Record for number / BigInt
 * @returns {Object} - AST node
 */
