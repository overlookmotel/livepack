...
function serializePrimitive(val) { // eslint-disable-line consistent-return
	...
	assertBug(false, `Unexpected primitive type '${type}'`);
}
...
function serializeBigInt(bigInt) {
	if (bigInt >= 0) return t.bigIntLiteral(bigIntToString.call(bigInt));
	return t.unaryExpression('-', t.bigIntLiteral(bigIntToString.call(-bigInt)));
}
