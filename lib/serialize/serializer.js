...
class Serializer {
	...
	serializeMain(val) {
		...
		const {elements} = this.processBlock(topBlock, parentCreateScopes, true).body;

		for (const {val: dependentVal} of topScope.record.dependents) {
	...
	processBlock(block, parentCreateScopes, isRoot) {
		...
		const isSingular = !isRoot && index + block.children.length === 1;
