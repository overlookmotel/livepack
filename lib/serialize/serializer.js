...
class Serializer {
	...
	serializeMain(record) {
		...
		const createScopeRecord = this.createRecord(null, 'createScopeRoot');
	...
	processBlock(block, parentCreateScopes, inheritedVars, isRoot) {
		...
		for (const childBlock of childBlocks) {
			...
			for (const scope of blockScopes) {
				...
				if (isSingular) {
					...
					const createScopeRecord = this.createRecord(null, createScopeName);
		...
		for (const paramName of paramNames) {
			...
			if (mangle || paramName === 'this' || paramName === 'arguments') {
