...
class Serializer {
	...
	serializeMain(record) {
		...
		function addDependencies(valRecord, isRoot) {
			...
			if (inline && !isRoot && dependents.length === 1) {
				...
				let {node} = valRecord;
				if (
					(t.isFunction(node) || t.isClass(node))
					&& !node.id && t.isObjectProperty(parentNode) && key === 'value'
				) {
	...
	processBlock(block, parentCreateScopes, inheritedVars, isRoot) {
		...
		for (const {
			...
			for (const [scope, fnRecord] of fnScopes) {
				...
				// (maintain wrapping in `Object.assign()` etc)
				if (t.isIdentifier(fnRecord.node)) {
					fnRecord.node = fnNode;
				} else {
					let argumentsNode = fnRecord.node.arguments;
					while (!t.isIdentifier(argumentsNode[0])) {
						argumentsNode = argumentsNode[0].arguments;
					}
					argumentsNode[0] = fnNode;
				}
			}
