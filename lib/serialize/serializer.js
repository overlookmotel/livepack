...
class Serializer {
	...
	processBlock(block, parentCreateScopes, inheritedVars, globalVarNames, isRoot) {
		...
		for (const {node, scopes: fnScopes, externalVars, internalVars, functionNames} of blockFunctions) {
			...
			const thisReservedVarNames = new Set(inheritedVarNames);
			for (const varName in externalVars) {
				...
				if (localVar) {
					...
					if (inheritiedVar) {
						...
						thisReservedVarNames.add(varName);
					}
				}
			}
			...
			for (const fnName of functionNames) {
				thisReservedVarNames.add(fnName);
				internalVarNames.add(fnName);
			}

			// Rename internal vars
			// Names will avoid clashing with inherited names and globals used within this function
			const transformVarName = this.createVarNameTransform(thisReservedVarNames);
		...
		// Rename params
		const reservedVarNames = new Set([...globalVarNames, ...inheritedVarNames, ...internalVarNames]);
		const transformVarName = this.createVarNameTransform(reservedVarNames);

		const {mangle} = this.options;
		for (const varName of params) {
			let newName = varName;
			if (mangle || varName === 'this' || varName === 'arguments') {
				newName = transformVarName(varName);
