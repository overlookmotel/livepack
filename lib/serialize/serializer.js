...
class Serializer {
	...
	serialize(val) {
		...
		let node = this.serializeValue(val, 'exports');
	...
	serializeMain(val) {
		...
		const {elements} = this.processBlock(rootBlock, parentCreateScopes, {}, globalVarNames, true).body;
		...
		function addDependencies(child, isRoot) {
			...
			if (inline && !isRoot && !assignments && dependents.length === 1) {
				...
				if (mangle) {
					varName = getNextVarName(varName, globalVarNames);
	...
	processBlock(block, parentCreateScopes, inheritedVars, globalVarNames, isRoot) {
		const returnNodes = [];
		...
		for (const varName of params) {
			localVars[varName] = {nodes: [], newName: undefined};
		}
		...
		function createInjection(paramName) {
			...
			localVars[paramName].nodes.push(outputParamNode);
		...
		// Create functions
		for (const {node, scopes: fnScopes, externalVars} of blockFunctions) {
			...
			for (const varName in externalVars) {
				const localVar = localVars[varName];
				if (localVar) {
					localVar.nodes.push(...externalVars[varName]);
				} else {
					const inheritiedVar = inheritedVars[varName];
					if (inheritiedVar) {
						inheritiedVar.nodes.push(...externalVars[varName]);
					} else {
		...
		// Rename params
		const inheritedVarNames = new Set(Object.values(inheritedVars).map(({newName}) => newName));

		function renameParams(transformName, getInjectName) {
			for (const varName of params) {
				let newName = transformName(varName, true);
				while (true) { // eslint-disable-line no-constant-condition
					if (!inheritedVarNames.has(newName)) break;
					newName = transformName(newName, false);
				}

				const injectionVarNode = injectionVarNodes[varName];
				if (injectionVarNode) injectionVarNode.name = getInjectName(newName);

				localVars[varName].newName = newName;
			}
		}
		...
		if (this.options.mangle) {
			let newName = '';
			renameParams(
				() => newName = getNextVarName(newName, globalVarNames), // eslint-disable-line no-return-assign
				varName => (varName === 'a' ? 'b' : 'a')
			);
		} else {
			// Change `this`/`arguments` to `_this`/`_arguments`
			renameParams(
				(varName, first) => (
					!first || varName === 'this' || varName === 'arguments' ? `_${varName}` : varName
				),
				varName => `_${varName}`
			);
		}
		...
		for (const childBlock of blockChildren) {
			...
			returnNodes[index++] = this.processBlock(
				childBlock, createScopes, nextInheritedVars, globalVarNames
			);
		}

		// Rename all var nodes
		const paramNodes = [];
		for (const varName of params) {
			const {newName, nodes} = localVars[varName];

			if (newName !== varName) {
				for (const varNode of nodes) {
					varNode.name = newName;
					if (varName === 'this') varNode.type = 'Identifier';
				}
			}
 ...
 * @param {Set} globalVarNames - Set of var names not to use
 * @returns {string} - Next name
 */
function getNextVarName(name, globalVarNames) {
	...
	const nextName = codes.map(code => String.fromCharCode(code)).join('');
	if (!globalVarNames.has(nextName)) return nextName;
	return getNextVarName(nextName, globalVarNames);
}
