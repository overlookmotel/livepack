	...
	serialize(val) {
		...
		if (format === 'exec') {
			...
			if (statementNodes.length === 1) {
				// Only 1 statement - remove `return ...` and wrap in brackets,
				// so brackets can unambiguously be removed later.
				statementNodes = [t.expressionStatement(t.parenthesizedExpression(exportNode.argument))];
				removeBrackets = true;
			} else {
				// Multiple statements - wrap in IIFE
				statementNodes = [
					t.expressionStatement(
						t.callExpression(t.arrowFunctionExpression([], t.blockStatement(statementNodes)), [])
					)
				];
			}
		}
		...
		// TODO This will throw all source map references off by 1 char. Fix it!
