...
const generate = require('@babel/generator').default,
	{last, pick, upperFirst} = require('lodash'),
...
const valueMethods = require('./values.js'),
	...
	{createDependency, createAssignment} = require('./dependencies.js'),
...
class Serializer {
	constructor(options) {
		...
		this.rootBlock = this.createBlock(null);
		this.rootScope = this.createScope(null, this.rootBlock, null);
	...
	serializeMain(record) {
		...
		const createScopeRecord = this.createRecord({}, t.identifier('createScopeRoot'));

		const parentCreateScopes = new Map([[null, createScopeRecord]]);
		const {node: blockNode, globalVarNames} = this.processBlock(rootBlock, parentCreateScopes, {}, true);
		...
		for (const {record: dependentRecord} of rootScopeRecord.dependents) {
			dependentRecord.node = elements.shift();
		...
		function addDependencies(valRecord, isRoot) {
			...
			if (inline && !isRoot && !assignments && dependents.length === 1) {
				...
				varNodes.push(t.variableDeclarator(varNode, valRecord.node));
		...
		let node;
		if (inline && record.dependents.length === 0) {
	...
	processBlock(block, parentCreateScopes, inheritedVars, isRoot) {
			...
			params.forEach((paramName, paramIndex) => {
				...
				if (!valProps) {
					...
					if (isCircular) {
						...
						if (valRecord) createDependency(scopeRecord, valRecord, argumentNodes, paramIndex);
						node = valProps.node;
		...
		for (const {node, scopes: fnScopes, externalVars, internalVars, functionNames} of blockFunctions) {
			for (const [scope, val] of fnScopes) {
				const scopeRecord = scope.record,
					fnRecord = records.get(val); // TODO Avoid this lookup by storing records in `fnScopes`
		...
		for (const childBlock of childBlocks) {
			...
			for (const scope of blockScopes) {
				...
				if (isSingular) {
					...
					const createScopeRecord = this.createRecord({}, t.identifier(createScopeName));
		...
		for (const varName of params) {
			...
			if (mangle || varName === 'this' || varName === 'arguments') {
				...
				if (newName !== varName) {
					// Rename all nodes
					for (const varNode of localVars[varName]) {
						varNode.name = newName;
						// TODO Do this when getting AST of function, not here
