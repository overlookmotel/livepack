...
class Serializer {
	...
	processBlock(block, parentCreateScopes) {
		...
		const returnNodes = blockFunctions.map((
			{id: fnId, node, scopes: fnScopes, hasDuplicateScopes}, index // eslint-disable-line no-shadow
		) => {
			...
			for (const [scope, instances] of fnScopes) {
				const scopeRecord = scope.record;
