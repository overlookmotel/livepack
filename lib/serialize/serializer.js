...
class Serializer {
	...
	serializeMain(val) {
		...
		function addDependencies(child, isRoot) {
			...
			if (inline && !isRoot && !assignments && dependents.length === 1) {
				...
				if (mangle) {
					// TODO Sanitize var name
					varName = varName; // eslint-disable-line no-self-assign
				}
				varNode.name = transformVarName(varName);
	...
	processBlock(block, parentCreateScopes, inheritedVars, globalVarNames, isRoot) {
		...
		// Rename params
		const reservedVarNames = new Set(globalVarNames);
		Object.values(inheritedVars).forEach(({newName}) => reservedVarNames.add(newName));
