...
class Serializer {
	...
	processBlock(block, parentCreateScopes, isRoot) {
		...
		for (const scope of blockScopes) {
				...
				if (!valProps) {
					...
					if (isCircular) {
						...
						if (!isPrimitive(val)) {
							...
							records.get(val).dependents.push({val: scope, node: argumentNodes, key: paramIndex});
						}
						node = valProps.node;
						numTrailingEmptyParams = 0;
					}
				}
			...
			if (numTrailingEmptyParams > 0) argumentNodes.length -= numTrailingEmptyParams;
		}
