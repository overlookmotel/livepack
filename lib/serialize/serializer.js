	...
	serializeMain(record) {
		...
		function addDependencies(valRecord, isRoot) {
			...
			if (inline && !isRoot && !assignments && dependents.length === 1) {
				...
				let {node} = valRecord;
				if (t.isFunction(node) && !node.id) node = t.sequenceExpression([t.numericLiteral(0), node]);
...
Object.assign(Serializer.prototype, valueMethods, functionMethods, objectMethods, arrayMethods);
