...
class Serializer {
	...
	processBlock(block, parentCreateScopes, isRoot) {
		...
		const blockFunctions = block.functions;
		const numFunctions = blockFunctions.length;

		const injectNodes = [];
		const injextIndexes = {};
		function getInjectionIndex(paramName) {
			let index = injextIndexes[paramName];
			...
			index = numFunctions + injectNodes.length;
			injectNodes.push(injectNode);
			injextIndexes[paramName] = index;
			return index;
		}
		...
		for (const scope of blockScopes) {
			...
			params.forEach((paramName, paramIndex) => {
				...
				if (!valProps) {
				...
				} else if (valProps.isCircular || isFunction(valProps.val)) {
					// TODO Don't inject all functions, only ones which are defined within this block
					...
					const injectionIndex = getInjectionIndex(paramName);
					...
					if (!isPrimitive(val)) {
						...
						records.get(val).dependents.push({
							val: scope, node: argumentNodes, key: paramIndex
						});
					}
				}
		...
		const isSingular = !isRoot && numFunctions + injectNodes.length + block.children.length === 1;

		// Create functions
		const returnNodes = blockFunctions.map(({node, scopes: fnScopes}, index) => {
		...
		if (injectNodes.length > 0) returnNodes.push(...injectNodes);
