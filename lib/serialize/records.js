...
const {deleteItem} = require('./utils.js'),
	{PRIMITIVE_TYPE, GLOBAL_TYPE, NO_TYPE} = require('./types.js');

// Constants
const PRIMITIVE_OR_GLOBAL_TYPE = PRIMITIVE_TYPE | GLOBAL_TYPE; // eslint-disable-line no-bitwise
...
function createDependency(record, dependencyRecord) {
	// Don't create dependencies for primitives or globals
	if (dependencyRecord.type & PRIMITIVE_OR_GLOBAL_TYPE) return; // eslint-disable-line no-bitwise

	let count;
	if (record.dependencies) {
		count = (record.dependencies.get(dependencyRecord) || 0) + 1;
		record.dependencies.set(dependencyRecord, count);
	} else {
		record.dependencies = new Map([[dependencyRecord, 1]]);
		count = 1;
	}

	if (dependencyRecord.dependents) {
		dependencyRecord.dependents.set(record, count);
	} else {
		dependencyRecord.dependents = new Map([[record, 1]]);
	}
}
