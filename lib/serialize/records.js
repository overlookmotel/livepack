...
function createRecord(name, val) {
	return {
		...
		output: undefined,
		// `record` property always remains undefined.
		// Only present to distinguish from assignments, which do have a `record` property.
		// `maybeAssignmentOrMaybeRecord.record` will not fall through to prototype
...
function createBlock(id, name, parent) {
	const block = {
		...
		// `.bindings` + `.exportVarNames` set in `lib/functions.js` if block contains live bindings
		bindings: undefined,
		exportVarNames: undefined,
		containsEval: false,
		argNames: undefined
	};
...
function createScope(id, block, parent) {
	const scope = {
		...
		exportBindings: undefined // Set in `lib/functions.js` if scope contains live bindings
	};
