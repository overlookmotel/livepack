...
module.exports = {
	...
	wrapFunctionWithProperties(fn, record, node, name, isClass, isGenerator, isArrow, isAsync) {
		// Determine if `.name` should be set, or delete if deleted
		const nameDescriptor = Object.getOwnPropertyDescriptor(fn, 'name');
		let excludeName;
		if (nameDescriptor) {
			// Check if `.name` changed, or descriptor props changed
			if (nameDescriptor.value !== name) {
				excludeName = false;
			} else {
				excludeName = !nameDescriptor.writable
					&& !nameDescriptor.enumerable
					&& nameDescriptor.configurable;
			}
		} else {
			// `.name` prop deleted - add assignment `delete fn.name`
			createAssignment(
				record, t.unaryExpression('delete', t.memberExpression(record.varNode), t.identifier('name'))
			);
			excludeName = false;
		}
	...
	convertPrototype(fn, fnRecord, fnNode, proto, protoRecord, protoNode, isGenerator, protoIsReferenced) {
		...
		*/
		...
		if (t.isIdentifier(fnNode)) {
			...
			if (definePropertiesRecord && fnNode.callee === definePropertiesRecord.varNode) {
				...
				const args = fnNode.arguments,
					{properties} = args[1];
				deletePrototypeProperty(properties, fnRecord, protoRecord);
				...
				if (properties.length === 0) {
					...
					const descriptorsAreNotRequired = !properties.find((propNode) => {
						...
						return ['writable', 'enumerable', 'configurable'].find((keyName, index) => {
							const descriptorPropNode = descriptorPropNodes[index + 1];
							if (descriptorPropNode.key.name !== keyName) return true;
							return !descriptorPropNode.value.value;
						});
					});
				...
				deleteFirst(
					fnNode.arguments[1].properties, propNode => propIsNamedIdentifier(propNode, 'prototype')
				);
		...
		if (removeFnAssignment) {
			...
			deletePrototypeProperty(properties, assignment, protoRecord);
...
function deletePrototypeProperty(properties, dependencyRecord, protoRecord) {
	const propNode = deleteFirst(
		properties, // eslint-disable-next-line no-shadow
		propNode => propIsNamedIdentifier(propNode, 'prototype')
	);

	const protoDependentNode = propNode.value.properties[0];
...
function parseFunction(fn, js, isMethod) {
	...
	const idNode = !isArrow && fnName && isJsIdentifier(fnName) ? t.identifier(fnName) : null;
