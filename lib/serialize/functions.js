...
module.exports = {
	serializeFunction(fn) {
		...
		if (!fnDef) {
			...
			if (virtualBlock) {
				const counter = (scopesCounters.get(scope) || 0);
				scopesCounters.set(scope, counter + 1);

				scope = this.createScope(`${scope.id}$${counter}`, virtualBlock, scope);
			}
		}
		...
		return {fnId, scope}; // TODO Is it neccesary to return `fnId`?
	},
