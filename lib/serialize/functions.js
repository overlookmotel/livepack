...
const {setTrackerCallback, trackerError} = require('../shared/tracker.js'),
	...
	{TRACKER_COMMENT_PREFIX, TOP_BLOCK_ID} = require('../shared/constants.js'),
	{
...
module.exports = {
	serializeFunction(fn, record) {
		...
		const {id: fnId, scopes: scopeDefs, argNames} = info,
		...
		if (isNotParsedBefore) {
			...
			fnDef = this.parseFunction(
				...
				isMethod, !!info.superIsProto, info.isEval, !!this.options.sourceMaps
			);
			...
			const unusedBlockIndexes = fnDef.unusedBlockIndexes = new Set();

			functions.set(fnId, fnDef);

			// Create blocks
			const {readFromVarNames, assignedToVarNames} = fnDef;
			let parentBlock = this.rootBlock;
			scopeDefs.forEach((scopeDef, blockIndex) => {
				// Skip block if no vars in block used (can happen if only usage was a const violation)
				const {varNames} = scopeDef;
				if (!varNames.some(
					varName => readFromVarNames.has(varName) || assignedToVarNames.has(varName)
				)) {
					unusedBlockIndexes.add(blockIndex);
					return;
				}
				...
				const {paramNames, mutableNames} = block;
				for (const varName of varNames) {
					if (assignedToVarNames.has(varName)) {
						mutableNames.add(varName);
					} else if (!readFromVarNames.has(varName)) {
						continue;
					}
				...
				if (varNames.includes('arguments') && argNames && !block.argNames) {
		...
		if (scopeDefs.length > 0) {
			...
			try {
				...
				err = _err;
				if (err === null || typeof err !== 'object') err = {message: `${err}`};
			}
			...
			if (expectPromise) {
				...
				err = {message: 'Tracker function did not throw an error as expected'};
			}
		...
		// Record scope values
		const {readFromVarNames, unusedBlockIndexes} = fnDef;
		let block = this.rootBlock,
			scope = this.rootScope;
		scopeDefs.forEach(({blockId, varNames}, blockIndex) => {
			// Skip block if no vars in block used (can happen if only usage was a const violation)
			if (unusedBlockIndexes.has(blockIndex)) return;
			...
			varNames.forEach((varName, varIndex) => {
				if (!readFromVarNames.has(varName)) return;
		...
		const {importNames, exportNames} = info;
		if ((importNames || exportNames) && !unusedBlockIndexes.has(0)) {
			const moduleBlock = blocks.get(TOP_BLOCK_ID);

			let scopeVarsIndex = scopeDefs.length;
			if (importNames) {
				// Function uses vars which are imported from other modules.
				// Resolve each to ultimate binding and record bindings on block.
				const importUrls = scopeVars[scopeVarsIndex],
					blockBindings = moduleBlock.bindings || (moduleBlock.bindings = Object.create(null));
				importNames.forEach(([localName, targetName], index) => {
					if (!blockBindings[localName] && readFromVarNames.has(localName)) {
						blockBindings[localName] = this.recordImportBinding(importUrls[index], targetName);
					}
				});
			...
			if (exportNames) {
				// Function uses vars which are exported from module.
				// Record exported var name on block and binding on scope.
				const exportUrl = scopeVars[scopeVarsIndex],
					moduleScope = moduleBlock.scopes.get(scopeVars[0][0]),
					{assignedToVarNames} = fnDef;
				let {exportVarNames} = moduleBlock,
					{exportBindings} = moduleScope;
				for (const exportName of exportNames) {
					const isMutable = assignedToVarNames.has(exportName);
					if (!isMutable && !readFromVarNames.has(exportName)) continue;

					const binding = this.recordExportBinding(exportUrl, exportName);
					if (isMutable) binding.isMutable = true;

					if (!exportVarNames) exportVarNames = moduleBlock.exportVarNames = new Set();
					exportVarNames.add(exportName);
					if (!exportBindings) exportBindings = moduleScope.exportBindings = Object.create(null);
					exportBindings[exportName] = binding;
				}
			}
		}
		...
		return this.wrapFunctionWithProperties(
			...
			isClass, isAsync, isGenerator, fnDef.isArrow, isMethod
		);
	},
	...
	wrapFunctionWithProperties(
		...
		// Set default `length` property based on function definition
		const defaultProps = [
		...
		// Anonymous classes have no `name` property on some versions of Node.
		// In Node < v16.9.0 classes' `name` property comes after `prototype` property and static methods.
		// Identify any changes made to `name` property (as far as possible) on current Node version.
		// Produce output which will produce same `configurable` descriptor property on `name` prop
		// regardless of where it runs.
		// Always output `name` prop before static methods on classes.
		// This is not perfect - the position of the `name` property will differ from input on versions
		// of Node prior to v16.9.0, but best we can do, and the difference will not be significant except
		// in very niche cases where application relies on exact order of a class's properties
		// (which would mean the application would rely on a precise Node version).
		let defaultName, defaultNamePropConfigurable;
		if (!isClass || name) {
			defaultName = name;
			defaultNamePropConfigurable = true;
		} else {
			// Defined initially as anonymous class, either because it is anonymous, or because name is
			// not a string or not a valid JS identifier.
			// In these cases, name property needs to be defined explicitly with `Object.defineProperty()`.
			...
			if (nameDescriptor) {
				if (
					...
					// Name prop needs to be set - ensure `configurable` will always be set
					// so output code will produce same result, regardless of where it runs
					defaultNamePropConfigurable = !nameDescriptor.configurable;
				} else if (anonClassHasNameProp) {
					// Name prop should not be set - ensure default prop matches actual prop
					defaultName = '';
					defaultNamePropConfigurable = true;
				}
			} else if (anonClassHasNameProp) {
				// `name` prop has been deleted - create default prop for `wrapWithProperties()` to delete
				defaultNamePropConfigurable = true;
			}
		}

		const isClassWithNamePropLast = isClass && classHasNamePropLast;
		if (defaultNamePropConfigurable !== undefined) {
			const defaultProp = {
				name: 'name',
				value: defaultName,
				writable: false,
				enumerable: false,
				configurable: defaultNamePropConfigurable
			};

			if (isClassWithNamePropLast) {
				defaultProps.push(defaultProp);
			} else {
				defaultProps.splice(1, 0, defaultProp);
			}
		}

		let fnWithPropsReordered = fn;
		if (isClassWithNamePropLast) {
			const propNames = Object.getOwnPropertyNames(fn),
				nameIndex = propNames.indexOf('name');
			if (nameIndex !== -1 && nameIndex !== propNames.indexOf('prototype') + 1) {
				// Re-order properties so `name` prop comes directly after `prototype` prop,
				// rather than after static methods
				fnWithPropsReordered = Object.create(Object.getPrototypeOf(fn));

				const descriptors = Object.getOwnPropertyDescriptors(fn);
				for (const propName of propNames.concat(Object.getOwnPropertySymbols(fn))) {
					if (propName === 'name') continue;
					Object.defineProperty(fnWithPropsReordered, propName, descriptors[propName]);
					if (propName === 'prototype') {
						Object.defineProperty(fnWithPropsReordered, 'name', descriptors.name);
					}
				}

				if (Object.isFrozen(fn)) {
					Object.freeze(fnWithPropsReordered);
				} else if (Object.isSealed(fn)) {
					Object.seal(fnWithPropsReordered);
				} else if (!Object.isExtensible(fn)) {
					Object.preventExtensions(fnWithPropsReordered);
				}
			}
		}
		...
		node = this.wrapWithProperties(
			fnWithPropsReordered, record, node, defaultProto, defaultProps,
			functionShouldSkipKey, functionShouldForceDescriptor
		);
