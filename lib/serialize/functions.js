...
const {setTrackerCallback, resetTrackerCallback, trackerError} = require('../shared/tracker.js'),
	...
	{identifierIsVariable} = require('../shared/functions.js'),
	{
		...
		FN_TYPE_ASYNC_GENERATOR_FUNCTION, FN_TYPE_CLASS
...
module.exports = {
	serializeFunction(fn, record) {
		...
		let fnDef = functions.get(fnId),
			scopeDefs, isClass, isAsync, isGenerator, callArguments;
		const isNotParsedBefore = !fnDef;
		if (isNotParsedBefore) {
			({isClass, isAsync, isGenerator} = commentTypePropsMap[commentMatch[2]]);
			const callArgumentsStr = commentMatch[3];
			callArguments = callArgumentsStr ? JSON.parse(callArgumentsStr) : [];
		} else {
			({scopeDefs, isClass, isAsync, isGenerator, callArguments} = fnDef);
		}
		...
		if (isNotParsedBefore || scopeDefs.length > 0) {
			let getScopes;
			setTrackerCallback((_getFnInfo, _getScopes) => {
				getFnInfo = _getFnInfo;
		...
		if (isNotParsedBefore) {
			...
			fnDef = this.parseFunction(fn, getFnInfo, isClass, isAsync, isGenerator, filename);
			scopeDefs = fnDef.scopeDefs;

			fnDef.callArguments = callArguments;
			...
			// Create blocks
			const {readFromVarNames, assignedToVarNames, argNames} = fnDef;
