...
module.exports = {
		...
		for (let i = 0; i < scopeDefs.length; i++) {
			...
			for (let j = 0; j < varNames.length; j++) {
				...
				if (!(varName in scopeValues)) {
					...
					const isCircular = valRecord.node === undefined;
					scopeValues[varName] = {val, node: valRecord.varNode || valRecord.node, isCircular};
	...
	serializeArguments(args, record) {
		...
		const {node: arrayNode} = this.serializeArray(args, record, 'arguments');
	...
	createBlock(id, name, blocks) {
		if (!name) name = 'anon';
		// TODO Convert `params` to a Set to avoid using `.includes()` on it
		const block = {id, name, children: [], functions: [], scopes: [], params: [], argNames: undefined};
	...
	createScope(id, block, parentScope) {
		...
		const record = this.createRecord(scope, t.identifier(`scope${upperFirst(block.name)}`));
		scope.record = record;
...
function parseFunction(fn, js, isClass, isMethod) {
	...
	traverse(ast, {
		...
		ThisExpression(path) {
			...
			if (!enclosingFunctionPath) addToVars('this', path.node, externalVars);
		}
	});
