...
module.exports = {
	serializeFunction(fn, record) {
		...
		if (isNotParsedBefore) {
			...
			} = parseFunction(fn, js, externalVarNames, isMethod);
	...
	wrapFunctionWithProperties(
		...
		node = this.wrapWithProperties(fn, record, node, defaultProto, defaultProps, functionShouldSkipKey);
 ...
 */
function parseFunction(fn, js, externalVarNames, isMethod) {
