...
module.exports = {
	serializeFunction(fn) {
		...
		if (!fnDef) {
			...
			fnDef = {
				id: fnId, node: fnNode, scopes: fnScopes, virtualBlock: undefined, scopesCounters: undefined
			};
			...
			let {virtualBlock, scopesCounters} = fnDef;
			if (fnScopes.has(parentScope)) {
				...
				parentBlock.children.push(virtualBlock);

				// Move function into new block
				virtualBlock.functions.push(fnDef);
				parentBlock.functions = parentBlock.functions.filter(thisFnDef => thisFnDef !== fnDef);

				// Move all previous instances into virtual scopes
				// (one virtual scope for each parent scope).
				// Counters map is to ensure all virtual scopes have unique IDs.
				const oldFnScopes = fnScopes;
				fnScopes = new Map();
				scopesCounters = new Map();
				for (const [instanceScope, instance] of oldFnScopes) {
					const virtualScope = this.createScope(`${instanceScope.id}$0`, virtualBlock, instanceScope);
					fnScopes.set(virtualScope, instance);
					scopesCounters.set(instanceScope, 1);
				}
				fnDef.scopes = fnScopes;
				fnDef.scopesCounters = scopesCounters;
			}

			// If some scopes hold multiple instances of this function, use virtual block to hold scope
			if (virtualBlock) {
				const counter = (scopesCounters.get(parentScope) || 0);
				scopesCounters.set(parentScope, counter + 1);

				parentScope = this.createScope(`${parentScope.id}$${counter}`, virtualBlock, parentScope);
			}
		}
