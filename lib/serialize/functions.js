...
module.exports = {
	serializeFunction(fn, record) {
		...
		// Get/create file
		const {files, isCurrentlyInSplit} = this;
		...
		if (isNotParsedBefore) {
			...
			// Create blocks (unless reached here from a split point)
			if (!isCurrentlyInSplit) {
		...
		scopeDefs.forEach(({blockId, varNames}, blockIndex) => {
			...
			// If reached here from split point, add values as dependencies of function instance
			// instead of adding to scope
			if (isCurrentlyInSplit) {
				varNames.forEach((varName, varIndex) => {
					...
					// Ignore circular dependencies as they must also have been reached from same split point
					if (!recordIsCircular(valRecord)) createDependency(record, valRecord, undefined, undefined);
				});
		...
		// Place function in block (unless reached here from split point)
		if (!isCurrentlyInSplit) {
	...
	wrapFunctionWithProperties(
		...
		// Workaround for some versions of Node where anonymous classes have a 'name' property
