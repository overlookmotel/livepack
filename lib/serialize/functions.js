...
const tracker = require('../tracker.js'),
	...
	{
		identifierIsVariable, replaceWith, TRACKER_COMMENT_PREFIX, TEMP_COMMENT_PREFIX,
	...
	{isJsIdentifier, isNumberKey, recordIsCircular, deleteFirst, setAddFrom} = require('./utils.js'),
	...
	serializeFunction(fn, record) {
		...
		for (let i = 0; i < scopeDefs.length; i++) {
			...
			for (let j = 0; j < varNames.length; j++) {
				...
				if (!hasOwnProp(scopeValues, varName)) {
					const val = values[j];
...
function parseFunction(fn, js, externalVarNames, isMethod, isProtoMethod, superVarName) {
	...
	traverse(ast, {
		...
		ThisExpression(path) {
			...
			} else if (thisReplacementNode && functionDepth === 1) {
				// Replace `this` with `_this` in class constructor
				path.replaceWith(thisReplacementNode);
			}
		},
		Super(path) {
			// Add to external vars if `super` refers to `super` from a higher function
			...
			if (
				!isSuperCall && expressionPath.parentPath.isAssignmentExpression()
				&& expressionPath.key === 'left'
			) {
			...
			addToVars(superVarName, superVarNode, externalVars);

			const reflectNode = t.identifier('Reflect'),
				objectNode = t.identifier('Object');
			...
			if (isSuperCall) {
				...
				if (!thisReplacementNode) {
					thisReplacementNode = path.scope.generateUidIdentifier('this');
				...
				// `_this = Reflect.construct(Object.getPrototypeOf(Klass), [...], Klass)`
				replacementNode = t.assignmentExpression(
					'=',
					thisReplacementNode,
					t.callExpression(
						...
						[
							...
							t.arrayExpression(expressionNode.arguments),
							superVarNode
						]
					)
				);
