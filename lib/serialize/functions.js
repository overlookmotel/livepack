...
const {setTrackerCallback, resetTrackerCallback, trackerError} = require('../shared/tracker.js'),
	...
	{identifierIsVariable, setAddFrom} = require('../shared/functions.js'),
	{
...
module.exports = {
	serializeFunction(fn, record) {
		...
		const fnId = commentMatch[1] * 1,
			filename = JSON.parse(`"${commentMatch[4]}"`);
		...
		if (isNotParsedBefore) {
			...
			for (const [blockId, scopeDef] of scopeDefs) {
				...
				for (const [varName, {isAssignedTo}] of Object.entries(vars)) {
					...
					if (isAssignedTo) mutableNames.add(varName);
				}

				// Record argument names
				if (vars.arguments && argNames && !block.argNames) {
