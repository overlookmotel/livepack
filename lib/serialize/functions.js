...
module.exports = {
	...
	wrapFunctionWithProperties(
		...
		function shouldExclude(propName, expectedValue) {
			...
			const memberNode = t.memberExpression(record.varNode, t.identifier(propName));
			const assignmentNode = t.unaryExpression('delete', memberNode);
			createAssignment(record, assignmentNode, memberNode, 'object');
			return false;
		}
		...
		*/
		...
		const descriptors = Object.getOwnPropertyDescriptors(fn);
		if (descriptors.arguments) {
			expectedProps.push(...[
				{name: 'arguments', value: null, writable: false, enumerable: false, configurable: false},
				{name: 'caller', value: null, writable: false, enumerable: false, configurable: false}
			]);
		}

		if (!isArrow && (!isAsync || isGenerator) && !isMethod) {
			// Should have prototype
			const expectedProtoProp = {
				name: 'prototype', value: undefined, writable: !isClass, enumerable: false, configurable: false
			};

			const protoDescriptor = descriptors.prototype;
			if (protoDescriptor && !protoDescriptor.get && !protoDescriptor.set) {
				const proto = protoDescriptor.value;
				if (isGenerator) {
					...
					if (
						...
						expectedProtoProp.value = proto;
					} else if (proto.value === undefined) {
						expectedProtoProp.value = null;
					}
				} else {
					// Prototype should have constructor
					// TODO Deal with where prototype redefined as non-object
					expectedProtoProp.value = proto;
				}
			}

			expectedProps.push(expectedProtoProp);
		}

		// Wrap with properties
		node = this.wrapWithProperties(fn, record, node, expectedProps);
