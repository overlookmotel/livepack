		...
		*/
		...
		if (!isArrow && (!isAsync || isGenerator) && !isMethod) {
			...
			if (isGenerator) {
				...
				// TODO Deal with case where prototype already serialized
				if (
					proto
			...
			if (protoValue) {
				this.prototypes.set(proto, record);
		...
		node = this.wrapWithProperties(fn, record, node, defaultProps);
		...
		if (protoValue) {
			...
			if (protoIsAltered) {
				...
				const protoDefaultProps = isGenerator
					? []
					: [{name: 'constructor', value: fn, writable: true, enumerable: false, configurable: true}];
				this.serializeProperties(proto, protoRecord, null, null, protoDefaultProps, null, true);
