...
module.exports = {
	serializeFunction(fn) {
		...
		for (let i = 0; i < scopeDefs.length; i++) {
			...
			// Add values to scope
			const {params} = block;
			const {values: scopeValues, valueNodes: scopeValueNodes} = scope;
			for (let j = 0; j < varNames.length; j++) {
				const varName = varNames[j];

				if (!(varName in scopeValues)) {
					const val = values[j];
					scopeValues[varName] = val;
					scopeValueNodes[varName] = this.serializeValue(val);
		...
		if (!functionDef) {
			...
			functionDef = {id: fnId, node: fnNode, instances: []};
			functions.set(fnId, functionDef);

			previousBlock.functions.push(functionDef);
		}
		functionDef.instances.push({val: fn, scope: previousScope});
	...
	createScope(id, block, parentScope) {
		const scope = {id, values: {}, valueNodes: {}, parentScope, record: undefined};
		const record = this.createRecord(scope, t.identifier(`scope${id}`)); // TODO Change to 'x'
		record.dependencies = [];
		scope.record = record;
