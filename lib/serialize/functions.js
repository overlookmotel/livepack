...
// Imports
const {setTrackerCallback, resetTrackerCallback, trackerError} = require('../shared/tracker.js'),
...
module.exports = {
	serializeFunction(fn, record) {
		...
		const {id: fnId, scopes: scopeDefs, argNames} = info,
			...
			callArguments = info.callArguments || [];
		...
		let getAst, scopeVars;
		if (isNotParsedBefore || scopeDefs.length > 0) {
			let getScopes;
			setTrackerCallback((_getAst, _getScopes) => {
				getAst = _getAst;
				getScopes = _getScopes;
			});
			...
			try {
				if (isClass) {
					res = new fn(...callArguments); // eslint-disable-line new-cap
				} else {
					res = fn(...callArguments);
					if (isGenerator) {
		...
		if (isNotParsedBefore) {
			// Function not encountered before - identify variable nodes
			const externalVarNames = new Set(),
				constVarNames = new Set(),
				constInStrictVarNames = new Set();
			for (const {varNames, constNames, constInStrictNames} of scopeDefs) {
				setAddFrom(externalVarNames, varNames);
				if (constNames) setAddFrom(constVarNames, constNames);
				if (constInStrictNames) {
					setAddFrom(constVarNames, constInStrictNames);
					setAddFrom(constInStrictVarNames, constInStrictNames);
				}
			}

			fnDef = this.parseFunction(
				fn, getAst, filename, externalVarNames, constVarNames, constInStrictVarNames,
				isClass, isAsync, isGenerator, !!info.isStrict, !!info.containsEval,
				isClass || !!info.superIsProto, !!this.options.sourceMaps
			);
		...
		scopeDefs.forEach(({blockId, varNames}, blockIndex) => {
			...
			varNames.forEach((varName, varIndex) => {
				...
				if (!scopeValue) {
					let val = values[varIndex];
					if (!val && varName === 'super') val = fn; // `super` used in class constructor
