...
const tracker = require('../tracker.js'),
	{boundFunctions} = require('../internal.js'),
	{argumentProxy} = require('./argumentProxy.js'),
	{
...
module.exports = {
	serializeFunction(fn, record) {
		...
		const {id: fnId, scopes: scopeDefs, isMethod, untracked} = info;

		let filename = info.filename || '';
		if (untracked) {
			assert(filename === 'tracker.js', 'Unexpected untracked file');
			filename = pathJoin(__dirname, '..', filename);
		}
		...
		if (isNotParsedBefore) {
			...
			const {
				...
				externalVarNames, isMethod, info.isProtoMethod, info.isStaticMethod, info.superVarName,
				!!untracked
			);
		...
		let scopeVars;
		if (untracked) {
			if (untracked === 'tracker') {
				scopeVars = [[1, undefined, 1]];
			} else {
				assert(untracked === 'setCallback');
				scopeVars = [[1, undefined]];
			}
		} else if (scopeDefs.length > 0) {
