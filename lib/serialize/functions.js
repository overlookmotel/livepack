...
module.exports = {
	...
	wrapFunctionWithProperties(
		...
		node = this.wrapWithProperties(fn, record, node, defaultProps, functionShouldSkipKey);
...
function parseFunction(fn, js, isMethod) {
	...
	if (isMethod) {
		...
		params = node.params;
		node = t.functionExpression(idNode, params, node.body, node.generator, node.async);
		isClass = false;
	} else {
