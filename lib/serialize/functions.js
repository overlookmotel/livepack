...
module.exports = {
	serializeFunction(fn, record) {
		...
		if (isNotParsedBefore) {
			...
			const {
				node: fnNode, externalVars, internalVars, functionNames, isClass, isGenerator
		...
		return this.wrapFunctionWithProperties(fn, record, t.identifier('x'), fnName);
	},
	...
	wrapFunctionWithProperties(fn, record, node, name) {
		const includeNameProp = fn.name !== name;
		// TODO Deal with prototype
		return this.wrapWithProperties(
			fn, record, node,
			key => key === 'length' || key === 'prototype' || (key === 'name' && !includeNameProp),
			key => key === 'name'
		);
	},
...
function parseFunction(fn, js, isMethod) {
	...
	return {node, externalVars, internalVars, functionNames, isClass, isGenerator: !!node.generator};
}
