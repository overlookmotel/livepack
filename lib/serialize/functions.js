...
module.exports = {
	serializeFunction(fn, record) {
		...
		if (isNotParsedBefore) {
			...
			if (fnDef.containsEval) freezeVarNames = true;
		}
		...
		for (let i = 0; i < scopeDefs.length; i++) {
			...
			if (!block) {
				block = createBlock(blockId, blockName || 'anon', parentBlock, blocks);
			} else {
			...
			// Add values to scope
			const {paramNames, frozenNames} = block;
			...
			for (let j = 0; j < varNames.length; j++) {
				...
				if (freezeVarNames) frozenNames.add(varName);
			}
	...
	serializeDebuglogFunction(info, record) {
		...
		info.args.forEach((arg, index) => {
			const argRecord = this.serializeValue(arg, `debuglogArg${index}`, `<debuglog argument ${index}>`);
			argumentsNodes[index] = argRecord.varNode;
			createDependency(record, argRecord, argumentsNodes, index);
		});

		const debuglogRecord = this.serializeValue(debuglog);
		const node = t.callExpression(
			t.memberExpression(debuglogRecord.varNode, t.identifier('call')),
			argumentsNodes
		);
		createDependency(record, debuglogRecord, node.callee, 'object');
		return node;
	},
	...
	serializeArguments(args, record) {
		...
		const arrayNode = this.serializeArray(args, record);
