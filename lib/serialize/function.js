	...
	serializeFunction(fn) {
		...
		const fnScopeVars = scopeVars.map(([scopeId, values, blockId]) => {
			...
			if (scope) {
				...
				if (!block) {
					const blockNode = t.identifier(`createScope${blockId}`); // TODO Change to 'x'
					const blockRecord = this.createRecord({}, blockNode);
					block = this.createBlock(blockId, blockRecord);
				}

				previousBlock.children.add(block);

				const scopeNode = t.identifier(`scope${scopeId}`); // TODO Change to 'x'
				const scopeRecord = this.createRecord({}, scopeNode);

				scope = {id: scopeId, record: scopeRecord, values: {}, block, parentScope: previousScope};
		...
		return {fnId, scopeVars: fnScopeVars};
	},

	createBlock(id, record) {
		const block = {id, record, children: new Set(), functions: [], argNames: undefined};
