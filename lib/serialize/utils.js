...
/**
 * Get class prototype keys which have setters.
 ...
 * @returns {Set} - Set of keys which have setters
 */
function getClassProtoSetters(Klass) {
	const setters = new Set();
	let proto = Klass.prototype;
	do {
		const descriptors = Object.getOwnPropertyDescriptors(proto);
		for (const key of Object.keys(descriptors).concat(Object.getOwnPropertySymbols(descriptors))) {
			if (!('value' in descriptors[key])) setters.add(key);
		}
