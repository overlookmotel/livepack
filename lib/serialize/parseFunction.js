 ...
 *   {Set} .globalVarNames - Set of names of global vars used
 *   {Set} .functionNames - Set of function names used
...
module.exports = function parseFunction(
	fn, js, filename, externalVarNames, constVarNames,
	isMethod, isProtoMethod, superVarName, isEval, sourceMapsEnabled
) {
	...
	traverse(ast, {
		...
		Function: {
			enter(path) {
				...
				// Remove tracker comment
				if (!isConstructor && (!computedKey || !isTopFunction)) {
					// eslint-disable-next-line no-nested-ternary
					const commentHolderNode = (path.isMethod() && fnNode.computed)
						? fnNode.key
						: fnNode.params.length > 0
							? fnNode.params[0]
							: fnNode.body;
					commentHolderNode.leadingComments.shift();
				}
				...
				if (isConstructor && !isTopFunction && isUselessConstructor(fnNode)) {
					path.remove();
					return;
				}
			...
			exit(path) {
				if (!path.isArrowFunctionExpression() && !path.node[IS_INTERNAL]) fullFunctionDepth--;
			}
		},
		...
		Identifier(path) {
			...
			if (binding) {
			...
			} else if (name !== 'arguments' || fullFunctionDepth === 0) {
				if (externalVarNames.has(name)) {
					...
					if (parentPath.isAssignmentExpression() && path.key === 'left') {
						...
						// `for (c in {x: 1}) {}` / `for (c of [1, 2, 3]) {}`
						if (constVarNames.has(name)) {
							// `for (c in {x: 1}) { foo(); }` -> `for (const c in {x: 1}) { c = 0; foo(); }`
					...
					// TODO `eval` can also be an external var in non-strict mode
					if (name === 'eval') {
...
function createConstViolationThrowNode(name) {
	// `(() => { const c = 0; c = 0; })()`
	const fnNode = t.arrowFunctionExpression([], createConstViolationThrowBlockNode(name));
	fnNode[IS_INTERNAL] = true;
	return t.callExpression(fnNode, []);
}
...
function createConstViolationThrowAssignNode(name) {
	...
	const methodNode = t.objectMethod(
		'set',
		t.identifier('a'),
		[t.identifier('v')],
		createConstViolationThrowBlockNode(name)
	);
	methodNode[IS_INTERNAL] = true;
	return t.memberExpression(t.objectExpression([methodNode]), t.identifier('a'));
}
