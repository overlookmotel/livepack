...
const {
		...
		INTERNAL_VAR_NAMES_PREFIX, TRACKER_VAR_NAME_BODY, SCOPE_ID_VAR_NAME_BODY,
		TEMP_COMMENT_PREFIX
	} = require('../shared.js'),
...
module.exports = function parseFunction(
	fn, js, filename, externalVarNames, isMethod, isProtoMethod, superVarName
) {
	...
	if (isMethod) {
		...
		// Set function name
		const functionNameReferredTo = functionNameNodes.length !== 0;
		let treatNameAsInternalVar = false;
		if (functionNameReferredTo) {
			const originalName = node.id.name;
			if (name !== originalName) {
				if (name && isJsIdentifier(name) && !globalVarNames.has(name) && !functionNames.has(name)) {
