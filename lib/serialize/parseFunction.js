...
function parseFunction(fn, js, filename, externalVarNames, isMethod, isProtoMethod, superVarName) {
	...
	if (filename) {
		...
		let sourceMapConsumer = transpiledFile.consumer;
		if (!sourceMapConsumer) {
			const sourceMap = sourceMapFromComment(transpiledFile.sourceMapComment).toObject();
		...
		traverse(ast, {
			enter(path) {
				...
				const newEnd = sourceMapConsumer.originalPositionFor({line: end.line, column: end.column});
				assert(newEnd.source === source);

				amendLocs.push({loc, newStart, newEnd});
			}
		});

		for (const {loc, newStart, newEnd} of amendLocs) {
			loc.filename = newStart.source;
			const {name} = newStart;
