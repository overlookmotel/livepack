		...
		Function: {
			enter(path) {
				...
				if (!isConstructor && (!computedKey || topFunctionPath)) {
					let comments = path.isMethod() && fnNode.computed
						? fnNode.key.leadingComments
						: firstStatementPath.node.leadingComments;
					if (!comments) comments = fnNode.body.directives[0].leadingComments;
					comments.shift();
				}
