...
const {isJsIdentifier, isNumberKey, setAddFrom} = require('./utils.js'),
	{
		...
		getProp, getProps, setProp, isRuntimePath, traverseAll
...
module.exports = function parseFunction(
	...
	if (!isClass) {
		...
		} else if (isRuntimePath(filename)) {
...
function resolveFunctionInfo(
	fnInfo, getInfos, isNestedFunction, fnId, isInClassWithSuperClass,
	...
	fnInfo.childFns.forEach((trail, index) => {
		...
		resolveFunctionInfo(
			childInfo, childGetInfos, true, fnId, childInClassWithSuperClass, scopeDefs,
			externalVars, internalVars, globalVarNames, functionNames, amendments
		);
