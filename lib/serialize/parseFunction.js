...
const {identifierIsVariable} = require('../shared/functions.js'),
	{
		internalIdentifier, flagAsInternal, isInternalNode, isJsIdentifier, isNumberKey, setAddFrom
	} = require('./utils.js'),
 ...
 * @param {Function} getFnInfo - Function info getter function
 ...
 *   {Array<Object>} .scopeDefs - Scope definitions array
 ...
 */
module.exports = function parseFunction(fn, getFnInfo, isClass, isAsync, isGenerator, filename) {
	// Get function info and AST from info getter function
	const [infoJson, getChildInfos, getSources] = getFnInfo();
	const info = JSON.parse(infoJson);
	const {scopes: scopeDefs, isStrict, containsEval, argNames} = info,
		superIsProto = info.superIsProto || isClass;

	// If source maps enabled, add source files to `sourceFiles` map
	let filenameForSourceMaps;
	if (this.options.sourceMaps) {
		filenameForSourceMaps = filename;
	...
	// Add child functions into AST
	let node = resolveAst(info.ast, info.childFns, getChildInfos, filenameForSourceMaps);

	// Init variable names sets
	...
	if (isMethod) {
		...
		paramNodes = ctorNode ? ctorNode.params : [];
 ...
 * @param {Object} node - Function AST
 * @param {Array<string|number>} fnTrails - Paths to location in AST of child functions
 ...
 */
function resolveAst(node, fnTrails, getInfos, filename) {
	// Insert child functions' ASTs into this AST
	fnTrails.forEach((trail, index) => {
		const [childJson, childGetInfos] = getInfos[index]();
		const {ast: childNode, childFns: childFnTrails} = JSON.parse(childJson);
		setProp(node, trail, resolveAst(childNode, childFnTrails, childGetInfos, filename));
	});
