...
function parseFunction(fn, js, externalVarNames, isMethod, isProtoMethod, superVarName) {
	...
	traverse(ast, {
		...
		Identifier(path) {
			...
			if (idNode === thisReplacementNode) return;
		...
		Super(path) {
			...
			if (isSuperCall) {
				...
				if (!thisReplacementNode) {
					// Create var for `_this`.
					// Set as internal var manually as recognising binding in `Identifier` visitor
					// doesn't work for some reason.
					thisReplacementNode = path.scope.generateUidIdentifier('this');
					internalVars[thisReplacementNode.name] = [thisReplacementNode];

					// Insert `let _this;` at start of block + `return _this;` at end of block
