			...
			enter(path) {
				...
				const trackerIsRemovedFromParams = fnNode.params.some((_, paramIndex) => {
					...
					if (paramType === 'RestElement') {
						...
						if (paramType === 'ArrayPattern') {
							callArguments[paramIndex] = [];
						} else if (paramType === 'ObjectPattern') {
							callArguments[paramIndex] = {};
						}
					}
				...
				if (path.isArrowFunctionExpression()) {
					if (bodyPath.node.body.length === 1) {
						const firstStatementPath = bodyPath.get('body.0');
...
function removeStatementFromBlock(statementPath, blockPath) {
	...
	if (statements.length === 1) {
		...
		if (leadingComments && leadingComments.length > 0) {
			(node.innerComments || (node.innerComments = [])).push(...leadingComments);
		}
	}
