...
module.exports = function parseFunction(
	fn, js, filename, externalVarNames, isMethod, isProtoMethod, superVarName, isEval, sourceMapsEnabled
) {
		...
		Identifier(path) {
			...
			if (binding) {
				...
				if (externalVarNames.has(name)) {
					...
					// TODO If is const being assigned to, replace with code to throw error.
					// See Babel's logic for this here: https://github.com/babel/babel/pull/13248
					if (
						(parentPath.isAssignmentExpression() && path.key === 'left')
						|| parentPath.isUpdateExpression()
					) {
						assignedToVarNames.add(name);
						...
						|| (parentPath.isForXStatement() && path.key === 'left')
					) {
						assignedToVarNames.add(name);
					} else {
