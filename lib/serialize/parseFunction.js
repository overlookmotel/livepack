...
module.exports = function parseFunction(
	...
	traverse(ast, {
		...
		Function: {
			...
			exit(path) {
				if (path.node[IS_INTERNAL]) return;
	...
	return {
		...
		containsEval,
		callArguments,
		paramNames: undefined, // May be set in `processBlock()` if function is referred to in own scope
		isScopeInternalOnly: false // May be set in `processBlock()`
	};
};
