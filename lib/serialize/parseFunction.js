	...
	traverse(ast, {
		...
		Function: {
			enter(path) {
				...
				if (!isConstructor && (!computedKey || topFunctionPath)) {
					let comments;
					if (path.isMethod() && fnNode.computed) {
						comments = fnNode.key.leadingComments;
					} else if (fnNode.params.length > 0) {
						comments = fnNode.params[0].leadingComments;
					} else {
						const {directives} = fnNode.body;
						if (directives.length > 0) {
							comments = directives[0].leadingComments;
						} else {
							comments = firstStatementPath.node.leadingComments;
						}
					}
					comments.shift();
				}
