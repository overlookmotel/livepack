...
const {identifierIsVariable, replaceWith, addComments} = require('../shared/functions.js'),
	{
		...
		EVAL_VAR_NAME_BODY, ESM_VAR_NAME_BODY, EVAL_COMMENT
...
const TEMP_COMMENT_PREFIX_LEN = TEMP_COMMENT_PREFIX.length,
	...
	ESM_VAR_REGEX = new RegExp(`^${INTERNAL_VAR_NAMES_PREFIX}\\d*_${ESM_VAR_NAME_BODY}$`);
...
module.exports = function parseFunction(
	...
	isMethod, superIsProto, isEval, sourceMapsEnabled
) {
	// Prep wrapper for parsing code.
	// Either parse as function expression, or class method.
	// NB `js` for static class methods does not include 'static'
	// i.e. `class {static foo() {}}.foo.toString() === 'foo() {}'`.
	// So do not need to wrap in a class to allow static methods to be parsed.
	// eslint-disable-next-line prefer-const
	let [wrapperStart, wrapperEnd] = isMethod ? ['({', '})'] : ['(', ')'];
	...
	// Parse
	// `allowSuperOutsideMethod` option to handle arrow functions containing `super`.
	// Without this option, `() => super.foo()` cannot be parsed.
	const ast = parse(`${wrapperStart}${js}${wrapperEnd}`, {allowSuperOutsideMethod: true});
	...
	if (isMethod) {
		...
		assertBug(node.type === 'ObjectMethod', `Unexpected method node type '${node.type}'`);
	...
	traverse(ast, {
		...
		Function: {
			enter(path) {
				...
				// Throw error if this function is `import()` shim created by Livepack babel plugin.
				// Test for this here rather than with an `import` visitor, as otherwise it errors when trying
				// to remove non-existent tracker later in this function, which results in a
				...
				// Remove tracker comment
				if (fnNode.computed) fnNode.key.leadingComments.shift();
...
/**
 * Check if function is an import shim created by Livepack Babel plugin.
 * Babel plugin transpiles `import('foo')` to
 * `((specifier) => livepack_esm.recordDynamic(import(specifier), specifier))('foo')`
 * @param {Object} fnNode - Function AST node
 * @returns {boolean} - `true` if is `import()` shim
 */
function isImportShim(fnNode) {
	if (!t.isArrowFunctionExpression(fnNode)) return false;
	const paramNodes = fnNode.params;
	if (paramNodes.length !== 1) return false;
	const paramNode = paramNodes[0];
	if (!t.isIdentifier(paramNode) || paramNode.name !== 'specifier') return false;
	const bodyNode = fnNode.body;
	if (!t.isCallExpression(bodyNode)) return false;
	const calleeNode = bodyNode.callee;
	if (!t.isMemberExpression(calleeNode)) return false;
	const objectNode = calleeNode.object;
	return t.isIdentifier(objectNode) && ESM_VAR_REGEX.test(objectNode.name);
}
