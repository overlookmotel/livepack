 ...
 */
function parseFunction(fn, js, filename, externalVarNames, isMethod, isProtoMethod, superVarName) {
	...
	if (filename) {
		...
		// TODO Avoid this `Max.math()` by determining in Babel plugin if is a static method
		// and if not, just wrap in `({...})` rather than `(class{...})`
		if (lineNum === 0) charNum = Math.max(charNum - wrapperStart.length, 0);
	...
	traverse(ast, {
		...
		Function: {
			enter(path) {
				...
				if (isConstructor && (!isClass || functionDepth > 0) && isUselessConstructor(path.node)) {
					path.remove();
					return;
				}
	...
	if (filename) {
		...
		traverse(ast, {
			enter(path) {
				...
				const {start, end} = loc;
				if (start.line === 1 && start.column === 0) return;
		...
		const {sourceFiles} = this, // eslint-disable-line no-invalid-this
			{outputDir} = this.options; // eslint-disable-line no-invalid-this
