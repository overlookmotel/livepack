...
class Serializer {
	...
	serializeMain(val) {
		...
		console.dir(this.topBlock, {depth: 6});
		...
		createScopeRecord.node = this.processBlock(topBlock, parentCreateScopes);
	...
	processBlock(block, parentCreateScopes) {
		...
		for (const scope of block.scopes) {
			...
			const {values, valueNodes, record: scopeRecord} = scope;
			...
			params.forEach((paramName, index) => {
				...
				if (!isPrimitive(val)) {
					...
					records.get(val).dependents.push({val: createScopeVal, node: argumentNodes, key: index});
				}

				argumentNodes[index] = valueNodes[paramName];
			});
		}

		// Create functions
		const returnNodes = block.functions.map((functionDef, index) => {
