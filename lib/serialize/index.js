...
function conformOptions(options, filesDefault) {
	...
	// Conform `exec`, `minify`, `inline`, `mangle`, `comments`, `files`, `debug` options
	...
	options.commonChunkName = conformChunkName('commonChunkName', 'common.[hash]', true);
...
function conformChunkNameOption(options, name, defaultValue, hashRequired) {
	const value = options[name];
	...
	assert(placeholders.length > 0, `${opt} must include either '[name]' or '[hash]' placeholder`);
	assert(!hashRequired || placeholders.includes('hash'), `${opt} must contain '[hash]' placeholder`);
