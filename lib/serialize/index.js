...
class Serializer {
	...
	processBlock(block, parentCreateScopes) {
		...
		for (const scope of blockScopes) {
			...
			const scopeVarNode = t.identifier(`scope${scopeId}`); // TODO Change name to 'x'
			const scopeRecord = this.createRecord(scope, scopeVarNode);
...
module.exports = function serialize(val, options) {
	...
	for (const optName in defaults) {
		...
		if (optValue == null) {
			options[optName] = defaults[optName];
		} else {
	...
	if (format == null) {
		...
		const formats = ['js', 'cjs', 'esm'],
			aliases = {commonjs: 'cjs', mjs: 'esm'};
		const alias = aliases[format];
		if (alias) {
			options.format = alias;
		} else {
			assert(
				formats.includes(format),
				`options.format must be one of ${(formats.concat(Object.keys(aliases))).map(f => `'${f}'`).join(', ')} if provided`
			);
		}
	}
