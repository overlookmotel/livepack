	...
	processBlock(block) {
		...
		for (const functionRecord of block.functions) {
			...
			for (const fn of functionRecord.values) {
				const valRecord = records.get(fn);
				const scopeRecord = valRecord.scope.record;
				const accessNode = t.memberExpression(
					scopeRecord.varNode,
					t.numericLiteral(returnNodes.length),
					true
				);
				valRecord.node = accessNode;
		...
		const blockFnNode = t.functionExpression(
			t.identifier(blockRecord.varNode.name), // TODO Change to `null`
