...
class Serializer {
	...
	serializeMain(val) {
		...
		function addDependencies(child) {
			...
			if (!assignments && dependents.length === 1 && false) {
				// Only used once - substitute content for var identifier
				// TODO Doesn't work in all cases - fix. Currently disabled.
		...
		const node = (statementNodes.length > 0 ? last(varNodes).id : varNodes.pop().init);
