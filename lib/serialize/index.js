...
class Serializer {
	...
	serialize(val) {
		// Serialize value
		const node = this.serializeValue(val);

		let statementNodes;
		if (isPrimitive(val)) {
			statementNodes = [];
		} else {
			statementNodes = this.serializeMain(val);
		}
	...
	serializeMain(val) {
		...
		const varNodes = [],
			statementNodes = [],
			processed = new Set();
		function processDependencies(child) {
			if (processed.has(child)) return;
			processed.add(child);

			const record = records.get(child);
			for (const dependency of record.dependencies) {
				processDependencies(dependency.val);
			}
