...
module.exports = {
	...
	serialize(entryPoints) {
		...
		} = this.serializeEntryPointsAndSplitPoints(entryPoints, outputNames, getNameIfUnique);
	...
	splitOutputs(pointOutputs, entryPointOutputs, entryAndAsyncSplitPointOutputs, getNameIfUnique) {
		...
		function determineOutput(record) {
			...
			// If is global, create separate record for this output
			if (record.isGlobal) return duplicateGlobal(record);
		...
		// This is an attempt to solve https://github.com/overlookmotel/livepack/issues/149
		// But it doesn't work! See TODO comment below.
		function duplicateGlobal(record) {
			let firstOutput;
			const localRecords = new Map(); // Keyed by output

			// Iterate through dependents.
			// If dependents are in more than one output, leave first output's dependents depending
			// on original record, but create new duplicate local records for other outputs
			// and re-reference dependents in those outputs to depend on the local copy.
			record.dependents = record.dependents.filter((dependent) => {
				// Get dependent's output (`.record.record` is for assignments)
				const dependentRecord = dependent.record.record || dependent.record,
					dependentOutput = determineOutput(dependentRecord);

				// First output uses original record
				if (!firstOutput) {
					firstOutput = dependentOutput;
					return true;
				}
				if (dependentOutput === firstOutput) return true;

				// Different output - create duplicate record for it
				let localRecord = localRecords.get(dependentOutput);
				if (!localRecord) {
					const localVarNode = t.identifier(record.varNode.name),
						localNode = t.cloneDeep(record.node);
					localRecord = {
						...record,
						varNode: localVarNode,
						node: localNode,
						output: dependentOutput,
						dependents: []
					};
					localRecords.set(dependentOutput, localRecord);

					// Re-reference dependencies
					localRecord.dependencies = record.dependencies.map(({record: dependencyRecord}) => {
						dependencyRecord.dependents.push(
							...dependencyRecord.dependents.flatMap((dependencyDependent) => {
								if (dependencyDependent.record !== record) return [];
								// TODO Doesn't work.
								// `.node` here maybe should not refer to `localNode` itself but a deeper property of it.
								// There's no easy way to determine the correct `node`.
								return {
									record: localRecord,
									node: localNode,
									key: dependencyDependent.key
								};
							})
						);

						for (const dependencyDependent of dependencyRecord.dependents) {
							if (dependencyDependent.record !== record) continue;
							dependencyDependent.record = localRecord;
							dependencyDependent.node[dependencyDependent.key] = localVarNode;
						}

						return {record: dependencyRecord};
					});

					// Re-reference dependencies' dependents to point to local record
					for (const dependency of record.dependencies) {
						for (const dependencyDependent of dependency.record.dependents) {
							if (dependencyDependent.record !== record) continue;
							dependencyDependent.record = localRecord;
							dependencyDependent.node[dependencyDependent.key] = localRecord.varNode;
						}
					}
				}

				// Update dependent to reference to local record
				dependent.record = localRecord;
				dependent.node[dependent.key] = localRecord.varNode;
				localRecord.dependents.push(dependent);

				for (const dependency of dependentRecord.dependencies) {
					if (dependency.record === record) {
						dependency.record = localRecord;
						break;
					}
				}
...
function createOutput(id, type, index, name, record, requestedName) {
	return {
		...
		globals: new Map(), // Keys = records for global across all outputs, values = records for this output
		filename: undefined,
