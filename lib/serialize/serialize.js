...
const {splitPoints} = require('../internal.js'),
	...
	{
		ENTRY_POINT, SYNC_SPLIT_POINT, ASYNC_SPLIT_POINT, SHARED_POINT, SHARED_SPLIT_POINT,
		SPLIT_POINT_MASK, SHARED_MASK
	} = require('./constants'),
...
module.exports = {
	...
	serialize(entryPoints) {
		...
		// If only one entry point and no split points, skip splitting into outputs - fast path
		if (pointOutputs.length === 1) {
			const output = pointOutputs[0];
			return this.outputOne(output.record, output.name, true);
		}
		...
		// Output files.
		return this.output(outputs, entryPointOutputs);
	},
	...
	serializeEntryPointsAndSplitPoints(entryPoints, outputNames, getNameIfUnique) {
		...
		function createEntryOutput(type, name, record, requestedName) {
			...
			// Shared outputs get their IDs from a bitwise OR of their constituent outputs.
	...
	splitOutputs(pointOutputs, entryPointOutputs, getNameIfUnique) {
		...
		function determineOutput(record) {
			...
			const inProcessOutput = createOutput(0n, SHARED_POINT, undefined);
			...
			visitDependents(record, (dependentRecord) => {
				...
				if (dependentOutput.type & SPLIT_POINT_MASK) type = SHARED_SPLIT_POINT;
			});
			...
			if (isAwaited) {
				inProcessOutput.awaiting = awaiting;
				return reviseOutput(record, output);
			}
		...
		function reviseOutput(record, output) {
			// If this record already has output determined, skip it
			if (!record.output.awaiting) return record.output;
			...
			let isJoinPoint = false;
			visitDependents(record, (dependentRecord) => {
				const dependentOutput = reviseOutput(dependentRecord, output);
				if (dependentOutput.id !== id && dependentOutput.type !== SYNC_SPLIT_POINT) isJoinPoint = true;
			});
		...
		// Ensure all entry + split points have valid outputs, and rename shared outputs
		// which export only a entry/split point's record to point's name
		const {options} = this,
			entryPointsCanBeShared = !options.exec && options.format !== 'js';
				...
				if (
					...
					&& (
						pointType === ENTRY_POINT
							? output.type === SHARED_POINT && entryPointsCanBeShared
							: output.type & SHARED_MASK // eslint-disable-line no-bitwise
					)
				) {
					// A shared output exports only the point's record - make it the output for this point
					...
					// Output will reference the shared output via proxy.
