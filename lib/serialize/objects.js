...
module.exports = {
	...
	wrapWithProperties(val, record, inputNode, shouldSkipKey) {
		// Serialize properties
		const node = this.serializeProperties(val, record, shouldSkipKey);
		...
		// Properties with descriptors - `Object.defineProperties(input, { ... })`
		return t.callExpression(
			t.memberExpression(t.identifier('Object'), t.identifier('defineProperties')),
			[inputNode, node.arguments[1]]
		);
	},

	serializeProperties(obj, record, shouldSkipKey) {
		...
		for (const key of Object.getOwnPropertyNames(obj)) {
			...
			this.serializeProperty(obj, key, null, keyNode, keyIsComputed, props, assignments, state);
		}
		...
		Object.getOwnPropertySymbols(obj).forEach((key, index) => {
			...
			this.serializeProperty(obj, key, keyRecord, keyRecord.varNode, true, props, assignments, state);
		});
		if (state.numTrailingCircular > 0) props.length -= state.numTrailingCircular;
	...
	serializeProperty(obj, key, keyRecord, keyNode, keyIsComputed, props, assignments, state) {
