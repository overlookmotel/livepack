...
module.exports = {
	...
	wrapWithProperties(val, record, inputNode, shouldSkipKey, getDefaultDescriptor) {
		return this.serializeProperties(val, record, null, inputNode, shouldSkipKey, getDefaultDescriptor);
	},
	...
	serializeProperty(
		...
		let isCircular = false,
			needsDescriptor = defaultModifiers ? !defaultModifiers.writable : false;
		if (hasValue) {
		...
		if (isCircular) {
			...
			if (!defaultModifiers) {
				props.push({keyRecord, keyNode, keyIsComputed, records: {}, modifiers: {}});
