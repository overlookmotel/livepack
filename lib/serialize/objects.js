...
const {createDependency, createAssignment} = require('./records.js'),
	{createKeyNode, recordIsCircular, isNumberKey} = require('./utils.js');
	...
	serializeProperties(
		...
		function deleteDefaultProps(nextKey) {
			while (nextDefaultProp && nextDefaultProp.name !== nextKey) {
				...
				const memberNode = t.memberExpression(record.varNode, t.identifier(nextDefaultProp.name));
				const assignmentNode = t.unaryExpression('delete', memberNode);
				createAssignment(record, assignmentNode, memberNode, 'object', true);

				deletedPropNames.add(nextDefaultProp.name);
		...
		for (const key of Object.getOwnPropertyNames(val)) {
			...
			let defaultProp;
			if (isNumberKey(key)) {
				if (nextDefaultProp && nextDefaultProp.name === key) defaultProp = nextDefaultProp;
			} else {
