...
function traceObject(obj, record) {
	const s = this; // eslint-disable-line no-invalid-this
	const descriptors = Object.getOwnPropertyDescriptors(obj);
	const props = Object.entries(descriptors).map(([keyStr, descriptor]) => {
		const key = isIntegerKey(keyStr) ? keyStr * 1 : keyStr;
		return traceObjectProperty(descriptor, record, key, undefined, keyStr, `.${keyStr}`, s);
	});
	...
	Object.getOwnPropertySymbols(descriptors).forEach((key, index) => {
		...
		const keyRecord = s.traceValue(key, keyName, `[SymbolKeys ${index}]`);
		createDependency(record, keyRecord);

		props.push(
			traceObjectProperty(
				descriptors[key], record, undefined, keyRecord, keyName, `[${key.toString()}]`, s
			)
		);
	});
...
function traceObjectProperty(descriptor, objRecord, key, keyRecord, keyName, keyDescription, s) {
	let valRecord, getRecord, setRecord, writable;
	if ('value' in descriptor) {
		valRecord = s.traceValue(descriptor.value, keyName, keyDescription);
		createDependency(objRecord, valRecord);
		writable = descriptor.writable;
	} else {
		const {get, set} = descriptor;
		if (get !== undefined) {
			getRecord = s.traceValue(get, `${keyName}Getter`, `${keyDescription}[getter]`);
			createDependency(objRecord, getRecord);
		}
		if (set !== undefined) {
			getRecord = s.traceValue(set, `${keyName}Setter`, `${keyDescription}[setter]`);
			createDependency(objRecord, setRecord);
		}
	}
...
function serializeObject(record, s) {
	// TODO Deal with getters and setters
	return t.objectExpression(
		record.props.map((prop) => {
			const {keyRecord} = prop,
				keyNode = keyRecord ? s.serializeValue(keyRecord) : createKeyNode(prop.key);
			return t.objectProperty(keyNode, s.serializeValue(prop.valRecord), !!keyRecord, true);
		})
	);
}
