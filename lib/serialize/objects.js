...
module.exports = {
	...
	serializeNullObject(obj, record) {
		return this.serializeProperties(obj, record, t.nullLiteral(), null);
	},

	wrapWithProperties(val, record, inputNode, shouldSkipKey, shouldForceDescriptor) {
		return this.serializeProperties(val, record, null, inputNode, shouldSkipKey, shouldForceDescriptor);
	},

	serializeProperties(val, record, protoNode, inputNode, shouldSkipKey, shouldForceDescriptor) {
		...
		if (props.length === 0) {
			...
			if (inputNode) {
				...
				node = t.callExpression(objectCreateRecord.varNode, [protoNode]);
			...
			if (protoNode) {
				const objectCreateRecord = this.serializeValue(Object.create);
				inputNode = t.callExpression(objectCreateRecord.varNode, [protoNode]);
	...
	serializeProperty(
		...
		const varName = keyRecord ? keyRecord.varNode.name : key;
