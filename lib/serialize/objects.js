...
// Imports
const {createDependency, deleteDependency, createAssignment} = require('./records.js'),
	{recordIsCircular, isIntegerKey, isNumberKey, isJsIdentifier} = require('./utils.js'),
...
module.exports = {
	...
	serializeProperties(record, node, forceAssignment) {
		...
		for (const prop of record.props) {
			...
			if (valRecord) {
				if (!prop.valNode) prop.valNode = this.serializeValue(valRecord);
