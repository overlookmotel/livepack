...
module.exports = {
	...
	serializeProperties(val, record, protoRecord, inputNode, shouldSkipKey, shouldForceDescriptor) {
...
function createDescriptorNodes(props, record) {
	return props.map((prop) => {
		...
		let valueDefined = !!records.value;
		...
		for (const modifierName of [...(valueDefined ? ['writable'] : []), 'enumerable', 'configurable']) {
			if (!descriptor[modifierName]) continue;
			descriptorPropNodes.push(t.objectProperty(t.identifier(modifierName), t.booleanLiteral(true)));
		}
