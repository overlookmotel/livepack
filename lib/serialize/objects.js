...
module.exports = {
	...
	serializeProperties(
		...
		if (props.length === 0) {
			...
			node = t.callExpression(methodRecord.varNode, [targetNode, createDescriptorsNode(props, record)]);
		...
		if (assignments.length > 0) {
			if (!state.assignmentsNeedDescriptors) {
				...
				assignmentNode.arguments[1] = createDescriptorsNode(assignments, assignment);
			}
		}
 ...
 */
function createDescriptorsNode(props, record) {
