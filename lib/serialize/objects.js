...
// Imports
const {createDependency, createSelfDependency, deleteDependency} = require('./records.js'),
...
const objectProtoSetters = new Set(['__proto__']);
...
module.exports = {
	 ...
	 *   - deleted keys as `record.deletedKeys` (Map)
	 *
	...
	traceProperties(val, record, defaultProto, defaultProps, shouldSkipKey) {
		// Get extensibility
		const extensibility = record.extensibility = Object.isFrozen(val) ? FROZEN
		...
		if (proto !== defaultProto) {
			const protoRecord = this.traceValue(proto, `${varName}Prototype`, '[prototype]');
			record.protoRecord = protoRecord;
		...
		const deletedKeys = record.deletedKeys = new Map();
	 ...
	 * @param {Object} [record.protoRecord] - Record for prototype if it's not default
	 * @param {number} record.extensibility - Extensibility (e.g. `FROZEN`)
	 * @param {Map} record.deletedKeys - Map of deleted keys (key mapped to keyRecord)
	 * @returns {Object} - AST node
	 */
	serializeObject(record) {
		return this.serializeProperties(record, record.props, undefined, objectProtoSetters, false);
	},
	 ...
	 * @param {number} record.extensibility - Extensibility (e.g. `FROZEN`)
	 * @param {Map} record.deletedKeys - Map of deleted keys (key mapped to keyRecord)
	 * @param {Set<string|symbol>} defaultProtoSetters - Setters on default prototype
	 * @param {Array} [props=record.props] - Array of properties
	 * @returns {Object} - AST node wrapped
	 */
	wrapWithProperties(node, record, defaultProtoSetters, props) {
		return this.serializeProperties(record, props || record.props, node, defaultProtoSetters, false);
	},
	 ...
	 * @param {Set<string|symbol>} [defaultProtoSetters] - Setters on default prototype.
	 *   If not provided, will assume '__proto__' is only setter.
	 * @param {boolean} forceAssignment - `true` to force late assignment
	 * @returns {Object} - AST node
	 */
	serializeProperties(record, props, node, defaultProtoSetters, forceAssignment) {
		// Serialize prototype
		...
		if (protoRecord) {
			...
			if (protoIsCircular) {
				...
				// In this case, use default setters provided (which are the setters on default proto)
				// instead of consulting `prop.requiresDefine`.
				// Then set prototype last, after all properties are added.
				protoSetters = defaultProtoSetters || objectProtoSetters;
			} else if (node) {
		...
		for (const prop of props) {
			...
			let requiresDefine = protoSetters
				? prop.requiresDefine = protoSetters.has(key)
				: prop.requiresDefine;
		...
		if (!mainPropsRequireDefine) {
			...
			const objNode = t.objectExpression(mainProps.map(
				prop => createObjectPropNode(prop.keyNode, prop.keyRecord, prop.valNode, prop.valRecord)
			));
		...
		if (protoNode) {
			...
			assignmentNode = this.createObjectMethodCallNode(
				'setPrototypeOf', [assignmentNode, protoNode], record
			);
		}

		// Add extensibility wrapper
		const {extensibility} = record;
...
function createStringObjectPropNode(key, valNode, valRecord) {
	return createObjectPropNode(t.identifier(key), undefined, valNode, valRecord);
}
