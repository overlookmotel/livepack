...
module.exports = {
	traceObject(obj, record) {
		...
		const props = Object.entries(descriptors).map(([keyStr, descriptor]) => {
			...
			return this.traceObjectProperty(descriptor, record, key, undefined, keyStr, `.${keyStr}`);
		});
		...
		Object.getOwnPropertySymbols(descriptors).forEach((key, index) => {
			...
			const keyRecord = this.traceValue(key, keyName, `[SymbolKeys ${index}]`);
			createDependency(record, keyRecord);

			props.push(
				this.traceObjectProperty(
					descriptors[key], record, undefined, keyRecord, keyName, `[${key.toString()}]`
				)
			);
		});
	...
	traceObjectProperty(descriptor, objRecord, key, keyRecord, keyName, keyDescription) {
		let valRecord, getRecord, setRecord, writable;
		if ('value' in descriptor) {
			valRecord = this.traceValue(descriptor.value, keyName, keyDescription);
		...
		return t.objectExpression(
			record.props.map((prop) => {
				const {keyRecord} = prop,
					keyNode = keyRecord ? this.serializeValue(keyRecord) : createKeyNode(prop.key);
				return t.objectProperty(keyNode, this.serializeValue(prop.valRecord), !!keyRecord, true);
			})
		);
	}
};
