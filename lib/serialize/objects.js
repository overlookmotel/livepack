...
module.exports = {
	...
	serializeProperties(record, node, forceAssignment) {
		// TODO Use global vars rather than literals for `Object.seal()`, `void 0` etc
		...
		if (protoRecord) {
			...
			if (protoIsCircular) {
				...
				node = createObjectMethodCallNode('setPrototypeOf', [node, protoNode]);
				protoNode = undefined;
			}
		}
		...
		// console.log('nonCircularProps:', mainProps);
		...
		if (!mainPropsRequireDefine) {
			...
			// TODO unwrap `x: {x() {}}.x` to `x() {}`
			const objNode = t.objectExpression(mainProps.map(prop => (
				t.objectProperty(prop.keyNode, prop.valNode, !!prop.keyRecord, true)
			)));

			if (!node && protoNode && !protoIsCircular) {
				node = createObjectMethodCallNode('create', [protoNode]);
				protoNode = undefined;
			}
			...
			if (!node) {
				...
				node = createObjectMethodCallNode('assign', [node, objNode]);
			}
			...
			if (mainProps[0].key === '__proto__') {
				node = createDefinitionNode(
					mainProps,
					createObjectMethodCallNode('create', [protoNode]),
					'defineProperties'
				);
			} else {
				node = createDefinitionNode(mainProps, protoNode, 'create');
			}
			protoNode = undefined;
		} else {
			// No prototype
			if (!node) node = t.objectExpression([]);
			node = createDefinitionNode(mainProps, node, 'defineProperties');
		}
		...
		if (!circularPropsRequireDefine) {
			...
			assignmentNode = createDefinitionNode(circularProps, varNode, 'defineProperties');
		}
		...
		if (protoNode) {
			...
			assignmentNode = createObjectMethodCallNode(
				'setPrototypeOf', [assignmentNode, this.serializeValue(protoRecord)]
			);
		}
		...
		if (extensibility !== EXTENSIBLE) {
			...
			const wrappedNode = createObjectMethodCallNode(
				EXTENSIBLE_METHODS[extensibility], [assignmentNode || node]
			);
			if (assignmentNode) {
...
function createObjectMethodCallNode(methodName, argNodes) {
	return t.callExpression(
		t.memberExpression(t.identifier('Object'), t.identifier(methodName)), argNodes
	);
}

function createDefinitionNode(props, node, methodName) {
	const propNodes = [],
		postProtoPropNodes = [];
	let protoObjNode;
	for (const prop of props) {
		const attrNodes = [];
		// TODO unwrap `get: {get() {}}.get` to `get() {}`
		if (prop.valNode) attrNodes.push(createObjectPropNode('value', prop.valNode));
		if (prop.getNode) attrNodes.push(createObjectPropNode('get', prop.getNode));
		if (prop.setNode) attrNodes.push(createObjectPropNode('set', prop.setNode));

		for (const modifier of MODIFIER_NAMES) {
			if (prop[modifier] !== undefined) {
				attrNodes.push(createObjectPropNode(modifier, t.booleanLiteral(prop[modifier])));
			}
		}
	...
	if (propNodes.length > 0) {
		node = createObjectMethodCallNode(methodName, [node, t.objectExpression(propNodes)]);
	}

	if (protoObjNode) {
		node = createObjectMethodCallNode(
			'defineProperty', [node, t.stringLiteral('__proto__'), protoObjNode]
		);

		if (postProtoPropNodes.length > 0) {
			node = createObjectMethodCallNode(
				'defineProperties', [node, t.objectExpression(postProtoPropNodes)]
			);
		}
	}
...
function createObjectPropNode(key, valNode) {
	return t.objectProperty(t.identifier(key), valNode, false, true);
}
