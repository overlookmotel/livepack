...
const {serializePrimitive} = require('./primitives.js'),
	...
	{
		GLOBAL, MODULE, VALUE, GETTER, SETTER, PROTO, EVAL, COMMON_JS, ESM_MODULE, IMPORT_META
...
module.exports = {
	...
	serializeGlobal(val, {type, parent, key}) {
		...
		} else if (type === ESM_MODULE) {
			node = this.serializeEsmModule(val, key, record);
		} else if (type === IMPORT_META) {
			node = this.serializeImportMeta(parent, key);
	...
	/**
	 * Create replica ESM module namespace object using `createModule()` runtime function.
	 * @param {Object} val - Module namespace object
	 * @param {string} url - URL of module
	 * @param {Object} record - Record
	 * @returns {Object} - AST node
	 */
	serializeEsmModule(val, url, record) {
		// Name var with filename
		record.varNode.name = pathParse(fileURLToPath(url)).name;

		// Record all exports as imported
		for (const targetName in val) {
			this.recordImportBinding(url, targetName);
		}

		// Output as call to `createModule()` runtime function
		const createModuleRecord = this.serializeRuntime('createModule');
		const node = t.callExpression(createModuleRecord.varNode, [this.serializeObject({...val}, record)]);
		createDependency(record, createModuleRecord, node, 'callee');
		return node;
	},

	serializeImportMeta(url, key) {
		throw new Error(
			`Cannot serialize \`import.meta${key === 'resolve' ? '.resolve' : ''}\` (in ${url})`
		);
	},
