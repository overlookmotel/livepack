...
module.exports = {
	...
	serializeGlobal(val, {type, parent, key}) {
		...
		if (type === VALUE) {
			...
			// `exampleURL` will be in common file so this may as well be too
			isGlobal = false;
			...
			dependentKey = 'callee';
			// `getCallSite()` runtime function will be in common file so this may as well be too
			isGlobal = false;
		} else {
			assertBug(
		...
		if (isGlobal) record.isGlobal = true;
		return record;
	},
