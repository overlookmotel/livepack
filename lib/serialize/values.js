...
module.exports = {
	...
	serializeThing(val, record) {
		...
		// Serialize
		assertBug(typeof val === 'object', 'Unexpected value type');

		const type = typeOf(val);
		if (type === 'Object') {
			// `URL` and `URLSearchParams` are implemented in Javascript in Node internals
			// and so cannot be detected with a type check
			// TODO: This remains the case on Node v18, but on NodeJS v20 can better identify
			// `URL` and `URLSearchParams` by calling their prototype methods and seeing if they error.
	...
	serializeGlobal(val, {type, parent, key}) {
		...
		// TODO: Could convert this massive `if ... else` to an array lookup
		if (type === VALUE) {
