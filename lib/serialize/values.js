...
const typeOf = require('native-type-of'),
	{isFunction, isSymbol, isString} = require('is-it-type'),
...
const {serializePrimitive} = require('./primitives.js'),
	...
	{isPrimitive} = require('../shared/functions.js'),
	{
		...
		TypedArrayPrototype, URLSymbols, URLContextSymbol, URLContext, CallSite
	} = require('../shared/globals.js'),
	{createKeyNode, isNumberKey} = require('./utils.js'),
	{globals, commonJsVars} = require('../internal.js'),
	{getCallSite} = require('./external.js');
...
module.exports = {
	...
	serializeGlobal(val, {parent, key, isModule}) {
		const record = createRecord(isString(key) ? key : 'x', val),
			{varNode} = record;
		...
		if (parent) {
			if (typeof key === 'string') {
				...
				const {type} = key;
				key = key.key;
				const parentRecord = this.serializeValue(parent, null, `<parent global ${type} ${key}>`);
				...
				node = t.memberExpression(
					...
					t.identifier(type === 'getter' ? 'get' : 'set')
				);
				...
				varNode.name = `${parentVarNode.name}${upperFirst(key)}${upperFirst(type)}`;
			}
		} else if (key) {
		...
		} else if (val === CallSite) {
			varNode.name = 'CallSite';
			dependentRecord = this.serializeValue(getCallSite, 'getCallSite', '<getCallSite>');
