...
const {serializePrimitive} = require('./primitives.js'),
	...
	{globals, commonJsVars, urlVars} = internal,
	{URLSymbols, URLContextSymbol, URLContext} = urlVars;

const argumentProxy = require('./argumentProxy.js');
...
module.exports = {
	...
	serializeGlobal(val, {parent, key, isModule}) {
		...
		} else if (val === regexSourceGetter || val === regexFlagsGetter) {
			// `Object.getOwnPropertyDescriptors(RegExp.prototype).source.get` or
			// `Object.getOwnPropertyDescriptors(RegExp.prototype).flags.get`
			const isSourceGetter = val === regexSourceGetter;
			varNode.name = isSourceGetter ? 'regexSourceGetter' : 'regexFlagsGetter';
			const getOwnPropsRecord = this.serializeValue(Object.getOwnPropertyDescriptors);
			const RegExpProtoRecord = this.serializeValue(RegExp.prototype);
			node = t.memberExpression(
				t.memberExpression(
					t.callExpression(
						getOwnPropsRecord.varNode,
						[RegExpProtoRecord.varNode]
					),
					t.identifier(isSourceGetter ? 'source' : 'flags')
				),
				t.identifier('get')
			);
			createDependency(record, getOwnPropsRecord, node.object.object, 'callee');
			createDependency(record, RegExpProtoRecord, node.object.object.arguments, 0);
		} else {
