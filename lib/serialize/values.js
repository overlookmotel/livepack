	...
	serializeGlobal(val, {parent, key}) {
		if (!parent) {
			...
			if (key) {
				...
				const infinityRecord = this.serializeValue(Infinity);
				const record = createRecord('minusInfinity');
				node = t.unaryExpression('-', infinityRecord.varNode);
				record.node = node;
				createDependency(record, infinityRecord, node, 'argument');
				return record;
			} else {
