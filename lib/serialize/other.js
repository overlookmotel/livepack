...
const {createDependency} = require('./records.js'),
	...
	{firstMapKey, getClassProtoSetters} = require('./utils.js');
...
const regexSourceGetter = Object.getOwnPropertyDescriptors(RegExp.prototype).source.get,
	...
	regexpProtoSetters = getClassProtoSetters(RegExp),
	dateProtoSetters = getClassProtoSetters(Date),
	urlProtoSetters = getClassProtoSetters(URL),
	urlSearchParamsProtoSetters = getClassProtoSetters(URLSearchParams);
...
module.exports = {
	...
	traceRegexp(regexp, record) {
		this.traceProperties(
			regexp, record, RegExp.prototype,
			[{
				key: 'lastIndex',
				val: 0,
				get: undefined,
	...
	serializeURL(record) {
		...
		return this.wrapWithProperties(node, record, urlProtoSetters);
	},
	...
	serializeURLSearchParams(record) {
		...
		return this.wrapWithProperties(node, record, urlSearchParamsProtoSetters);
	},
