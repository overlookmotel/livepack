...
module.exports = {
	...
	serializeValue(record) {
		// If already processing, return var node
		const {processingRecords} = this;
		if (processingRecords.has(record)) return record.varNode;

		// If already processed, return var node
		if (record.varNode) return record.varNode;

		// Flag as currently processing
		processingRecords.add(record);

		// Create var node
		const varNode = t.identifier(this.transformVarName(record.name));
		...
		if (t.isImportDeclaration(node)) {
			...
			if (dependents.size > 1 || dependents.values().next().value > 1) {
				// Wrap unnamed functions in `(0, fn)` to prevent them getting implicitly named.
				if (record.serializer === this.serializeFunction && !node.id) {
		...
		processingRecords.delete(record);
