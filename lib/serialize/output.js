...
const {createRecord, createDependency} = require('./records.js'),
	...
	{
		DEFAULT_SPLIT_CHUNK_NAME, DEFAULT_COMMON_CHUNK_NAME,
		ENTRY_POINT, ASYNC_SPLIT_POINT, ENTRY_POINT_MASK, SPLIT_POINT_MASK
...
module.exports = {
	...
	output(outputs, entryPointOutputs) {
		...
		for (let i = outputs.length - 1; i >= 0; i--) {
			...
			for (const [dependentOutput, {importRecord}] of importsMap) {
				const relativePath = getRequirePath(this.getOutputFilename(dependentOutput), filename);
			...
			if (output.type === ASYNC_SPLIT_POINT) {
				for (const importFnRecord of output.importFnRecords) {
					const relativePath = getRequirePath(this.getOutputFilename(importFnRecord.output), filename);
	...
	outputOne(output, record) {
		...
		// Return file objects
		const files = [{type: getOutputTypeName(output), name: output.name || null, filename, content: js}];
	 ...
	 * If no file content provided, uses dummy hash '--------'.
	 * @param {Object} output - Output object
	 * @param {string} [content] - File content (optional, if not provided will create dummy hash)
	 * @returns {string} - Filename
	 */
	getOutputFilename(output, content) {
		if (output.filename) return output.filename;

		const typeName = getOutputTypeName(output),
			templateOpt = `${typeName}ChunkName`,
			template = this.options[templateOpt];

		let outName = template;
		const templateIncludesHash = template.includes('[hash]');
		if (templateIncludesHash) {
			const hash = content !== undefined ? hashFileContents(content, this.hashCounters) : '--------';
			outName = outName.replace(/\[hash\]/g, hash);
		}
		if (template.includes('[name]')) {
			let {name} = output;
			if (!name) {
				assert(templateIncludesHash, `options.${templateOpt} must include '[hash]' if unnamed chunks`);
				name = typeName === 'split' ? DEFAULT_SPLIT_CHUNK_NAME : DEFAULT_COMMON_CHUNK_NAME;
			}
			outName = outName.replace(/\[name\]/g, name);
		}

		return `${outName}.${this.options.ext}`;
	},
 ...
 */
function hashFileContents(content, hashCounters) {
