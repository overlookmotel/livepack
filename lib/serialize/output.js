...
module.exports = {
	...
	output(entryPointOutputs) {
		...
		entryPointOutputs.forEach((output, index) => {
			const node = this.serializeValue(output.record);
			...
			outputFiles.forEach((file, fileIndex) => {
				files[index * numFilesPerOutput + fileIndex] = file;
			});
		});
