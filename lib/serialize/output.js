...
module.exports = {
	...
	outputAst(record, output, format, strictEnv) {
		...
		if (format === 'exec') {
			...
			exportRecord.serializer = () => t.exportDefaultDeclaration(
				withFunctionWrapper(this.serializeValue(record), record)
			);
		}
	...
	serializeValue(record) {
		...
		if (t.isImportDeclaration(node)) {
			...
			// If used more than once or global, assign to var
			const {type} = record;
			if (type === GLOBAL_TYPE || type === UNDEFINED_TYPE) {
