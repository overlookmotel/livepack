...
const {createRecord, createDependency} = require('./records.js'),
	...
	{COMMON_FILENAME_PREFIX, ENTRY_POINT, ASYNC_SPLIT_POINT, ENTRY_POINT_MASK} = require('./constants.js');
...
module.exports = {
	...
	outputOne(record, name, isEntry) {
		...
		// Get filename
		if (!name) name = createCommonChunkName(js, this.hashCounters);
 ...
 */
function createCommonChunkName(content, hashCounters) {
	const hash = getHash(content, hashCounters);
	return `${COMMON_FILENAME_PREFIX}${hash}`;
}
