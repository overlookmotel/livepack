...
// Modules
const pathRelative = require('path').posix.relative,
...
const {toJsIdentifier} = require('./utils.js'),
	{DEFAULT_OUTPUT_FILENAME, JS_FILE_EXT, MAP_FILE_EXT} = require('./constants.js');
...
module.exports = {
	...
	output(record) {
		...
		// Get filename
		const filename = `${DEFAULT_OUTPUT_FILENAME}.${JS_FILE_EXT}`;
		...
		if (sourceMaps) {
			...
			if (outputDir) map.sources = map.sources.map(path => getRelativePathFromDir(outputDir, path));

			// Add source map inline or in external file
