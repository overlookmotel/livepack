...
const {createRecord, createDependency} = require('./records.js'),
	...
	{PRIMITIVE_TYPE, GLOBAL_TYPE, UNDEFINED_TYPE, FUNCTION_TYPE} = require('./types.js');
...
module.exports = {
	...
	outputMain(record, strictEnv, modifyStrictRecords) { // eslint-disable-line no-unused-vars
		...
		if (globalVars.length > 0) {
			...
			for (const {record: globalRecord, node: globalNode} of globalVars) {
				if (this.recordNeedsVar(globalRecord)) {
					globalVarNodes.push(t.variableDeclarator(globalRecord.varNode, globalNode));
				} else {
					replaceNode(globalRecord.varNode, globalNode);
				}
			}
			if (globalVarNodes.length > 0) varNodes.unshift(...globalVarNodes);
		}
	...
	serializeValue(record) {
		...
		// Create var node
		// TODO Var node should only be created if needs var
		...
		if (t.isImportDeclaration(node)) {
			...
			const {type} = record,
				isGlobal = type === GLOBAL_TYPE || type === UNDEFINED_TYPE;
			if (isGlobal || this.recordNeedsVar(record)) {
	...
	recordNeedsVar(record) {
		if (!this.options.inline && record.type !== PRIMITIVE_TYPE) return true;
		const {dependents} = record;
		return dependents.size > 1 || firstMapValue(dependents) > 1;
	},
		...
		const addDependencies = (valRecord) => {
			...
			if (modifyStrictRecords.has(valRecord)) {
				if (strictEnv) {
					...
					const {js} = this.generateJs(t.expressionStatement(node), false);
