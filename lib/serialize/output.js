...
module.exports = {
	...
	outputAst(record, output, format, strictEnv) {
		...
		if (format === 'exec') {
			...
			// TODO Wrap unnamed function in `(0, ...)` to prevent it getting named
	...
	serializeValue(record) {
		...
		if (t.isImportDeclaration(node)) {
			...
			if (isGlobal || this.recordNeedsVar(record)) {
				...
				// TODO This probably isn't necessary for globals
				if (
					type === FUNCTION_TYPE
				...
				(isGlobal ? this.globalVars : this.localVars).push({record, node});
				node = varNode;
			}
		}
