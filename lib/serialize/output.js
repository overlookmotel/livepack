...
const {createRecord, createDependency} = require('./records.js'),
	...
	{
		JS_FILE_EXT, MAP_FILE_EXT, SHARED_FILENAME_PREFIX,
		ENTRY_POINT, ASYNC_SPLIT_POINT, ENTRY_POINT_MASK
...
module.exports = {
	/**
	 * Init hash counters map.
	 * @returns {undefined}
	 */
	initOutputs() {
		this.hashCounters = Object.create(null);
	},
	...
	output(outputs, entryPointOutputs) {
		...
		for (let i = outputs.length - 1; i >= 0; i--) {
			...
			exports.forEach((record, exportIndex) => {
				...
				for (let dependentIndex = 0; dependentIndex < dependents.length; dependentIndex++) {
					...
					if (!dependentRecord.node) {
						// Dependent is proxy - set output's export to import from shared file
			...
			// Output
			const files = this.outputOne(combinedRecord, outputName, output.type === ENTRY_POINT);
			output.files = files;
			const {filename} = files[0];

			// Create import/require expressions
			for (const [dependentOutput, {importRecord}] of importsMap) {
				const relativePath = getRequirePath(dependentOutput.name, filename);
			...
			if (output.type === ASYNC_SPLIT_POINT) {
				for (const importFnRecord of output.importFnRecords) {
					const relativePath = getRequirePath(importFnRecord.output.name, filename);
		...
		// then shared + splits in (roughly) ascending order of depth
		const files = [];
	 ...
	 * @param {string} [name] - Output name (if not provided, filename will include hash of content)
	 * @param {boolean} isEntry - `true` if this is an entry point
	 * @returns {Array<Object>} - Array of file objects of form `{filename, content}`.
	 */
	outputOne(record, name, isEntry) {
		// Prepare export node
		const {options} = this;
		let {format} = options;
		if (isEntry) {
		...
		// Get filename
		if (!name) name = createSharedChunkName(js, this.hashCounters);
		const filename = `${name}.${JS_FILE_EXT}`;
		...
		if (sourceMaps) {
			...
			if (sourceMaps === 'inline') {
				...
				const mapFilename = `${filename}.${MAP_FILE_EXT}`;
				js += `\n${generateMapFileComment(mapFilename.replace(/^.+\//, ''))}`;
				mapFile = {
...
/**
 * Create name for unnamed chunk, based on hash of contents.
 * Uses SHA1 hash, Base32-encoded and truncated to 8 characters.
 * This is same as ESBuild uses.
 * Every hash will be unique, even if content is the same
 * (uses `hashCounters` to store hashes already used).
 * @param {string} content - File content
 * @param {Object} hashCounters - Hash counters object
 * @returns {string} - Name for file (does not include file extension)
 */
function createSharedChunkName(content, hashCounters) {
	const hash = getHash(content, hashCounters);
	return `${SHARED_FILENAME_PREFIX}${hash}`;
}

function getHash(content, hashCounters) {
	const hash = sha1Base32(content);
	const numUses = hashCounters[hash] || 0;
	hashCounters[hash] = numUses + 1;
	if (numUses === 0) return hash;
	return sha1Base32(`${hash}-${numUses}`);
}
